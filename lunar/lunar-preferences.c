...
static void     lunar_preferences_finalize           (GObject                *object);
static void     lunar_preferences_get_property       (GObject                *object,
                                                       guint                   prop_id,
                                                       GValue                 *value,
                                                       GParamSpec             *pspec);
static void     lunar_preferences_set_property       (GObject                *object,
                                                       guint                   prop_id,
                                                       const GValue           *value,
                                                       GParamSpec             *pspec);
static void     lunar_preferences_prop_changed       (EsconfChannel          *channel,
                                                       const gchar            *prop_name,
                                                       const GValue           *value,
                                                       LunarPreferences      *preferences);
static void     lunar_preferences_load_rc_file       (LunarPreferences      *preferences);



struct _LunarPreferencesClass
{
  GObjectClass __parent__;
};

struct _LunarPreferences
{
  GObject __parent__;
...
/* don't do anything in case esconf_init() failed */
static gboolean no_esconf = FALSE;



G_DEFINE_TYPE (LunarPreferences, lunar_preferences, G_TYPE_OBJECT)
...
static void
lunar_preferences_class_init (LunarPreferencesClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_preferences_finalize;
  gobject_class->get_property = lunar_preferences_get_property;
  gobject_class->set_property = lunar_preferences_set_property;

  /**
   * LunarPreferences:default-view:
   *
   * The name of the widget class, which should be used for the
   * view pane in new #LunarWindow<!---->s or "void" to use the
  ...
  /**
   * LunarPreferences:hidden-bookmarks:
   *
  ...
  /**
   * LunarPreferences:last-compact-view-zoom-level:
   *
   * The last selected #LunarZoomLevel for the #LunarCompactView.
   **/
  ...
  /**
   * LunarPreferences:last-details-view-column-order:
   *
   * The comma separated list of columns that specifies the order of the
   * columns in the #LunarDetailsView.
   **/
  preferences_props[PROP_LAST_DETAILS_VIEW_COLUMN_ORDER] =
      g_param_spec_string ("last-details-view-column-order",
                           ...
                           NULL,
                           "LUNAR_COLUMN_NAME,LUNAR_COLUMN_SIZE,LUNAR_COLUMN_SIZE_IN_BYTES,LUNAR_COLUMN_TYPE,LUNAR_COLUMN_DATE_MODIFIED",
                           ENDO_PARAM_READWRITE);

  /**
   * LunarPreferences:last-details-view-column-widths:
   *
  ...
  /**
   * LunarPreferences:last-details-view-fixed-columns:
   *
   * %TRUE to use fixed column widths in the #LunarDetailsView. Else the
  ...
  /**
   * LunarPreferences:last-details-view-visible-columns:
   *
   * The comma separated list of visible columns in the #LunarDetailsView.
   **/
  preferences_props[PROP_LAST_DETAILS_VIEW_VISIBLE_COLUMNS] =
      g_param_spec_string ("last-details-view-visible-columns",
                           ...
                           NULL,
                           "LUNAR_COLUMN_DATE_MODIFIED,LUNAR_COLUMN_NAME,LUNAR_COLUMN_SIZE,LUNAR_COLUMN_TYPE",
                           ENDO_PARAM_READWRITE);

  /**
   * LunarPreferences:last-details-view-zoom-level:
   *
   * The last selected #LunarZoomLevel for the #LunarDetailsView.
   **/
  ...
  /**
   * LunarPreferences:last-icon-view-zoom-level:
   *
   * The last selected #LunarZoomLevel for the #LunarIconView.
   **/
  ...
  /**
   * LunarPreferences:last-location-bar:
   *
   * The name of the widget class, which should be used for the
   * location bar in #LunarWindow<!---->s or "void" to hide the
   * location bar.
   **/
  ...
  /**
   * LunarPreferences:last-menubar-visible:
   *
  ...
  /**
   * LunarPreferences:last-separator-position:
   *
  ...
  /**
   * LunarPreferences:last-show-hidden:
   *
  ...
  /**
   * LunarPreferences:misc-directory-specific-settings:
   *
  ...
  /**
   * LunarPreferences:last-side-pane:
   *
   * The name of the widget class, which should be used for the
   * side pane in #LunarWindow<!---->s or "void" to hide the
  ...
  /**
   * LunarPreferences:last-sort-column:
   *
  ...
  /**
   * LunarPreferences:last-sort-order:
   *
  ...
  preferences_props[PROP_LAST_SORT_ORDER] =
      g_param_spec_enum ("last-sort-order",
                         ...
                         ENDO_PARAM_READWRITE);
  /**
   * LunarPreferences:last-statusbar-visible:
   *
  ...
  /**
   * LunarPreferences:last-view:
   *
   * The name of the widget class, which should be used for the
   * main view component in #LunarWindow<!---->s.
   **/
  ...
  /**
   * LunarPreferences:last-window-height:
   *
   * The last known height of a #LunarWindow, which will be used as
  ...
  /**
   * LunarPreferences:last-window-width:
   *
   * The last known width of a #LunarWindow, which will be used as
  ...
  /**
   * LunarPreferences:last-window-maximized:
   *
   * The last known maximized state of a #LunarWindow, which will be used as
   * default width for newly created windows.
   **/
  ...
  /**
   * LunarPreferences:misc-always-show-tabs:
   *
  ...
  /**
   * LunarPreferences:misc-case-sensitive:
   *
  ...
  /**
   * LunarPreferences:misc-date-style:
   *
  ...
  /**
   * LunarPreferences:misc-date-custom-style:
   *
  ...
  /**
   * LunarPreferences:misc-execute-shell-scripts-by-default:
   *
  ...
  /**
   * LunarPreferences:misc-folders-first:
   *
  ...
  /**
   * LunarPreferences:misc-full-path-in-title:
   *
  ...
  /**
   * LunarPreferences:misc-horizontal-wheel-navigates:
   *
   * Whether the horizontal mouse wheel is used to navigate
   * forth and back within a Lunar view.
   **/
  ...
  /**
   * LunarPreferences:misc-image-size-in-statusbar:
   *
  ...
  /**
   * LunarPreferences:misc-middle-click-in-tab:
   *
  ...
  /**
   * LunarPreferences:misc-open-new-window-as_tab:
   *
   * %TRUE to open a new tab in an existing lunar instance. instead of a new window
   * if a lunar windows already is present
   **/
  ...
  /**
   * LunarPreferences:misc-recursive-permissions:
   *
  ...
  preferences_props[PROP_MISC_RECURSIVE_PERMISSIONS] =
      g_param_spec_enum ("misc-recursive-permissions",
                         ...
                         NULL,
                         LUNAR_TYPE_RECURSIVE_PERMISSIONS,
                         LUNAR_RECURSIVE_PERMISSIONS_ASK,
                         ENDO_PARAM_READWRITE);

  /**
   * LunarPreferences:misc-remember-geometry:
   *
   * Whether Lunar should remember the size of windows and apply
  ...
  /**
   * LunarPreferences:misc-show-about-templates:
   *
 ...
 /**
   * LunarPreferences:misc-show-delete-action:
   *
  ...
  preferences_props[PROP_MISC_SHOW_DELETE_ACTION] =
      g_param_spec_boolean ("misc-show-delete-action",
                            ...
                            NULL,
                            !lunar_g_vfs_is_uri_scheme_supported ("trash"),
                            ENDO_PARAM_READWRITE);

  /**
   * LunarPreferences:misc-single-click:
   *
  ...
  /**
   * LunarPreferences:misc-single-click-timeout:
   *
  ...
  /**
   * LunarPreferences:misc-small-toolbar-icons:
   *
  ...
  /**
   * LunarPreferences:misc-tab-close-middle-click:
   *
  ...
  /**
   * LunarPreferences:misc-text-beside-icons:
   *
  ...
  /**
   * LunarPreferences:misc-thumbnail-mode:
   *
  ...
  /**
   * LunarPreferences:misc-thumbnail-draw-frames:
   *
  ...
  /**
   * LunarPreferences:misc-file-size-binary:
   *
  ...
  /**
   * LunarPreferences:misc-parallel-copy-mode:
   *
  ...
  /**
   * LunarPreferences:misc-change-window-icon:
   *
  ...
  /**
   * LunarPreferences:misc-confirm-close-multiple-tabs:
   *
  ...
  /**
   * LunarPreferences:shortcuts-icon-emblems:
   *
  ...
  /**
   * LunarPreferences:shortcuts-icon-size:
   *
  ...
  preferences_props[PROP_SHORTCUTS_ICON_SIZE] =
      g_param_spec_enum ("shortcuts-icon-size",
                         "ShortcutsIconSize",
                         NULL,
                         LUNAR_TYPE_ICON_SIZE,
                         LUNAR_ICON_SIZE_24,
                         ENDO_PARAM_READWRITE);

  /**
   * LunarPreferences:tree-icon-emblems:
   *
   * Whether to display emblems for file icons (if defined) in the
   * tree side pane.
   **/
  ...
  /**
   * LunarPreferences:tree-icon-size:
   *
   * The icon size to use for the icons displayed in the
   * tree side pane.
   **/
...
static void
lunar_preferences_init (LunarPreferences *preferences)
{
  const gchar check_prop[] = "/last-view";

  /* don't set a channel if esconf init failed */
  if (no_esconf)
    return;

  /* load the channel */
  preferences->channel = esconf_channel_get ("lunar");

  /* check one of the property to see if there are values */
  if (!esconf_channel_has_property (preferences->channel, check_prop))
    {
      /* try to load the old config file */
      lunar_preferences_load_rc_file (preferences);

      /* set the string we check */
      if (!esconf_channel_has_property (preferences->channel, check_prop))
        esconf_channel_set_string (preferences->channel, check_prop, "LunarIconView");
    }

  preferences->property_changed_id =
    g_signal_connect (G_OBJECT (preferences->channel), "property-changed",
                      G_CALLBACK (lunar_preferences_prop_changed), preferences);
}



static void
lunar_preferences_finalize (GObject *object)
{
  LunarPreferences *preferences = LUNAR_PREFERENCES (object);

  /* disconnect from the updates */
  g_signal_handler_disconnect (preferences->channel, preferences->property_changed_id);

  (*G_OBJECT_CLASS (lunar_preferences_parent_class)->finalize) (object);
}



static void
lunar_preferences_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  if (G_VALUE_TYPE (value) == G_TYPE_STRV)
    {
      ...
      array = esconf_channel_get_string_list (preferences->channel, prop_name);
      g_value_take_boxed (value, array);
    }
  else if (esconf_channel_get_property (preferences->channel, prop_name, &src))
    {
      if (G_VALUE_TYPE (value) == G_VALUE_TYPE (&src))
        g_value_copy (&src, value);
      else if (!g_value_transform (&src, value))
        g_printerr ("Lunar: Failed to transform property %s\n", prop_name);
      g_value_unset (&src);
    }
  else
    {
...
static void
lunar_preferences_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  if (G_VALUE_HOLDS_ENUM (value))
    {
      ...
      if (g_value_transform (value, &dst))
        esconf_channel_set_property (preferences->channel, prop_name, &dst);
      g_value_unset (&dst);
    }
  else if (G_VALUE_HOLDS (value, G_TYPE_STRV))
    {
      /* convert to a GValue GPtrArray in esconf */
      array = g_value_get_boxed (value);
      if (array != NULL && *array != NULL)
        esconf_channel_set_string_list (preferences->channel, prop_name, (const gchar * const *) array);
      else
        esconf_channel_reset_property (preferences->channel, prop_name, FALSE);
    }
  else
    {
      /* other types we support directly */
      esconf_channel_set_property (preferences->channel, prop_name, value);
    }
...
static void
lunar_preferences_prop_changed (EsconfChannel     *channel,
                                 const gchar       *prop_name,
                                 const GValue      *value,
                                 LunarPreferences *preferences)
{
...
static void
lunar_preferences_load_rc_file (LunarPreferences *preferences)
{
  GParamSpec  **pspecs;
  GParamSpec   *pspec;
  ExpidusRc       *rc;
  ...
  /* find file */
  filename = expidus_resource_lookup (EXPIDUS_RESOURCE_CONFIG, "Lunar/lunarrc");
  ...
  for (n = 0; n < nspecs; ++n)
    {
      ...
      string = expidus_rc_read_entry (rc, nick, NULL);
      if (G_UNLIKELY (string == NULL))
        continue;

      /* esconf property name, continue if exists */
      g_snprintf (prop_name, sizeof (prop_name), "/%s", g_param_spec_get_name (pspec));
      if (esconf_channel_has_property (preferences->channel, prop_name))
        continue;
      ...
      if (G_IS_PARAM_SPEC_STRING (pspec) || G_IS_PARAM_SPEC_ENUM (pspec))
        {
          esconf_channel_set_property (preferences->channel, prop_name, &src);
        }
  ...
  if (!expidus_rc_read_bool_entry (rc, "MiscShowThumbnails", TRUE))
    {
      esconf_channel_set_string (preferences->channel,
                                 "/misc-thumbnail-mode",
  ...
  g_print ("\n\n"
           "Your Lunar settings have been migrated to Esconf.\n"
           "The config file \"%s\"\n"
           "is not used anymore.\n\n", filename);
...
/**
 * lunar_preferences_get:
 *
 * Queries the global #LunarPreferences instance, which is shared
 * by all modules. The function automatically takes a reference
 * for the caller, so you'll need to call g_object_unref() when
 * you're done with it.
 *
 * Return value: the global #LunarPreferences instance.
 **/
LunarPreferences*
lunar_preferences_get (void)
{
  static LunarPreferences *preferences = NULL;

  if (G_UNLIKELY (preferences == NULL))
    {
      preferences = g_object_new (LUNAR_TYPE_PREFERENCES, NULL);
...
void
lunar_preferences_esconf_init_failed (void)
{
  no_esconf = TRUE;
}
