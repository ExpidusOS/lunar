...
#define LUNAR_ICON_FACTORY_SWEEP_TIMEOUT (30)
...
typedef struct _LunarIconKey LunarIconKey;



static void       lunar_icon_factory_dispose               (GObject                  *object);
static void       lunar_icon_factory_finalize              (GObject                  *object);
static void       lunar_icon_factory_get_property          (GObject                  *object,
                                                             guint                     prop_id,
                                                             GValue                   *value,
                                                             GParamSpec               *pspec);
static void       lunar_icon_factory_set_property          (GObject                  *object,
                                                             guint                     prop_id,
                                                             const GValue             *value,
                                                             GParamSpec               *pspec);
static gboolean   lunar_icon_factory_changed               (GSignalInvocationHint    *ihint,
                                                             guint                     n_param_values,
                                                             const GValue             *param_values,
                                                             gpointer                  user_data);
static gboolean   lunar_icon_factory_sweep_timer           (gpointer                  user_data);
static void       lunar_icon_factory_sweep_timer_destroy   (gpointer                  user_data);
static GdkPixbuf *lunar_icon_factory_load_from_file        (LunarIconFactory        *factory,
                                                             const gchar              *path,
                                                             gint                      size);
static GdkPixbuf *lunar_icon_factory_lookup_icon           (LunarIconFactory        *factory,
                                                             const gchar              *name,
                                                             gint                      size,
                                                             gboolean                  wants_default);
static guint      lunar_icon_key_hash                      (gconstpointer             data);
static gboolean   lunar_icon_key_equal                     (gconstpointer             a,
                                                             gconstpointer             b);
static void       lunar_icon_key_free                      (gpointer                  data);
static GdkPixbuf *lunar_icon_factory_load_fallback         (LunarIconFactory        *factory,
                                                             gint                      size);



struct _LunarIconFactoryClass
{
  GObjectClass __parent__;
};

struct _LunarIconFactory
{
  GObject __parent__;
  ...
  LunarThumbnailMode  thumbnail_mode;
...
static GQuark lunar_icon_factory_quark = 0;
static GQuark lunar_icon_factory_store_quark = 0;



G_DEFINE_TYPE (LunarIconFactory, lunar_icon_factory, G_TYPE_OBJECT)



static void
lunar_icon_factory_class_init (LunarIconFactoryClass *klass)
{
  GObjectClass *gobject_class;

  lunar_icon_factory_store_quark = g_quark_from_static_string ("lunar-icon-factory-store");

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->dispose = lunar_icon_factory_dispose;
  gobject_class->finalize = lunar_icon_factory_finalize;
  gobject_class->get_property = lunar_icon_factory_get_property;
  gobject_class->set_property = lunar_icon_factory_set_property;

  /**
   * LunarIconFactory:icon-theme:
   *
   * The #GtkIconTheme on which the given #LunarIconFactory instance operates
   * on.
   **/
  ...
  /**
   * LunarIconFactory:thumbnail-mode:
   *
   * Whether this #LunarIconFactory will try to generate and load thumbnails
   * when loading icons for #LunarFile<!---->s.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_THUMBNAIL_MODE,
                                   g_param_spec_enum ("thumbnail-mode",
                                                      "thumbnail-mode",
                                                      "thumbnail-mode",
                                                      LUNAR_TYPE_THUMBNAIL_MODE,
                                                      LUNAR_THUMBNAIL_MODE_ONLY_LOCAL,
                                                      ENDO_PARAM_READWRITE));

  /**
   * LunarIconFactory:thumbnail-draw-frames:
   * 
...
static void
lunar_icon_factory_init (LunarIconFactory *factory)
{
  factory->thumbnail_mode = LUNAR_THUMBNAIL_MODE_ONLY_LOCAL;
  factory->thumbnail_size = LUNAR_THUMBNAIL_SIZE_NORMAL;
  ...
  factory->changed_hook_id = g_signal_add_emission_hook (g_signal_lookup ("changed", GTK_TYPE_ICON_THEME),
                                                         0, lunar_icon_factory_changed, factory, NULL);

  /* allocate the hash table for the icon cache */
  factory->icon_cache = g_hash_table_new_full (lunar_icon_key_hash, lunar_icon_key_equal,
                                               lunar_icon_key_free, g_object_unref);
}



static void
lunar_icon_factory_dispose (GObject *object)
{
  LunarIconFactory *factory = LUNAR_ICON_FACTORY (object);

  _lunar_return_if_fail (LUNAR_IS_ICON_FACTORY (factory));

  if (G_UNLIKELY (factory->sweep_timer_id != 0))
    g_source_remove (factory->sweep_timer_id);

  (*G_OBJECT_CLASS (lunar_icon_factory_parent_class)->dispose) (object);
}



static void
lunar_icon_factory_finalize (GObject *object)
{
  LunarIconFactory *factory = LUNAR_ICON_FACTORY (object);
  ...
  if (G_LIKELY (factory->icon_theme != NULL))
    {
      g_object_set_qdata (G_OBJECT (factory->icon_theme), lunar_icon_factory_quark, NULL);
  ...
  (*G_OBJECT_CLASS (lunar_icon_factory_parent_class)->finalize) (object);
}



static void
lunar_icon_factory_get_property (GObject    *object,
                                  guint       prop_id,
                                  GValue     *value,
                                  GParamSpec *pspec)
{
...
static void
lunar_icon_factory_set_property (GObject      *object,
                                  guint         prop_id,
                                  const GValue *value,
                                  GParamSpec   *pspec)
{
...
static gboolean
lunar_icon_factory_changed (GSignalInvocationHint *ihint,
                             guint                  n_param_values,
                             const GValue          *param_values,
                             gpointer               user_data)
{
  LunarIconFactory *factory = LUNAR_ICON_FACTORY (user_data);
...
static gboolean
lunar_icon_check_sweep (LunarIconKey *key,
                         GdkPixbuf     *pixbuf)
{
...
static gboolean
lunar_icon_factory_sweep_timer (gpointer user_data)
{
  LunarIconFactory *factory = LUNAR_ICON_FACTORY (user_data);

  _lunar_return_val_if_fail (LUNAR_IS_ICON_FACTORY (factory), FALSE);

LUNAR_THREADS_ENTER

  /* ditch all icons whose ref_count is 1 */
  g_hash_table_foreach_remove (factory->icon_cache,
                               (GHRFunc) (void (*)(void)) lunar_icon_check_sweep,
                               factory);
...
static void
lunar_icon_factory_sweep_timer_destroy (gpointer user_data)
{
  LUNAR_ICON_FACTORY (user_data)->sweep_timer_id = 0;
}
...
static GdkPixbuf*
lunar_icon_factory_get_thumbnail_frame (void)
{
  GInputStream *stream;
  ...
  stream = g_resources_open_stream ("/com/expidus/lunar/thumbnail-frame.png", 0, NULL);
...
static GdkPixbuf*
lunar_icon_factory_load_from_file (LunarIconFactory *factory,
                                    const gchar       *path,
                                    gint               size)
{
  GdkPixbuf *pixbuf;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_ICON_FACTORY (factory), NULL);
      ...
      if (G_LIKELY (width > max_width || height > max_height))
        {
          ...
          tmp = endo_gdk_pixbuf_scale_down (pixbuf, TRUE, MAX (1, max_height), MAX (1, max_height));
      ...
      if (G_LIKELY (needs_frame))
        {
          ...
          frame = lunar_icon_factory_get_thumbnail_frame ();
          tmp = endo_gdk_pixbuf_frame (pixbuf, frame, 4, 3, 5, 6);
          g_object_unref (G_OBJECT (pixbuf));
          pixbuf = tmp;
        }
    }
...
static GdkPixbuf*
lunar_icon_factory_lookup_icon (LunarIconFactory *factory,
                                 const gchar       *name,
                                 gint               size,
                                 gboolean           wants_default)
{
  LunarIconKey  lookup_key;
  LunarIconKey *key;
  GtkIconInfo   *icon_info;
  GdkPixbuf     *pixbuf = NULL;

  _lunar_return_val_if_fail (LUNAR_IS_ICON_FACTORY (factory), NULL);
  _lunar_return_val_if_fail (name != NULL && *name != '\0', NULL);
  _lunar_return_val_if_fail (size > 0, NULL);
      ...
      if (G_UNLIKELY (g_path_is_absolute (name)))
        {
          ...
          pixbuf = lunar_icon_factory_load_from_file (factory, name, size);
        }
      else
        {
      ...
      if (G_UNLIKELY (pixbuf == NULL))
        {
          ...
          if (!wants_default)
            ...
            return lunar_icon_factory_load_fallback (factory, size);
        }

      /* generate a key for the new cached icon */
      key = g_slice_new (LunarIconKey);
      key->size = size;
  ...
  if (G_UNLIKELY (factory->sweep_timer_id == 0))
    {
      factory->sweep_timer_id = g_timeout_add_seconds_full (G_PRIORITY_LOW, LUNAR_ICON_FACTORY_SWEEP_TIMEOUT,
                                                            lunar_icon_factory_sweep_timer, factory,
                                                            lunar_icon_factory_sweep_timer_destroy);
    }
...
static guint
lunar_icon_key_hash (gconstpointer data)
{
  const LunarIconKey *key = data;
  const gchar         *p;
  guint                h;
...
static gboolean
lunar_icon_key_equal (gconstpointer a,
                       gconstpointer b)
{
  const LunarIconKey *a_key = a;
  const LunarIconKey *b_key = b;
  ...
  return endo_str_is_equal (a_key->name, b_key->name);
}



static void
lunar_icon_key_free (gpointer data)
{
  LunarIconKey *key = data;

  g_free (key->name);
  g_slice_free (LunarIconKey, key);
}



static void
lunar_icon_store_free (gpointer data)
{
  LunarIconStore *store = data;

  if (store->icon != NULL)
    g_object_unref (store->icon);
  g_slice_free (LunarIconStore, store);
}



static GdkPixbuf*
lunar_icon_factory_load_fallback (LunarIconFactory *factory,
                                   gint               size)
{
  return lunar_icon_factory_lookup_icon (factory, "text-x-generic", size, FALSE);
}



/**
 * lunar_icon_factory_get_default:
 *
 * Returns the #LunarIconFactory that operates on the default #GtkIconTheme.
 * The default #LunarIconFactory instance will be around for the time the
 ...
 *
 * Return value: the #LunarIconFactory for the default icon theme.
 **/
LunarIconFactory*
lunar_icon_factory_get_default (void)
{
  static LunarIconFactory *factory = NULL;

  if (G_UNLIKELY (factory == NULL))
    {
      factory = lunar_icon_factory_get_for_icon_theme (gtk_icon_theme_get_default ());
...
/**
 * lunar_icon_factory_get_for_icon_theme:
 * @icon_theme : a #GtkIconTheme instance.
 *
 * Determines the proper #LunarIconFactory to be used with the specified
 * @icon_theme and returns it.
 *
 * You need to explicitly free the returned #LunarIconFactory object
 * using g_object_unref() when you are done with it.
 *
 * Return value: the #LunarIconFactory for @icon_theme.
 **/
LunarIconFactory*
lunar_icon_factory_get_for_icon_theme (GtkIconTheme *icon_theme)
{
  LunarIconFactory *factory;

  _lunar_return_val_if_fail (GTK_IS_ICON_THEME (icon_theme), NULL);

  /* generate the quark on-demand */
  if (G_UNLIKELY (lunar_icon_factory_quark == 0))
    lunar_icon_factory_quark = g_quark_from_static_string ("lunar-icon-factory");

  /* check if the given icon theme already knows about an icon factory */
  factory = g_object_get_qdata (G_OBJECT (icon_theme), lunar_icon_factory_quark);
  if (G_UNLIKELY (factory == NULL))
    {
      /* allocate a new factory and connect it to the icon theme */
      factory = g_object_new (LUNAR_TYPE_ICON_FACTORY, NULL);
      factory->icon_theme = GTK_ICON_THEME (g_object_ref (G_OBJECT (icon_theme)));
      g_object_set_qdata (G_OBJECT (factory->icon_theme), lunar_icon_factory_quark, factory);

      /* connect the "show-thumbnails" property to the global preference */
      factory->preferences = lunar_preferences_get ();
      endo_binding_new (G_OBJECT (factory->preferences), "misc-thumbnail-mode",
...
/**
 * lunar_icon_factory_get_show_thumbnail:
 * @factory       : a #LunarIconFactory instance.
 * @file          : a #LunarFile.
 *
 * Return value: if a Thumbnail show be shown for @file.
 **/
gboolean
lunar_icon_factory_get_show_thumbnail (const LunarIconFactory *factory,
                                        const LunarFile        *file)
{
  GFilesystemPreviewType preview;

  _lunar_return_val_if_fail (LUNAR_IS_ICON_FACTORY (factory), LUNAR_THUMBNAIL_MODE_NEVER);
  _lunar_return_val_if_fail (file == NULL || LUNAR_IS_FILE (file), LUNAR_THUMBNAIL_MODE_NEVER);

  if (file == NULL
      || factory->thumbnail_mode == LUNAR_THUMBNAIL_MODE_NEVER)
    return FALSE;

  /* always create thumbs for local files */
  if (lunar_file_is_local (file))
    return TRUE;

  preview = lunar_file_get_preview_type (file);
  ...
  if (factory->thumbnail_mode == LUNAR_THUMBNAIL_MODE_ONLY_LOCAL)
 ...
 **/
GdkPixbuf*
lunar_icon_factory_load_icon (LunarIconFactory        *factory,
                               const gchar              *name,
                               gint                      size,
                               gboolean                  wants_default)
{
   ...
   */
  if (G_UNLIKELY (endo_str_is_empty (name)))
    {
  ...
  /* lookup the icon */
  return lunar_icon_factory_lookup_icon (factory, name, size, wants_default);
}



/**
 * lunar_icon_factory_load_file_icon:
 * @factory    : a #LunarIconFactory instance.
 * @file       : a #LunarFile.
 ...
 **/
GdkPixbuf*
lunar_icon_factory_load_file_icon (LunarIconFactory  *factory,
                                    LunarFile         *file,
                                    LunarFileIconState icon_state,
                                    gint                icon_size)
{
  ...
  if (custom_icon != NULL)
    {
      ...
      icon = lunar_icon_factory_lookup_icon (factory, custom_icon, icon_size, FALSE);
      if (G_LIKELY (icon != NULL))
        return icon;
    }

  /* check if thumbnails are enabled and we can display a thumbnail for the item */
  if (lunar_icon_factory_get_show_thumbnail (factory, file)
      && (lunar_file_is_regular (file) || lunar_file_is_directory (file)) )
    {
      /* determine the preview icon first */
      gicon = lunar_file_get_preview_icon (file);
      ...
      if (gicon != NULL)
      ...
      else
        {
          ...
          thumbnail_path = lunar_file_get_thumbnail_path (file, factory->thumbnail_size);

          /* check if we have a valid path */
          if (thumbnail_path != NULL)
            {
              /* try to load the thumbnail */
              icon = lunar_icon_factory_load_from_file (factory, thumbnail_path, icon_size);
            }
        }
    }
  ...
  if (G_LIKELY (icon == NULL))
    {
      icon_name = lunar_file_get_icon_name (file, icon_state, factory->icon_theme);
      icon = lunar_icon_factory_load_icon (factory, icon_name, icon_size, TRUE);
    }

  if (G_LIKELY (icon != NULL))
    {
      store = g_slice_new (LunarIconStore);
      store->icon_size = icon_size;
      store->icon_state = icon_state;
      store->stamp = factory->theme_stamp;
      store->thumb_state = lunar_file_get_thumb_state (file);
      ...
      g_object_set_qdata_full (G_OBJECT (file), lunar_icon_factory_store_quark,
                               store, lunar_icon_store_free);
    }
...
/**
 * lunar_icon_factory_clear_pixmap_cache:
 * @file : a #LunarFile.
 *
 * Unset the pixmap cache on a file to force a reload on the next request.
 **/
void
lunar_icon_factory_clear_pixmap_cache (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));

  /* unset the data */
  if (lunar_icon_factory_store_quark != 0)
    g_object_set_qdata (G_OBJECT (file), lunar_icon_factory_store_quark, NULL);
}
