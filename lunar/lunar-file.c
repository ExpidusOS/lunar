...
static void               lunar_file_info_init                (LunarxFileInfoIface   *iface);
static void               lunar_file_dispose                  (GObject                *object);
static void               lunar_file_finalize                 (GObject                *object);
static gchar             *lunar_file_info_get_name            (LunarxFileInfo        *file_info);
static gchar             *lunar_file_info_get_uri             (LunarxFileInfo        *file_info);
static gchar             *lunar_file_info_get_parent_uri      (LunarxFileInfo        *file_info);
static gchar             *lunar_file_info_get_uri_scheme      (LunarxFileInfo        *file_info);
static gchar             *lunar_file_info_get_mime_type       (LunarxFileInfo        *file_info);
static gboolean           lunar_file_info_has_mime_type       (LunarxFileInfo        *file_info,
                                                                const gchar            *mime_type);
static gboolean           lunar_file_info_is_directory        (LunarxFileInfo        *file_info);
static GFileInfo         *lunar_file_info_get_file_info       (LunarxFileInfo        *file_info);
static GFileInfo         *lunar_file_info_get_filesystem_info (LunarxFileInfo        *file_info);
static GFile             *lunar_file_info_get_location        (LunarxFileInfo        *file_info);
static void               lunar_file_info_changed             (LunarxFileInfo        *file_info);
static gboolean           lunar_file_denies_access_permission (const LunarFile       *file,
                                                                LunarFileMode          usr_permissions,
                                                                LunarFileMode          grp_permissions,
                                                                LunarFileMode          oth_permissions);
static void               lunar_file_monitor                  (GFileMonitor           *monitor,
                                                                GFile                  *path,
                                                                GFile                  *other_path,
                                                                GFileMonitorEvent       event_type,
                                                                gpointer                user_data);
static void               lunar_file_watch_reconnect          (LunarFile             *file);
static gboolean           lunar_file_load                     (LunarFile             *file,
                                                                GCancellable           *cancellable,
                                                                GError                **error);
static gboolean           lunar_file_is_readable              (const LunarFile       *file);
static gboolean           lunar_file_same_filesystem          (const LunarFile       *file_a,
...
static LunarUserManager *user_manager;
static GHashTable        *file_cache;
static guint32            effective_user_id;
static GQuark             lunar_file_watch_quark;
static guint              file_signals[LAST_SIGNAL];



#define FLAG_SET_THUMB_STATE(file,new_state) G_STMT_START{ (file)->flags = ((file)->flags & ~LUNAR_FILE_FLAG_THUMB_MASK) | (new_state); }G_STMT_END
#define FLAG_GET_THUMB_STATE(file)           ((file)->flags & LUNAR_FILE_FLAG_THUMB_MASK)
...
typedef enum
{
  LUNAR_FILE_FLAG_THUMB_MASK     = 0x03,   /* storage for LunarFileThumbState */
  LUNAR_FILE_FLAG_IN_DESTRUCTION = 1 << 2, /* for avoiding recursion during destroy */
  LUNAR_FILE_FLAG_IS_MOUNTED     = 1 << 3, /* whether this file is mounted */
}
LunarFileFlags;

struct _LunarFileClass
{
  GObjectClass __parent__;

  /* signals */
  void (*destroy) (LunarFile *file);
};
...
G_DEFINE_TYPE_WITH_CODE (LunarFile, lunar_file, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE (LUNARX_TYPE_FILE_INFO, lunar_file_info_init))
...
#ifdef HAVE_ATEXIT
static gboolean lunar_file_atexit_registered = FALSE;



static void
lunar_file_atexit_foreach (gpointer key,
                            gpointer value,
                            gpointer user_data)
{
  gchar *uri;
...
lunar_file_atexit (void)
{
  ...
  g_print ("--- Leaked a total of %u LunarFile objects:\n",
           g_hash_table_size (file_cache));

  g_hash_table_foreach (file_cache, lunar_file_atexit_foreach, NULL);
...
#if DUMP_FILE_CACHE
static void
lunar_file_cache_dump_foreach (gpointer gfile,
                                gpointer value,
                                gpointer user_data)
{
  gchar *name;
...
static gboolean
lunar_file_cache_dump (gpointer user_data)
{
  G_LOCK (file_cache_mutex);

  if (file_cache != NULL)
    {
      g_print ("--- %d LunarFile objects in cache:\n",
               g_hash_table_size (file_cache));

      g_hash_table_foreach (file_cache, lunar_file_cache_dump_foreach, NULL);
...
static void
lunar_file_class_init (LunarFileClass *klass)
{
  GObjectClass *gobject_class;

#ifdef G_ENABLE_DEBUG
#ifdef HAVE_ATEXIT
  if (G_UNLIKELY (!lunar_file_atexit_registered))
    {
      atexit ((void (*)(void)) lunar_file_atexit);
      lunar_file_atexit_registered = TRUE;
    }
#endif
#endif

#if DUMP_FILE_CACHE
  g_timeout_add_seconds (DUMP_FILE_CACHE, lunar_file_cache_dump, NULL);
#endif

  /* pre-allocate the required quarks */
  lunar_file_watch_quark = g_quark_from_static_string ("lunar-file-watch");

  /* grab a reference on the user manager */
  user_manager = lunar_user_manager_get_default ();

  /* determine the effective user id of the process */
  effective_user_id = geteuid ();

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->dispose = lunar_file_dispose;
  gobject_class->finalize = lunar_file_finalize;

  /**
   * LunarFile::destroy:
   * @file : the #LunarFile instance.
   *
  ...
  file_signals[DESTROY] =
    g_signal_new (I_("destroy"),
                  ...
                  G_STRUCT_OFFSET (LunarFileClass, destroy),
                  NULL, NULL,
...
static void
lunar_file_init (LunarFile *file)
{
}
...
static void
lunar_file_info_init (LunarxFileInfoIface *iface)
{
  iface->get_name = lunar_file_info_get_name;
  iface->get_uri = lunar_file_info_get_uri;
  iface->get_parent_uri = lunar_file_info_get_parent_uri;
  iface->get_uri_scheme = lunar_file_info_get_uri_scheme;
  iface->get_mime_type = lunar_file_info_get_mime_type;
  iface->has_mime_type = lunar_file_info_has_mime_type;
  iface->is_directory = lunar_file_info_is_directory;
  iface->get_file_info = lunar_file_info_get_file_info;
  iface->get_filesystem_info = lunar_file_info_get_filesystem_info;
  iface->get_location = lunar_file_info_get_location;
  iface->changed = lunar_file_info_changed;
}



static void
lunar_file_dispose (GObject *object)
{
  LunarFile *file = LUNAR_FILE (object);

  /* check that we don't recurse here */
  if (!FLAG_IS_SET (file, LUNAR_FILE_FLAG_IN_DESTRUCTION))
    {
      /* emit the "destroy" signal */
      FLAG_SET (file, LUNAR_FILE_FLAG_IN_DESTRUCTION);
      g_signal_emit (object, file_signals[DESTROY], 0);
      FLAG_UNSET (file, LUNAR_FILE_FLAG_IN_DESTRUCTION);
    }

  (*G_OBJECT_CLASS (lunar_file_parent_class)->dispose) (object);
}



static void
lunar_file_finalize (GObject *object)
{
  LunarFile *file = LUNAR_FILE (object);

  /* verify that nobody's watching the file anymore */
#ifdef G_ENABLE_DEBUG
  LunarFileWatch *file_watch = g_object_get_qdata (G_OBJECT (file), lunar_file_watch_quark);
  if (file_watch != NULL)
    {
      g_error ("Attempt to finalize a LunarFile, which has an active "
  ...
  (*G_OBJECT_CLASS (lunar_file_parent_class)->finalize) (object);
}



static gchar *
lunar_file_info_get_name (LunarxFileInfo *file_info)
{
  return g_strdup (lunar_file_get_basename (LUNAR_FILE (file_info)));
}



static gchar*
lunar_file_info_get_uri (LunarxFileInfo *file_info)
{
  return lunar_file_dup_uri (LUNAR_FILE (file_info));
}



static gchar*
lunar_file_info_get_parent_uri (LunarxFileInfo *file_info)
{
  GFile *parent;
  gchar *uri = NULL;

  parent = g_file_get_parent (LUNAR_FILE (file_info)->gfile);
...
static gchar*
lunar_file_info_get_uri_scheme (LunarxFileInfo *file_info)
{
  return g_file_get_uri_scheme (LUNAR_FILE (file_info)->gfile);
}



static gchar*
lunar_file_info_get_mime_type (LunarxFileInfo *file_info)
{
  return g_strdup (lunar_file_get_content_type (LUNAR_FILE (file_info)));
}



static gboolean
lunar_file_info_has_mime_type (LunarxFileInfo *file_info,
                                const gchar     *mime_type)
{
  if (LUNAR_FILE (file_info)->info == NULL)
    return FALSE;

  return g_content_type_is_a (lunar_file_get_content_type (LUNAR_FILE (file_info)), mime_type);
}



static gboolean
lunar_file_info_is_directory (LunarxFileInfo *file_info)
{
  return lunar_file_is_directory (LUNAR_FILE (file_info));
}



static GFileInfo *
lunar_file_info_get_file_info (LunarxFileInfo *file_info)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_info), NULL);

  if (LUNAR_FILE (file_info)->info != NULL)
    return g_object_ref (LUNAR_FILE (file_info)->info);
  else
    return NULL;
}
...
static GFileInfo *
lunar_file_info_get_filesystem_info (LunarxFileInfo *file_info)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_info), NULL);

  return g_file_query_filesystem_info (LUNAR_FILE (file_info)->gfile,
                                       LUNARX_FILESYSTEM_INFO_NAMESPACE,
                                       NULL, NULL);
}
...
static GFile *
lunar_file_info_get_location (LunarxFileInfo *file_info)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_info), NULL);
  return g_object_ref (LUNAR_FILE (file_info)->gfile);
}



static void
lunar_file_info_changed (LunarxFileInfo *file_info)
{
  LunarFile *file = LUNAR_FILE (file_info);

  _lunar_return_if_fail (LUNAR_IS_FILE (file_info));

  /* set the new thumbnail state manually, so we only emit file
   * changed once */
  FLAG_SET_THUMB_STATE (file, LUNAR_FILE_THUMB_STATE_UNKNOWN);

  /* tell the file monitor that this file changed */
  lunar_file_monitor_file_changed (file);
}



static gboolean
lunar_file_denies_access_permission (const LunarFile *file,
                                      LunarFileMode    usr_permissions,
                                      LunarFileMode    grp_permissions,
                                      LunarFileMode    oth_permissions)
{
  ...
  if (lunar_user_is_me (user))
    ...
    {
      group = lunar_file_get_group (file);
      if (G_LIKELY (group != NULL))
        {
          ...
          user = lunar_user_manager_get_user_by_id (user_manager, effective_user_id);
          if (G_LIKELY (user != NULL))
            {
              /* check the group permissions */
              groups = lunar_user_get_groups (user);
              for (lp = groups; lp != NULL; lp = lp->next)
                if (LUNAR_GROUP (lp->data) == group)
                  {
                    g_object_unref (G_OBJECT (user));
...
static void
lunar_file_monitor_update (GFile             *path,
                            GFileMonitorEvent  event_type)
{
  LunarFile *file;

  _lunar_return_if_fail (G_IS_FILE (path));
  file = lunar_file_cache_lookup (path);
...
static void
lunar_file_move_thumbnail_cache_file (GFile *old_file,
                                       GFile *new_file)
{
  LunarApplication    *application;
  LunarThumbnailCache *thumbnail_cache;

  _lunar_return_if_fail (G_IS_FILE (old_file));
  _lunar_return_if_fail (G_IS_FILE (new_file));

  application = lunar_application_get ();
  thumbnail_cache = lunar_application_get_thumbnail_cache (application);
  lunar_thumbnail_cache_move_file (thumbnail_cache, old_file, new_file);
...
static void
lunar_file_monitor_moved (LunarFile *file,
                           GFile      *renamed_file)
{
  GFile *previous_file;
...
void
lunar_file_reload_parent (LunarFile *file)
{
  LunarFile *parent = NULL;
  ...
  if (lunar_file_has_parent (file))
    {
      ...
      parent = lunar_file_cache_lookup (parent_file);
      g_object_unref (parent_file);
    }

  if (parent)
    {
      lunar_file_reload (parent);
      g_object_unref (parent);
    }
}
...
static void
lunar_file_monitor (GFileMonitor     *monitor,
                     GFile            *event_path,
                     GFile            *other_path,
                     GFileMonitorEvent event_type,
                     gpointer          user_data)
{
  ...
  LunarFile *other_file;
  gboolean    reload_ok = TRUE;

  _lunar_return_if_fail (G_IS_FILE_MONITOR (monitor));
  _lunar_return_if_fail (G_IS_FILE (event_path));
  _lunar_return_if_fail (LUNAR_IS_FILE (file));

  if (g_file_equal (event_path, file->gfile))
    {
      /* the event occurred for the monitored LunarFile */
      if (event_type == G_FILE_MONITOR_EVENT_RENAMED ||
          event_type == G_FILE_MONITOR_EVENT_MOVED_IN ||
          event_type == G_FILE_MONITOR_EVENT_MOVED_OUT)
        {
          G_LOCK (file_rename_mutex);
          lunar_file_monitor_moved (file, other_path);
          G_UNLOCK (file_rename_mutex);
          return;
        }

      if (G_LIKELY (event_path))
          lunar_file_monitor_update (event_path, event_type);
    }
  else
    {
      /* The event did not occur for the monitored LunarFile, but for
         a file that is contained in LunarFile which is actually a
         directory. */
      if (event_type == G_FILE_MONITOR_EVENT_RENAMED ||
          event_type == G_FILE_MONITOR_EVENT_MOVED_IN ||
          event_type == G_FILE_MONITOR_EVENT_MOVED_OUT)
        {
          ...
          other_file = lunar_file_cache_lookup (other_path);
          if (other_file)
              reload_ok = lunar_file_reload (other_file);
          else
              other_file = lunar_file_get (other_path, NULL);

          if (reload_ok && other_file != NULL)
            {
              /* notify the thumbnail cache that we can now also move the thumbnail */
              lunar_file_move_thumbnail_cache_file (event_path, other_path);

              /* reload the containing target folder */
              lunar_file_reload_parent (other_file);
...
static void
lunar_file_watch_destroyed (gpointer data)
{
  LunarFileWatch *file_watch = data;
  ...
  g_slice_free (LunarFileWatch, file_watch);
}



static void
lunar_file_watch_reconnect (LunarFile *file)
{
  LunarFileWatch *file_watch;

  /* recreate the monitor without changing the watch_count for file renames */
  file_watch = g_object_get_qdata (G_OBJECT (file), lunar_file_watch_quark);
  if (file_watch != NULL)
    {
      ...
      if (G_LIKELY (file_watch->monitor != NULL))
        {
          ...
          g_signal_connect (file_watch->monitor, "changed", G_CALLBACK (lunar_file_monitor), file);
        }
    }
}
...
static void
lunar_file_set_emblem_names_ready (GObject      *source_object,
                                    GAsyncResult *result,
                                    gpointer      user_data)
{
  LunarFile *file = LUNAR_FILE (user_data);
...
static void
lunar_file_info_clear (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));
  ...
  FLAG_SET (file, LUNAR_FILE_FLAG_IS_MOUNTED);
...
static void
lunar_file_info_reload (LunarFile   *file,
                         GCancellable *cancellable)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_FILE (file));
  _lunar_return_if_fail (file->info == NULL || G_IS_FILE_INFO (file->info));
      ...
      if (file->kind == G_FILE_TYPE_MOUNTABLE)
        {
          ...
          if (target_uri != NULL
            ...
            FLAG_SET (file, LUNAR_FILE_FLAG_IS_MOUNTED);
             ...
             FLAG_UNSET (file, LUNAR_FILE_FLAG_IS_MOUNTED);
        }
    }

  /* determine the basename */
  file->basename = g_file_get_basename (file->gfile);
  _lunar_assert (file->basename != NULL);
  ...
  if (lunar_file_is_desktop_file (file, &is_secure) && is_secure)
    {
      /* determine the custom icon and display name for .desktop files */

      /* query a key file for the .desktop file */
      key_file = lunar_g_file_query_key_file (file->gfile, cancellable, NULL);
      if (key_file != NULL)
        {
          ...
          if (G_UNLIKELY (endo_str_is_empty (file->custom_icon_name)))
            {
              /* make sure we set null if the string is empty else the assertion in
               * lunar_icon_factory_lookup_icon() will fail */
              g_free (file->custom_icon_name);
              file->custom_icon_name = NULL;
            }
          else
  ...
  if (file->display_name == NULL)
    {
      if (G_UNLIKELY (lunar_g_file_is_trash (file->gfile)))
      ...
      if (file->display_name == NULL)
        file->display_name = lunar_g_file_get_display_name (file->gfile);
    }
...
static void
lunar_file_get_async_finish (GObject      *object,
                              GAsyncResult *result,
                              gpointer      user_data)
{
  LunarFileGetData *data = user_data;
  LunarFile        *file;
  GFileInfo         *file_info;
  GError            *error = NULL;
  ...
  file = g_object_new (LUNAR_TYPE_FILE, NULL);
  file->gfile = g_object_ref (location);

  /* reset the file */
  lunar_file_info_clear (file);

  /* set the file information */
  file->info = file_info;

  /* update the file from the information */
  lunar_file_info_reload (file, data->cancellable);
  ...
  /* release the file, see description in LunarFileGetFunc */
  g_object_unref (file);
  ...
  g_slice_free (LunarFileGetData, data);
}
 ...
 **/
static gboolean
lunar_file_load (LunarFile   *file,
                  GCancellable *cancellable,
                  GError      **error)
{
  GError *err = NULL;
  ...
  _lunar_return_val_if_fail (cancellable == NULL || G_IS_CANCELLABLE (cancellable), FALSE);
  _lunar_return_val_if_fail (G_IS_FILE (file->gfile), FALSE);
  ...
  file->info = g_file_query_info (file->gfile,
                                  LUNARX_FILE_INFO_NAMESPACE,
                                  G_FILE_QUERY_INFO_NONE,
                                  cancellable, &err);

  /* update the file from the information */
  lunar_file_info_reload (file, cancellable);
 ...
 *
 * Looks up the #LunarFile referred to by @file. This function may return a
 * LunarFile even though the file doesn't actually exist. This is the case
 ...
 *
 * Return value: the #LunarFile for @file or %NULL on errors.
 **/
LunarFile*
lunar_file_get (GFile   *gfile,
                 GError **error)
{
  LunarFile *file;

  _lunar_return_val_if_fail (G_IS_FILE (gfile), NULL);
  ...
  if (G_UNLIKELY (file != NULL))
  ...
  else
    {
      ...
      if (lunar_file_load (file, NULL, error))
        {
 ...
 **/
LunarFile *
lunar_file_get_with_info (GFile     *gfile,
                           GFileInfo *info,
                           gboolean   not_mounted)
{
  ...
  _lunar_return_val_if_fail (G_IS_FILE (gfile), NULL);
  _lunar_return_val_if_fail (G_IS_FILE_INFO (info), NULL);
  ...
  if (G_UNLIKELY (file != NULL))
  ...
  else
    {
      ...
      /* update the file from the information */
      lunar_file_info_reload (file, NULL);
...
/**
 * lunar_file_get_for_uri:
 * @uri   : an URI or an absolute filename.
 * @error : return location for errors or %NULL.
 *
 * Convenience wrapper function for lunar_file_get_for_path(), as its
 * often required to determine a #LunarFile for a given @uri.
 *
 * The caller is responsible to free the returned object using
 * g_object_unref() when no longer needed.
 *
 * Return value: the #LunarFile for the given @uri or %NULL if
 *               unable to determine.
 **/
LunarFile*
lunar_file_get_for_uri (const gchar *uri,
                         GError     **error)
{
  LunarFile *file;
  GFile      *path;

  _lunar_return_val_if_fail (uri != NULL, NULL);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, NULL);

  path = g_file_new_for_commandline_arg (uri);
  file = lunar_file_get (path, error);
  g_object_unref (path);
...
/**
 * lunar_file_get_async:
 **/
void
lunar_file_get_async (GFile            *location,
                       GCancellable     *cancellable,
                       LunarFileGetFunc func,
                       gpointer          user_data)
{
  LunarFile        *file;
  LunarFileGetData *data;

  _lunar_return_if_fail (G_IS_FILE (location));
  _lunar_return_if_fail (func != NULL);

  /* check if we already have a cached version of that file */
  file = lunar_file_cache_lookup (location);
  if (G_UNLIKELY (file != NULL))
    {
    ...
    {
      /* allocate get data */
      data = g_slice_new0 (LunarFileGetData);
 ...
 **/
GFile *
lunar_file_get_file (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  _lunar_return_val_if_fail (G_IS_FILE (file->gfile), NULL);
  return file->gfile;
}
 ...
 **/
GFileInfo *
lunar_file_get_info (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  _lunar_return_val_if_fail (file->info == NULL || G_IS_FILE_INFO (file->info), NULL);
  return file->info;
}
...
/**
 * lunar_file_get_parent:
 * @file  : a #LunarFile instance.
 * @error : return location for errors.
 *
 * Determines the parent #LunarFile for @file. If @file has no parent or
 ...
 * describes the cause. Else, the #LunarFile will be returned, and
 * the caller must call g_object_unref() on it.
 *
 * You may want to call lunar_file_has_parent() first to
 * determine whether @file has a parent.
 *
 * Return value: the parent #LunarFile or %NULL.
 **/
LunarFile*
lunar_file_get_parent (const LunarFile *file,
                        GError          **error)
{
  LunarFile *parent = NULL;
  GFile      *parent_file;
  ...
  parent = lunar_file_get (parent_file, error);
  g_object_unref (parent_file);
 ...
 **/
gboolean
lunar_file_check_loaded (LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_execute (LunarFile  *file,
                     ...
                     GError     **error)
{
  ...
  location = lunar_g_file_get_location (file->gfile);
  for (li = file_list; li != NULL; li = li->next)
    uri_list = g_slist_prepend (uri_list, g_file_get_uri (li->data));
  uri_list = g_slist_reverse (uri_list);

  if (lunar_file_is_desktop_file (file, &is_secure))
    {
      /* parse file first, even if it is insecure */
      key_file = lunar_g_file_query_key_file (file->gfile, NULL, &err);
      if (key_file == NULL)
        {
      ...
      if (G_LIKELY (endo_str_is_equal (type, "Application")))
        {
          ...
          if (G_LIKELY (exec != NULL))
            {
              ...
              if (is_secure || lunar_dialogs_show_insecure_program (parent, _("Untrusted application launcher"), file, exec))
                {
                  /* parse other fields */
                  ...
                  command = expidus_expand_desktop_entry_field_codes (exec, uri_list, icon_name,
      ...
      else if (endo_str_is_equal (type, "Link"))
        {
          url = g_key_file_get_string (key_file, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_URL, NULL);
          if (G_LIKELY (url != NULL))
            {
              /* if the .desktop file is not secure, ask user what to do */
              if (is_secure || lunar_dialogs_show_insecure_program (parent, _("Untrusted link launcher"), file, url))
                {
  ...
  else
    {
      ...
      command = expidus_expand_desktop_entry_field_codes (exec, uri_list, NULL, NULL, NULL, FALSE);
      result = g_shell_parse_argv (command, NULL, &argv, error);
  ...
  if (G_LIKELY (result && argv != NULL))
    {
      ...
      if (G_LIKELY (directory == NULL))
        {
          ...
          if (G_LIKELY (working_directory != NULL))
          ...
          else if (file_list != NULL)
            {
              ...
              directory = (file_parent != NULL) ? lunar_g_file_get_location (file_parent) : NULL;
          ...
          else
            {
              ...
              directory = (parent != NULL) ? lunar_g_file_get_location (parent) : NULL;
              g_object_unref (parent);
            }
        }
      ...
      /* execute the command */
      result = expidus_spawn_on_screen (lunar_util_parse_parent (parent, NULL),
 ...
 * a directory, opens a new #LunarWindow to display the directory. Else,
 ...
 **/
gboolean
lunar_file_launch (LunarFile  *file,
                    gpointer     parent,
                    const gchar *startup_id,
                    GError     **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  _lunar_return_val_if_fail (parent == NULL || GDK_IS_SCREEN (parent) || GTK_IS_WIDGET (parent), FALSE);
  ...
  if (lunar_file_is_directory (file))
    {
      application = lunar_application_get ();
      lunar_application_open_window (application, file, screen, startup_id, FALSE);
      g_object_unref (G_OBJECT (application));
      return TRUE;
    }

  /* check if we should execute the file */
  if (lunar_file_is_executable (file))
    return lunar_file_execute (file, NULL, parent, NULL, NULL, error);

  /* determine the default application to open the file */
  /* TODO We should probably add a cancellable argument to lunar_file_launch() */
  app_info = lunar_file_get_default_handler (LUNAR_FILE (file));

  /* display the application chooser if no application is defined for this file
   * type yet */
  if (G_UNLIKELY (app_info == NULL))
    {
      lunar_show_chooser_dialog (parent, file, TRUE);
      return TRUE;
    }

  /* HACK: check if we're not trying to launch another file manager again, possibly
   * ourselfs which will end in a loop */
  if (g_strcmp0 (g_app_info_get_id (app_info), "endo-file-manager.desktop") == 0
      || g_strcmp0 (g_app_info_get_id (app_info), "lunar.desktop") == 0
      || g_strcmp0 (g_app_info_get_name (app_info), "endo-file-manager") == 0)
    {
 ...
 * lunar_file_is_renameable() method.
 *
 * Return value: %TRUE on success, else %FALSE.
 **/
gboolean
lunar_file_rename (LunarFile   *file,
                    const gchar  *name,
                    GCancellable *cancellable,
                    ...
                    GError      **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (g_utf8_validate (name, -1, NULL), FALSE);
  _lunar_return_val_if_fail (cancellable == NULL || G_IS_CANCELLABLE (cancellable), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (renamed_file != NULL)
    {
      ...
      lunar_file_monitor_moved (file, renamed_file);
 ...
 **/
GdkDragAction
lunar_file_accepts_drop (LunarFile     *file,
                          GList          *file_list,
                          ...
                          GdkDragAction  *suggested_action_return)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), 0);
  _lunar_return_val_if_fail (GDK_IS_DRAG_CONTEXT (context), 0);
  ...
  if (lunar_file_is_directory (file) && lunar_file_is_writable (file))
    {
      ...
      for (lp = file_list; lp != NULL; lp = lp->next)
        {
          ...
          if (G_UNLIKELY (lunar_g_file_is_trashed (lp->data) && lunar_file_is_trashed (file)))
            return 0;
        }
          ...
          for (lp = file_list; lp != NULL; lp = lp->next)
            {
              ...
              if (G_UNLIKELY (lunar_g_file_is_trashed (lp->data)))
                break;

              /* determine the cached version of the source file */
              ofile = lunar_file_cache_lookup (lp->data);

              /* fallback to non-cached version */
              if (ofile == NULL)
                ofile = lunar_file_get (lp->data, NULL);
               ...
               */
              if (ofile == NULL
                  || !lunar_file_same_filesystem (file, ofile)
                  || (ofile->info != NULL
  ...
  else if (lunar_file_is_executable (file))
    {
      /* determine the possible actions */
 ...
 **/
guint64
lunar_file_get_date (const LunarFile  *file,
 ...
 **/
gchar*
lunar_file_get_date_string (const LunarFile  *file,
                             LunarFileDateType date_type,
                             LunarDateStyle    date_style,
                             const gchar       *date_custom_style)
{
  return lunar_util_humanize_file_time (lunar_file_get_date (file, date_type), date_style, date_custom_style);
}



/**
 * lunar_file_get_mode_string:
 * @file : a #LunarFile instance.
 *
 ...
 **/
gchar*
lunar_file_get_mode_string (const LunarFile *file)
{
  LunarFileMode mode;
  GFileType      kind;
  gchar         *text;
  ...
  kind = lunar_file_get_kind (file);
  mode = lunar_file_get_mode (file);
  text = g_new (gchar, 11);

  /* file type */
  /* TODO earlier versions of Lunar had 'P' for ports and
  ...
  /* permission flags */
  text[1] = (mode & LUNAR_FILE_MODE_USR_READ)  ? 'r' : '-';
  text[2] = (mode & LUNAR_FILE_MODE_USR_WRITE) ? 'w' : '-';
  text[3] = (mode & LUNAR_FILE_MODE_USR_EXEC)  ? 'x' : '-';
  text[4] = (mode & LUNAR_FILE_MODE_GRP_READ)  ? 'r' : '-';
  text[5] = (mode & LUNAR_FILE_MODE_GRP_WRITE) ? 'w' : '-';
  text[6] = (mode & LUNAR_FILE_MODE_GRP_EXEC)  ? 'x' : '-';
  text[7] = (mode & LUNAR_FILE_MODE_OTH_READ)  ? 'r' : '-';
  text[8] = (mode & LUNAR_FILE_MODE_OTH_WRITE) ? 'w' : '-';
  text[9] = (mode & LUNAR_FILE_MODE_OTH_EXEC)  ? 'x' : '-';

  /* special flags */
  if (G_UNLIKELY (mode & LUNAR_FILE_MODE_SUID))
    text[3] = 's';
  if (G_UNLIKELY (mode & LUNAR_FILE_MODE_SGID))
    text[6] = 's';
  if (G_UNLIKELY (mode & LUNAR_FILE_MODE_STICKY))
    text[9] = 't';
 ...
 **/
gchar *
lunar_file_get_size_string (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  return g_format_size (lunar_file_get_size (file));
}



/**
 * lunar_file_get_size_in_bytes_string:
 * @file : a #LunarFile instance.
 *
 ...
 **/
gchar *
lunar_file_get_size_in_bytes_string (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  return g_strdup_printf ("%'"G_GUINT64_FORMAT, lunar_file_get_size (file));
}



/**
 * lunar_file_get_size_string_formatted:
 * @file             : a #LunarFile instance.
 ...
 **/
gchar *
lunar_file_get_size_string_formatted (const LunarFile *file, const gboolean file_size_binary)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  return g_format_size_full (lunar_file_get_size (file),
...
/**
 * lunar_file_get_size_string_long:
 * @file             : a #LunarFile instance.
 * @file_size_binary : indicates if file size format
 *                     should be binary or not.
 *
 ...
 **/
gchar *
lunar_file_get_size_string_long (const LunarFile *file, const gboolean file_size_binary)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  return g_format_size_full (lunar_file_get_size (file),
 ...
 **/
GVolume*
lunar_file_get_volume (const LunarFile *file)
{
  GVolume *volume = NULL;
  GMount  *mount;
...
/**
 * lunar_file_get_group:
 * @file : a #LunarFile instance.
 *
 * Determines the #LunarGroup for @file. If there's no
 ...
 *
 * Return value: the #LunarGroup for @file or %NULL.
 **/
LunarGroup *
lunar_file_get_group (const LunarFile *file)
{
  guint32 gid;
  ...
  return lunar_user_manager_get_group_by_id (user_manager, gid);
}
 ...
 * @file : a #LunarFile instance.
 *
 * Determines the #LunarUser for @file. If there's no
 ...
 * using g_object_unref().
 *
 * Return value: the #LunarUser for @file or %NULL.
 **/
LunarUser*
lunar_file_get_user (const LunarFile *file)
{
  guint32 uid;
  ...
  return lunar_user_manager_get_user_by_id (user_manager, uid);
}



/**
 * lunar_file_get_content_type:
 * @file : a #LunarFile.
 *
 ...
 **/
const gchar *
lunar_file_get_content_type (LunarFile *file)
{
  GFileInfo   *info;
  GError      *err = NULL;
...
gboolean
lunar_file_load_content_type (LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), TRUE);

  if (file->content_type != NULL)
    return FALSE;

  lunar_file_get_content_type (file);
 ...
 **/
const gchar *
lunar_file_get_symlink_target (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
 ...
 **/
const gchar *
lunar_file_get_basename (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
 ...
 **/
gboolean
lunar_file_is_symlink (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
guint64
lunar_file_get_size (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), 0);
...
/**
 * lunar_file_get_default_handler:
 * @file : a #LunarFile instance.
 *
 ...
 **/
GAppInfo *
lunar_file_get_default_handler (const LunarFile *file)
{
  ...
  content_type = lunar_file_get_content_type (LUNAR_FILE (file));
 ...
 **/
GFileType
lunar_file_get_kind (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), G_FILE_TYPE_UNKNOWN);
  return file->kind;
}
...
GFile *
lunar_file_get_target_location (const LunarFile *file)
{
  const gchar *uri;
 ...
 **/
LunarFileMode
lunar_file_get_mode (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), 0);
  ...
  if (g_file_info_has_attribute (file->info, G_FILE_ATTRIBUTE_UNIX_MODE))
    ...
    return lunar_file_is_directory (file) ? 0777 : 0666;
}



gboolean
lunar_file_is_mounted (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  return FLAG_IS_SET (file, LUNAR_FILE_FLAG_IS_MOUNTED);
}



gboolean
lunar_file_exists (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_directory (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_shortcut (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_mountable (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_local (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
...
/**
 * lunar_file_is_parent:
 * @file  : a #LunarFile instance.
 * @child : another #LunarFile instance.
 *
 ...
 **/
gboolean
lunar_file_is_parent (const LunarFile *file,
                       const LunarFile *child)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (child), FALSE);
 ...
 **/
gboolean
lunar_file_is_gfile_ancestor (const LunarFile *file,
                               GFile            *ancestor)
{
  GFile   *current = NULL;
  GFile   *tmp;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (G_IS_FILE (file->gfile), FALSE);
  _lunar_return_val_if_fail (G_IS_FILE (ancestor), FALSE);
 ...
 * lunar_file_is_ancestor:
 * @file     : a #LunarFile instance.
 * @ancestor : another #LunarFile instance.
 *
 * Determines whether @file is somewhere inside @ancestor,
 * possibly with intermediate folders.
 *
 ...
 **/
gboolean
lunar_file_is_ancestor (const LunarFile *file,
                         const LunarFile *ancestor)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (ancestor), FALSE);

  return lunar_file_is_gfile_ancestor (file, ancestor->gfile);
}



/**
 * lunar_file_is_executable:
 * @file : a #LunarFile instance.
 *
 ...
 **/
gboolean
lunar_file_is_executable (const LunarFile *file)
{
          ...
          if (can_execute)
            {
              ...
              preferences = lunar_preferences_get ();
  ...
  return can_execute || lunar_file_is_desktop_file (file, NULL);
}



/**
 * lunar_file_is_readable:
 * @file : a #LunarFile instance.
 *
 * Determines whether the owner of the current process is allowed
 * to read the @file.
 ...
 **/
static gboolean
lunar_file_is_readable (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_writable (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_hidden (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_home (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  return lunar_g_file_is_home (file->gfile);
}



/**
 * lunar_file_is_regular:
 * @file : a #LunarFile.
 *
 ...
 **/
gboolean
lunar_file_is_regular (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gboolean
lunar_file_is_trashed (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  return lunar_g_file_is_trashed (file->gfile);
}



/**
 * lunar_file_is_desktop_file:
 * @file      : a #LunarFile.
 ...
 **/
gboolean
lunar_file_is_desktop_file (const LunarFile *file,
                             gboolean         *is_secure)
{
  ...
  if (g_file_info_get_attribute_boolean (file->info, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE))
  ...
  else
    {
      ...
      *is_secure = FALSE;
      ...
      if (g_file_is_native (lunar_file_get_file (file)))
        {
          data_dirs = g_get_system_data_dirs ();
          if (G_LIKELY (data_dirs != NULL))
            {
              path = g_file_get_path (lunar_file_get_file (file));
 ...
 **/
const gchar *
lunar_file_get_display_name (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
gchar*
lunar_file_get_deletion_date (const LunarFile *file,
                               LunarDateStyle   date_style,
                               const gchar      *date_custom_style)
{
  const gchar *date;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  _lunar_return_val_if_fail (G_IS_FILE_INFO (file->info), NULL);
  ...
  deletion_time = lunar_util_time_from_rfc3339 (date);

  /* humanize the time value */
  return lunar_util_humanize_file_time (deletion_time, date_style, date_custom_style);
}



/**
 * lunar_file_get_original_path:
 * @file : a #LunarFile instance.
 *
 ...
 **/
const gchar *
lunar_file_get_original_path (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
 ...
 **/
guint32
lunar_file_get_item_count (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), 0);
 ...
 **/
gboolean
lunar_file_is_chmodable (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  ...
  if (file->info == NULL)
    {
      return (effective_user_id == 0 && !lunar_file_is_trashed (file));
    }
  else
    {
...
/**
 * lunar_file_is_renameable:
 * @file : a #LunarFile instance.
 *
 * Determines whether @file can be renamed using
 * #lunar_file_rename(). Note that the return
 * value is just a guess and #lunar_file_rename()
 * may fail even if this method returns %TRUE.
 *
 * Return value: %TRUE if @file can be renamed.
 **/
gboolean
lunar_file_is_renameable (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
...
gboolean
lunar_file_can_be_trashed (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
 ...
 **/
GList*
lunar_file_get_emblem_names (LunarFile *file)
{
  guint32   uid;
  gchar   **emblem_names;
  GList    *emblems = NULL;
  ...
  if (lunar_file_is_symlink (file))
    emblems = g_list_prepend (emblems, LUNAR_FILE_EMBLEM_NAME_SYMBOLIC_LINK);
   ...
   * x-bit, see https://bugzilla.expidus.org/show_bug.cgi?id=1408 for the details about this change.
   */
  if (!lunar_file_is_readable (file)
      || (lunar_file_is_directory (file)
          && lunar_file_denies_access_permission (file, LUNAR_FILE_MODE_USR_EXEC,
                                                         LUNAR_FILE_MODE_GRP_EXEC,
                                                         LUNAR_FILE_MODE_OTH_EXEC)))
    {
      emblems = g_list_prepend (emblems, LUNAR_FILE_EMBLEM_NAME_CANT_READ);
    }
  else if (G_UNLIKELY (uid == effective_user_id && !lunar_file_is_writable (file) && !lunar_file_is_trashed (file)))
    {
       ...
       */
      emblems = g_list_prepend (emblems, LUNAR_FILE_EMBLEM_NAME_CANT_WRITE);
    }
 ...
 **/
void
lunar_file_set_emblem_names (LunarFile *file,
                              GList      *emblem_names)
{
  GList      *lp;
  gchar     **emblems = NULL;
  ...
  _lunar_return_if_fail (LUNAR_IS_FILE (file));
  _lunar_return_if_fail (G_IS_FILE_INFO (file->info));
  ...
  for (lp = emblem_names, n = 0; lp != NULL; lp = lp->next)
    {
      ...
      if (strcmp (lp->data, LUNAR_FILE_EMBLEM_NAME_SYMBOLIC_LINK) == 0
          || strcmp (lp->data, LUNAR_FILE_EMBLEM_NAME_CANT_READ) == 0
          || strcmp (lp->data, LUNAR_FILE_EMBLEM_NAME_CANT_WRITE) == 0
          || strcmp (lp->data, LUNAR_FILE_EMBLEM_NAME_DESKTOP) == 0)
        continue;
  ...
  g_file_set_attributes_async (file->gfile, info,
                               ...
                               G_PRIORITY_DEFAULT,
                               NULL,
                               lunar_file_set_emblem_names_ready,
                               file);
  g_object_unref (G_OBJECT (info));
 ...
 **/
gboolean
lunar_file_set_custom_icon (LunarFile  *file,
                             ...
                             GError     **error)
{
  ...
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (custom_icon != NULL, FALSE);

  key_file = lunar_g_file_query_key_file (file->gfile, NULL, error);
  ...
  if (lunar_g_file_write_key_file (file->gfile, key_file, NULL, error))
    {
 ...
 **/
gboolean
lunar_file_is_desktop (const LunarFile *file)
{
  GFile   *desktop;
...
const gchar *
lunar_file_get_thumbnail_path (LunarFile *file, LunarThumbnailSize thumbnail_size)
{
  GChecksum *checksum;
  gchar     *filename;
  gchar     *uri;
  ...
  if (lunar_file_get_thumb_state (file) == LUNAR_FILE_THUMB_STATE_NONE)
    return NULL;
      ...
      if (G_LIKELY (checksum != NULL))
        {
          ...
          file->thumbnail_path = g_build_path ("/", g_get_user_cache_dir(),
                                               "thumbnails", lunar_thumbnail_size_get_nick (thumbnail_size),
                                               filename, NULL);
          ...
          if (!g_file_test(file->thumbnail_path, G_FILE_TEST_EXISTS))
            {
              ...
              file->thumbnail_path = g_build_filename (expidus_get_homedir (),
                                                       ".thumbnails", lunar_thumbnail_size_get_nick (thumbnail_size),
                                                       filename, NULL);
...
/**
 * lunar_file_get_thumb_state:
 * @file : a #LunarFile.
 *
 * Returns the current #LunarFileThumbState for @file. This
 * method is intended to be used by #LunarIconFactory only.
 *
 * Return value: the #LunarFileThumbState for @file.
 **/
LunarFileThumbState
lunar_file_get_thumb_state (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), LUNAR_FILE_THUMB_STATE_UNKNOWN);
...
/**
 * lunar_file_set_thumb_state:
 * @file        : a #LunarFile.
 * @thumb_state : the new #LunarFileThumbState.
 *
 * Sets the #LunarFileThumbState for @file to @thumb_state.
 * This will cause a "file-changed" signal to be emitted from
 * #LunarFileMonitor.
 **/
void
lunar_file_set_thumb_state (LunarFile          *file,
                             LunarFileThumbState state)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));

  /* check if the state changes */
  if (lunar_file_get_thumb_state (file) == state)
    return;
  ...
  if (state == LUNAR_FILE_THUMB_STATE_NONE
      && file->thumbnail_path != NULL)
    {
      g_free (file->thumbnail_path);
      file->thumbnail_path = NULL;
  ...
  if (state == LUNAR_FILE_THUMB_STATE_READY)
    lunar_file_monitor_file_changed (file);
}
 ...
 **/
const gchar *
lunar_file_get_custom_icon (const LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
 ...
 **/
GIcon *
lunar_file_get_preview_icon (const LunarFile *file)
{
  GObject *icon;
...
lunar_file_get_preview_type (const LunarFile *file)
{
  GFilesystemPreviewType  preview;
  GFileInfo              *info;

  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), G_FILESYSTEM_PREVIEW_TYPE_NEVER);
  _lunar_return_val_if_fail (G_IS_FILE (file->gfile), G_FILESYSTEM_PREVIEW_TYPE_NEVER);
...
static const gchar *
lunar_file_get_icon_name_for_state (const gchar         *icon_name,
                                     LunarFileIconState  icon_state)
{
  if (endo_str_is_empty (icon_name))
    return NULL;

  /* check if we have an accept icon for the icon we found */
  if (icon_state != LUNAR_FILE_ICON_STATE_DEFAULT
      && (strcmp (icon_name, "inode-directory") == 0
          || strcmp (icon_name, "folder") == 0))
    {
      if (icon_state == LUNAR_FILE_ICON_STATE_DROP)
        return "folder-drag-accept";
      else if (icon_state == LUNAR_FILE_ICON_STATE_OPEN)
        return "folder-open";
    }
 ...
 **/
const gchar *
lunar_file_get_icon_name (LunarFile          *file,
                           LunarFileIconState  icon_state,
                           GtkIconTheme        *icon_theme)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  _lunar_return_val_if_fail (GTK_IS_ICON_THEME (icon_theme), NULL);

  /* return cached name */
  if (G_LIKELY (file->icon_name != NULL))
    return lunar_file_get_icon_name_for_state (file->icon_name, icon_state);

  /* the system root folder has a special icon */
  if (lunar_file_is_directory (file))
    {
      if (G_LIKELY (lunar_file_is_local (file)))
        {
          path = g_file_get_path (file->gfile);
          if (G_LIKELY (path != NULL))
            {
              ...
              else if (strcmp (path, expidus_get_homedir ()) == 0)
                *special_names = "user-home";
              else
                {
                  for (i = 0; i < G_N_ELEMENTS (lunar_file_dirs); i++)
                    {
                      special_dir = g_get_user_special_dir (lunar_file_dirs[i].type);
                      if (special_dir != NULL
                          && strcmp (path, special_dir) == 0)
                        {
                          *special_names = lunar_file_dirs[i].icon_name;
                          break;
                        }
                    }
                }
      ...
      else if (!lunar_file_has_parent (file))
        {
          if (g_file_has_uri_scheme (file->gfile, "trash"))
            {
              special_names[0] = lunar_file_get_item_count (file) > 0 ? "user-trash-full" : "user-trash";
  ...
  else if (lunar_file_is_mountable (file)
  ...
  icon = g_content_type_get_icon (lunar_file_get_content_type (file));
  if (G_LIKELY (icon != NULL))
    {
      check_icon:
 ...
 * @file : a #LunarFile instance.
 *
 * Tells @file to watch itself for changes. Not all #LunarFile
 * implementations must support this, but if a #LunarFile
 * implementation implements the lunar_file_watch() method,
 * it must also implement the lunar_file_unwatch() method.
 *
 * The #LunarFile base class implements automatic "ref
 * counting" for watches, that says, you can call lunar_file_watch()
 * multiple times, but the virtual method will be invoked only
 * once. This also means that you MUST call lunar_file_unwatch()
 * for every lunar_file_watch() invokation, else the application
 * will abort.
 **/
void
lunar_file_watch (LunarFile *file)
{
  LunarFileWatch *file_watch;
  GError          *error = NULL;
  ...
  if (file_watch == NULL)
    {
      file_watch = g_slice_new (LunarFileWatch);
      ...
      /* attach to file */
      g_object_set_qdata_full (G_OBJECT (file), lunar_file_watch_quark, file_watch, lunar_file_watch_destroyed);
    }
  else if (G_LIKELY (!file->no_file_watch))
    {
      /* increase watch count */
      _lunar_return_if_fail (G_IS_FILE_MONITOR (file_watch->monitor));
 ...
 * @file : a #LunarFile instance.
 *
 * See lunar_file_watch() for a description of how watching
 * #LunarFile<!---->s works.
 **/
void
lunar_file_unwatch (LunarFile *file)
{
  LunarFileWatch *file_watch;
  ...
  if (file_watch != NULL)
    {
      ...
      if (--file_watch->watch_count == 0)
        g_object_set_qdata (G_OBJECT (file), lunar_file_watch_quark, NULL);
    }
  else
    {
      _lunar_assert_not_reached ();
 ...
 **/
gboolean
lunar_file_reload (LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);

  /* clear file pxmap cache */
  lunar_icon_factory_clear_pixmap_cache (file);

  if (!lunar_file_load (file, NULL, NULL))
    {
      /* destroy the file if we cannot query any file information */
      lunar_file_destroy (file);
      return FALSE;
    }
...
static gboolean
lunar_file_reload_cb_once (gpointer user_data)
{
  LunarFile *file = user_data;
...
/**
 * lunar_file_reload_idle:
 * @file : a #LunarFile instance.
 *
 * Schedules a single reload of the @file by calling lunar_file_reload
 * when idle.
 *
 **/
void
lunar_file_reload_idle (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));

  g_idle_add (lunar_file_reload_cb_once, file);
}



/**
 * lunar_file_reload_idle_unref:
 * @file : a #LunarFile instance.
 *
 * Schedules a reload of the @file by calling lunar_file_reload
 * when idle. When scheduled function returns @file object will be
 * unreferenced.
 *
 **/
void
lunar_file_reload_idle_unref (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));

  g_idle_add_full (G_PRIORITY_DEFAULT_IDLE,
                   lunar_file_reload_cb_once,
                   file,
 ...
 **/
void
lunar_file_destroy (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));
  ...
  if (!FLAG_IS_SET (file, LUNAR_FILE_FLAG_IN_DESTRUCTION))
    {
      ...
      lunar_file_monitor_file_destroyed (file);
...
/**
 * lunar_file_compare_by_type:
 * @a : the first #LunarFile.
 * @b : the second #LunarFile.
 *
 ...
 **/
gint
lunar_file_compare_by_type (LunarFile *a,
                             LunarFile *b)
{
  GFile *file_a;
  GFile *file_b;
  ...
  file_a = lunar_file_get_file (a);
  file_b = lunar_file_get_file (b);
...
/**
 * lunar_file_compare_by_name:
 * @file_a         : the first #LunarFile.
 * @file_b         : the second #LunarFile.
 ...
 **/
gint
lunar_file_compare_by_name (const LunarFile *file_a,
                             const LunarFile *file_b,
                             gboolean          case_sensitive)
{
   ...
   */
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_a), 0);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_b), 0);
#endif
  ...
  if (result == 0)
    {
      result = g_strcmp0 (lunar_file_get_original_path (file_a),
                          lunar_file_get_original_path (file_b));
    }
...
static gboolean
lunar_file_same_filesystem (const LunarFile *file_a,
                             const LunarFile *file_b)
{
  const gchar *filesystem_id_a;
  const gchar *filesystem_id_b;

  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_a), FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file_b), FALSE);
  ...
  return endo_str_is_equal (filesystem_id_a, filesystem_id_b);
}



/**
 * lunar_file_cache_lookup:
 * @file : a #GFile.
 *
 * Looks up the #LunarFile for @file in the internal file
 * cache and returns the file present for @file in the
 * cache or %NULL if no #LunarFile is cached for @file.
 *
 * Note that no reference is taken for the caller.
 *
 * This method should not be used but in very rare cases.
 * Consider using lunar_file_get() instead.
 *
 * Return value: the #LunarFile for @file in the internal
 *               cache, or %NULL. If you are done with the
 *               file, use g_object_unref to release.
 **/
LunarFile *
lunar_file_cache_lookup (const GFile *file)
{
  GWeakRef   *ref;
  LunarFile *cached_file;

  _lunar_return_val_if_fail (G_IS_FILE (file), NULL);

  G_LOCK (file_cache_mutex);

  /* allocate the LunarFile cache on-demand */
...
gchar *
lunar_file_cached_display_name (const GFile *file)
{
  LunarFile *cached_file;
  gchar      *display_name;

  /* check if we have a LunarFile for it in the cache (usually is the case) */
  cached_file = lunar_file_cache_lookup (file);
  if (cached_file != NULL)
    {
      /* determine the display name of the file */
      display_name = g_strdup (lunar_file_get_display_name (cached_file));
      g_object_unref (cached_file);
    }
  else
    {
      /* determine something a hopefully good approximation of the display name */
      display_name = lunar_g_file_get_display_name (G_FILE (file));
    }
...
/**
 * lunar_file_list_get_applications:
 * @file_list : a #GList of #LunarFile<!---->s.
 *
 * Returns the #GList of #GAppInfo<!---->s that can be used to open
 * all #LunarFile<!---->s in the given @file_list.
 *
 ...
 **/
GList*
lunar_file_list_get_applications (GList *file_list)
{
  ...
  for (lp = file_list; lp != NULL; lp = lp->next)
    {
      current_type = lunar_file_get_content_type (lp->data);
  ...
  for (ap = applications; ap != NULL; ap = next)
    {
      ...
      if (!lunar_g_app_info_should_show (ap->data))
        {
          /* drop our reference on the application */
          g_object_unref (G_OBJECT (ap->data));
...
/**
 * lunar_file_list_to_lunar_g_file_list:
 * @file_list : a #GList of #LunarFile<!---->s.
 *
 * Transforms the @file_list to a #GList of #GFile<!---->s for
 * the #LunarFile<!---->s contained within @file_list.
 *
 * The caller is responsible to free the returned list using
 * lunar_g_file_list_free() when no longer needed.
 *
 * Return value: the list of #GFile<!---->s for @file_list.
 **/
GList*
lunar_file_list_to_lunar_g_file_list (GList *file_list)
{
  GList *list = NULL;
  GList *lp;

  for (lp = g_list_last (file_list); lp != NULL; lp = lp->prev)
    list = g_list_prepend (list, g_object_ref (LUNAR_FILE (lp->data)->gfile));
...
/**
 * lunar_file_get_metadata_setting:
 * @file         : a #LunarFile instance.
 ...
 **/
const gchar*
lunar_file_get_metadata_setting (LunarFile  *file,
                                  const gchar *setting_name)
{
  ...
  attr_name = g_strdup_printf ("metadata::lunar-%s", setting_name);
...
static void
lunar_file_set_metadata_setting_finish (GObject      *source_object,
                                         GAsyncResult *result,
                                         gpointer      user_data)
{
  LunarFile *file = LUNAR_FILE (user_data);
...
/**
 * lunar_file_set_metadata_setting:
 * @file          : a #LunarFile instance.
 ...
 **/
void
lunar_file_set_metadata_setting (LunarFile  *file,
                                  const gchar *setting_name,
                                  const gchar *setting_value)
{
  ...
  g_file_set_attributes_async (file->gfile, info,
                               ...
                               G_PRIORITY_DEFAULT,
                               NULL,
                               lunar_file_set_metadata_setting_finish,
                               file);
  g_free (attr_name);
  g_object_unref (G_OBJECT (info));
}
...
/**
 * lunar_file_clear_directory_specific_settings:
 * @file : a #LunarFile instance.
 *
 * Clears all directory specific settings stored in the metadata of the folder represented by @file
 **/
void
lunar_file_clear_directory_specific_settings (LunarFile *file)
{
  _lunar_return_if_fail (LUNAR_IS_FILE (file));
  ...
  g_file_info_remove_attribute (file->info, "metadata::lunar-view-type");
  g_file_info_remove_attribute (file->info, "metadata::lunar-sort-column");
  g_file_info_remove_attribute (file->info, "metadata::lunar-sort-order");

  g_file_set_attribute (file->gfile, "metadata::lunar-view-type", G_FILE_ATTRIBUTE_TYPE_INVALID,
                        NULL, G_FILE_QUERY_INFO_NONE, NULL, NULL);
  g_file_set_attribute (file->gfile, "metadata::lunar-sort-column", G_FILE_ATTRIBUTE_TYPE_INVALID,
                        NULL, G_FILE_QUERY_INFO_NONE, NULL, NULL);
  g_file_set_attribute (file->gfile, "metadata::lunar-sort-order", G_FILE_ATTRIBUTE_TYPE_INVALID,
                        NULL, G_FILE_QUERY_INFO_NONE, NULL, NULL);
...
/**
 * lunar_file_has_directory_specific_settings:
 * @file : a #LunarFile instance.
 *
 ...
 **/
gboolean
lunar_file_has_directory_specific_settings (LunarFile *file)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  ...
  if (g_file_info_has_attribute (file->info, "metadata::lunar-view-type"))
    return TRUE;
  if (g_file_info_has_attribute (file->info, "metadata::lunar-sort-column"))
    return TRUE;
  if (g_file_info_has_attribute (file->info, "metadata::lunar-sort-order"))
    return TRUE;
