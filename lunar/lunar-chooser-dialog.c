...
#include <lunar/lunar-chooser-model.h>
...
static void        lunar_chooser_dialog_dispose             (GObject                  *object);
static void        lunar_chooser_dialog_get_property        (GObject                  *object,
                                                              guint                     prop_id,
                                                              GValue                   *value,
                                                              GParamSpec               *pspec);
static void        lunar_chooser_dialog_set_property        (GObject                  *object,
                                                              guint                     prop_id,
                                                              const GValue             *value,
                                                              GParamSpec               *pspec);
static void        lunar_chooser_dialog_realize             (GtkWidget                *widget);
static void        lunar_chooser_dialog_response            (GtkDialog                *widget,
                                                              gint                      response);
static gboolean    lunar_chooser_dialog_selection_func      (GtkTreeSelection         *selection,
                                                              GtkTreeModel             *model,
                                                              GtkTreePath              *path,
                                                              gboolean                  path_currently_selected,
                                                              gpointer                  user_data);
static gboolean    lunar_chooser_dialog_context_menu        (LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_update_accept       (LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_update_header       (LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_action_remove       (LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_action_forget       (LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_browse_clicked      (GtkWidget                *button,
                                                              LunarChooserDialog      *dialog);
static gboolean    lunar_chooser_dialog_button_press_event  (GtkWidget                *tree_view,
                                                              GdkEventButton           *event,
                                                              LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_notify_expanded     (GtkExpander              *expander,
                                                              GParamSpec               *pspec,
                                                              LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_expand              (LunarChooserDialog      *dialog);
static gboolean    lunar_chooser_dialog_popup_menu          (GtkWidget                *tree_view,
                                                              LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_row_activated       (GtkTreeView              *treeview,
                                                              GtkTreePath              *path,
                                                              GtkTreeViewColumn        *column,
                                                              LunarChooserDialog      *dialog);
static void        lunar_chooser_dialog_selection_changed   (GtkTreeSelection         *selection,
                                                              LunarChooserDialog      *dialog);
static LunarFile *lunar_chooser_dialog_get_file            (LunarChooserDialog *dialog);
static void        lunar_chooser_dialog_set_file            (LunarChooserDialog *dialog,
                                                              LunarFile          *file);
static gboolean    lunar_chooser_dialog_get_open            (LunarChooserDialog *dialog);
static void        lunar_chooser_dialog_set_open            (LunarChooserDialog *dialog,
                                                              gboolean             open);


struct _LunarChooserDialogClass
{
  LunarAbstractDialogClass __parent__;
};

struct _LunarChooserDialog
{
...
G_DEFINE_TYPE (LunarChooserDialog, lunar_chooser_dialog, LUNAR_TYPE_ABSTRACT_DIALOG)



static void
lunar_chooser_dialog_class_init (LunarChooserDialogClass *klass)
{
  ...
  gobject_class->dispose = lunar_chooser_dialog_dispose;
  gobject_class->get_property = lunar_chooser_dialog_get_property;
  gobject_class->set_property = lunar_chooser_dialog_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->realize = lunar_chooser_dialog_realize;

  gtkdialog_class = GTK_DIALOG_CLASS (klass);
  gtkdialog_class->response = lunar_chooser_dialog_response;

  /**
   * LunarChooserDialog::file:
   *
   * The #LunarFile for which an application should be chosen.
   **/
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_boolean ("open", "open", "open",
                                                         FALSE,
                                                         G_PARAM_CONSTRUCT | ENDO_PARAM_READWRITE));
}



static void
lunar_chooser_dialog_init (LunarChooserDialog *dialog)
{
  GtkTreeViewColumn *column;
  ...
  g_signal_connect (G_OBJECT (dialog->tree_view), "button-press-event", G_CALLBACK (lunar_chooser_dialog_button_press_event), dialog);
  g_signal_connect (G_OBJECT (dialog->tree_view), "popup-menu", G_CALLBACK (lunar_chooser_dialog_popup_menu), dialog);
  g_signal_connect (G_OBJECT (dialog->tree_view), "row-activated", G_CALLBACK (lunar_chooser_dialog_row_activated), dialog);
  ...
  renderer = g_object_new (ENDO_TYPE_CELL_RENDERER_ICON, "follow-state", FALSE, "size", 24, NULL);
  gtk_tree_view_column_pack_start (column, renderer, FALSE);
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "gicon", LUNAR_CHOOSER_MODEL_COLUMN_ICON,
                                       NULL);
  renderer = gtk_cell_renderer_text_new ();
  gtk_tree_view_column_pack_start (column, renderer, TRUE);
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "style", LUNAR_CHOOSER_MODEL_COLUMN_STYLE,
                                       "text", LUNAR_CHOOSER_MODEL_COLUMN_NAME,
                                       "weight", LUNAR_CHOOSER_MODEL_COLUMN_WEIGHT,
                                       NULL);
  ...
  endo_binding_new_with_negation (G_OBJECT (dialog->custom_expander), "expanded", G_OBJECT (dialog->tree_view), "sensitive");
  g_signal_connect (G_OBJECT (dialog->custom_expander), "notify::expanded", G_CALLBACK (lunar_chooser_dialog_notify_expanded), dialog);
  ...
  g_signal_connect_swapped (G_OBJECT (dialog->custom_entry), "changed", G_CALLBACK (lunar_chooser_dialog_update_accept), dialog);
  ...
  g_signal_connect (G_OBJECT (dialog->custom_button), "clicked", G_CALLBACK (lunar_chooser_dialog_browse_clicked), dialog);
  ...
  endo_binding_new (G_OBJECT (dialog), "open", G_OBJECT (dialog->default_button), "visible");
  ...
  gtk_tree_selection_set_select_function (selection, lunar_chooser_dialog_selection_func, dialog, NULL);
  g_signal_connect (G_OBJECT (selection), "changed", G_CALLBACK (lunar_chooser_dialog_selection_changed), dialog);
}



static void
lunar_chooser_dialog_dispose (GObject *object)
{
  LunarChooserDialog *dialog = LUNAR_CHOOSER_DIALOG (object);

  /* drop the reference on the file */
  lunar_chooser_dialog_set_file (dialog, NULL);

  (*G_OBJECT_CLASS (lunar_chooser_dialog_parent_class)->dispose) (object);
}



static void
lunar_chooser_dialog_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FILE:
      g_value_set_object (value, lunar_chooser_dialog_get_file (dialog));
      break;

    case PROP_OPEN:
      g_value_set_boolean (value, lunar_chooser_dialog_get_open (dialog));
      break;
...
static void
lunar_chooser_dialog_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FILE:
      lunar_chooser_dialog_set_file (dialog, g_value_get_object (value));
      break;

    case PROP_OPEN:
      lunar_chooser_dialog_set_open (dialog, g_value_get_boolean (value));
      break;
...
static void
lunar_chooser_dialog_realize (GtkWidget *widget)
{
  LunarChooserDialog *dialog = LUNAR_CHOOSER_DIALOG (widget);

  /* let the GtkWindow class realize the dialog */
  (*GTK_WIDGET_CLASS (lunar_chooser_dialog_parent_class)->realize) (widget);

  /* update the dialog header */
  lunar_chooser_dialog_update_header (dialog);
}



static void
lunar_chooser_dialog_response (GtkDialog *widget,
                                gint       response)
{
  ...
  content_type = lunar_file_get_content_type (dialog->file);
  ...
  if (!gtk_expander_get_expanded (GTK_EXPANDER (dialog->custom_expander)))
    {
      ...
      if (gtk_tree_selection_get_selected (selection, &model, &iter))
        gtk_tree_model_get (model, &iter, LUNAR_CHOOSER_MODEL_COLUMN_APPLICATION, &app_info, -1);
    }
  else
    {
      ...
      if (G_UNLIKELY (app_info == NULL))
        {
          ...
          lunar_dialogs_show_error (GTK_WIDGET (dialog), error, _("Failed to add new application \"%s\""), custom_command);
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (dialog->default_button)))
    {
      ...
      if (G_UNLIKELY (!succeed))
        {
          /* display an error to the user */
          lunar_dialogs_show_error (GTK_WIDGET (dialog),
                                     error,
                                     _("Failed to set default application for \"%s\""),
                                     lunar_file_get_display_name (dialog->file));
      ...
      if (G_LIKELY (succeed))
        lunar_file_changed (dialog->file);
    }
  else
    {
  ...
  if (G_LIKELY (succeed && dialog->open))
    {
      ...
      list.data = lunar_file_get_file (dialog->file); list.next = list.prev = NULL;
...
static gboolean
lunar_chooser_dialog_selection_func (GtkTreeSelection *selection,
                                      GtkTreeModel     *model,
                                      GtkTreePath      *path,
                                      gboolean          path_currently_selected,
                                      gpointer          user_data)
  ...
  if (G_UNLIKELY (!path_currently_selected))
    {
      ...
      gtk_tree_model_get_value (model, &iter, LUNAR_CHOOSER_MODEL_COLUMN_APPLICATION, &value);
...
static gboolean
lunar_chooser_dialog_context_menu (LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog), FALSE);
  ...
  g_signal_connect_swapped (G_OBJECT (item), "activate", G_CALLBACK (lunar_chooser_dialog_action_remove), dialog);
  ...
  g_signal_connect_swapped (G_OBJECT (item), "activate", G_CALLBACK (lunar_chooser_dialog_action_forget), dialog);
  gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
  gtk_widget_show (item);
...
static void
lunar_chooser_dialog_update_accept (LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
  ...
  _lunar_return_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog));
...
static void
lunar_chooser_dialog_update_header (LunarChooserDialog *dialog)
{
  const gchar *content_type;
  GIcon       *icon;
  gchar       *description;
  ...
  _lunar_return_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog));
  _lunar_return_if_fail (gtk_widget_get_realized (GTK_WIDGET (dialog)));
  ...
  if (G_UNLIKELY (dialog->file == NULL))
  ...
  else
    {
      ...
      lunar_gtk_widget_set_tooltip (dialog->custom_button,
      ...
      lunar_gtk_widget_set_tooltip (dialog->default_button,
...
static void
lunar_chooser_dialog_action_remove (LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
      ...
      if (G_LIKELY (response == GTK_RESPONSE_YES))
        {
          ...
          if (!lunar_chooser_model_remove (LUNAR_CHOOSER_MODEL (model), &iter, TRUE, &error))
            {
              /* display an error to the user */
              lunar_dialogs_show_error (dialog, error, _("Failed to remove \"%s\""), name);
              g_error_free (error);
            }
...
static void
lunar_chooser_dialog_action_forget (LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
  ...
  if (g_app_info_can_remove_supports_type (app_info))
    {
      ...
      if (G_LIKELY (response == GTK_RESPONSE_YES))
        {
          /* try to delete the application from the model */
          if (!lunar_chooser_model_remove (LUNAR_CHOOSER_MODEL (model), &iter, FALSE, &error))
            {
              /* display an error to the user */
              lunar_dialogs_show_error (dialog, error, _("Failed to forget \"%s\""), name);
              g_error_free (error);
            }
          else if (G_LIKELY (dialog->file != NULL))
            {
...
static void
lunar_chooser_dialog_browse_clicked (GtkWidget           *button,
                                      LunarChooserDialog *dialog)
{
  ...
  if (gtk_dialog_run (GTK_DIALOG (chooser)) == GTK_RESPONSE_ACCEPT)
    {
      ...
      filename_escaped = lunar_g_strescape (filename);
...
static gboolean
lunar_chooser_dialog_button_press_event (GtkWidget           *tree_view,
                                          GdkEventButton      *event,
                                          LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog), FALSE);
  _lunar_return_val_if_fail (dialog->tree_view == tree_view, FALSE);
      ...
      if (gtk_tree_view_get_path_at_pos (GTK_TREE_VIEW (tree_view), event->x, event->y, &path, NULL, NULL, NULL))
        {
          ...
          return lunar_chooser_dialog_context_menu (dialog);
        }
    }
...
static void
lunar_chooser_dialog_notify_expanded (GtkExpander         *expander,
                                       GParamSpec          *pspec,
                                       LunarChooserDialog *dialog)
{
  GtkTreeSelection *selection;

  _lunar_return_if_fail (GTK_IS_EXPANDER (expander));
  _lunar_return_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog));
  ...
  lunar_chooser_dialog_update_accept (dialog);
}



static void
lunar_chooser_dialog_expand (LunarChooserDialog *dialog)
{
  GtkTreeModel *model;
  GtkTreePath  *path;
  GtkTreeIter   iter;
...
static gboolean
lunar_chooser_dialog_popup_menu (GtkWidget           *tree_view,
                                  LunarChooserDialog *dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog), FALSE);
  _lunar_return_val_if_fail (dialog->tree_view == tree_view, FALSE);
...
static void
lunar_chooser_dialog_row_activated (GtkTreeView         *treeview,
                                     GtkTreePath         *path,
                                     GtkTreeViewColumn   *column,
                                     LunarChooserDialog *dialog)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog));
  _lunar_return_if_fail (GTK_IS_TREE_VIEW (treeview));
...
static void
lunar_chooser_dialog_selection_changed (GtkTreeSelection    *selection,
                                         LunarChooserDialog *dialog)
{
  GAppInfo     *app_info;
  GtkTreeModel *model;
...
/**
 * lunar_chooser_dialog_get_file:
 * @dialog : a #LunarChooserDialog.
 *
 * Returns the #LunarFile currently associated with @dialog or
 * %NULL if there's no such association.
 *
 * Return value: the #LunarFile associated with @dialog.
 **/
static LunarFile*
lunar_chooser_dialog_get_file (LunarChooserDialog *dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog), NULL);
  return dialog->file;
}
 ...
 **/
static void
lunar_chooser_dialog_set_file (LunarChooserDialog *dialog,
                                LunarFile          *file)
{
  LunarChooserModel *model;
  ...
  if (G_LIKELY (dialog->file != NULL))
    {
      ...
      lunar_file_unwatch (LUNAR_FILE (dialog->file));
  ...
  if (G_LIKELY (file != NULL))
    {
      ...
      model = lunar_chooser_model_new (lunar_file_get_content_type (file));
      gtk_tree_view_set_model (GTK_TREE_VIEW (dialog->tree_view), GTK_TREE_MODEL (model));
      lunar_chooser_dialog_expand (dialog);
 ...
 **/
static gboolean
lunar_chooser_dialog_get_open (LunarChooserDialog *dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog), FALSE);
  return dialog->open;
}
 ...
 **/
static void
lunar_chooser_dialog_set_open (LunarChooserDialog *dialog,
                                gboolean             open)
{
  _lunar_return_if_fail (LUNAR_IS_CHOOSER_DIALOG (dialog));
 ...
 * @file   : the #LunarFile for which an application should be chosen.
 * @open   : whether to also open the @file.
 *
 * Convenience function to display a #LunarChooserDialog with the
 * given parameters.
 *
 ...
 **/
void
lunar_show_chooser_dialog (gpointer    parent,
                            LunarFile *file,
                            gboolean    open)
{
  ...
  dialog = g_object_new (LUNAR_TYPE_CHOOSER_DIALOG,
                         "file", file,
                         "open", open,
                         "screen", screen,
                         NULL);
