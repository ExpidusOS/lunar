...
#include <lunar/lunar-path-entry.h>
...
static void     lunar_path_entry_editable_init                 (GtkEditableInterface *iface);
static void     lunar_path_entry_finalize                      (GObject              *object);
static void     lunar_path_entry_get_property                  (GObject              *object,
                                                                 guint                 prop_id,
                                                                 GValue               *value,
                                                                 GParamSpec           *pspec);
static void     lunar_path_entry_set_property                  (GObject              *object,
                                                                 guint                 prop_id,
                                                                 const GValue         *value,
                                                                 GParamSpec           *pspec);
static gboolean lunar_path_entry_focus                         (GtkWidget            *widget,
                                                                 GtkDirectionType      direction);
static void     lunar_path_entry_icon_press_event              (GtkEntry            *entry,
                                                                 GtkEntryIconPosition icon_pos,
                                                                 GdkEventButton      *event,
                                                                 gpointer             userdata);
static void     lunar_path_entry_icon_release_event            (GtkEntry            *entry,
                                                                 GtkEntryIconPosition icon_pos,
                                                                 GdkEventButton      *event,
                                                                 gpointer             user_data);
static gboolean lunar_path_entry_motion_notify_event           (GtkWidget            *widget,
                                                                 GdkEventMotion       *event);
static gboolean lunar_path_entry_key_press_event               (GtkWidget            *widget,
                                                                 GdkEventKey          *event);
static void     lunar_path_entry_drag_data_get                 (GtkWidget            *widget,
                                                                 GdkDragContext       *context,
                                                                 GtkSelectionData     *selection_data,
                                                                 guint                 info,
                                                                 guint                 timestamp);
static void     lunar_path_entry_activate                      (GtkEntry             *entry);
static void     lunar_path_entry_changed                       (GtkEditable          *editable);
static void     lunar_path_entry_update_icon                   (LunarPathEntry      *path_entry);
static void     lunar_path_entry_do_insert_text                (GtkEditable          *editable,
                                                                 const gchar          *new_text,
                                                                 gint                  new_text_length,
                                                                 gint                 *position);
static void     lunar_path_entry_clear_completion              (LunarPathEntry      *path_entry);
static void     lunar_path_entry_common_prefix_append          (LunarPathEntry      *path_entry,
                                                                 gboolean              highlight);
static void     lunar_path_entry_common_prefix_lookup          (LunarPathEntry      *path_entry,
                                                                 gchar               **prefix_return,
                                                                 LunarFile          **file_return);
static gboolean lunar_path_entry_match_func                    (GtkEntryCompletion   *completion,
                                                                 const gchar          *key,
                                                                 GtkTreeIter          *iter,
                                                                 gpointer              user_data);
static gboolean lunar_path_entry_match_selected                (GtkEntryCompletion   *completion,
                                                                 GtkTreeModel         *model,
                                                                 GtkTreeIter          *iter,
                                                                 gpointer              user_data);
static gboolean lunar_path_entry_parse                         (LunarPathEntry      *path_entry,
                                                                 gchar               **folder_part,
                                                                 gchar               **file_part,
                                                                 GError              **error);
static void     lunar_path_entry_queue_check_completion        (LunarPathEntry      *path_entry);
static gboolean lunar_path_entry_check_completion_idle         (gpointer              user_data);
static void     lunar_path_entry_check_completion_idle_destroy (gpointer              user_data);



struct _LunarPathEntryClass
{
  GtkEntryClass __parent__;
};

struct _LunarPathEntry
{
  GtkEntry __parent__;

  LunarIconFactory *icon_factory;
  LunarFile        *current_folder;
  LunarFile        *current_file;
...
static GtkEditableInterface *lunar_path_entry_editable_parent_iface;



G_DEFINE_TYPE_WITH_CODE (LunarPathEntry, lunar_path_entry, GTK_TYPE_ENTRY,
    G_IMPLEMENT_INTERFACE (GTK_TYPE_EDITABLE, lunar_path_entry_editable_init))



static void
lunar_path_entry_class_init (LunarPathEntryClass *klass)
{
  ...
  gobject_class->finalize = lunar_path_entry_finalize;
  gobject_class->get_property = lunar_path_entry_get_property;
  gobject_class->set_property = lunar_path_entry_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->focus = lunar_path_entry_focus;
  gtkwidget_class->motion_notify_event = lunar_path_entry_motion_notify_event;
  gtkwidget_class->drag_data_get = lunar_path_entry_drag_data_get;

  gtkentry_class = GTK_ENTRY_CLASS (klass);
  gtkentry_class->activate = lunar_path_entry_activate;

  /**
   * LunarPathEntry:current-file:
   *
   * The #LunarFile currently displayed by the path entry or %NULL.
   **/
  ...
  gtk_widget_class_install_style_property (gtkwidget_class,
                                           g_param_spec_int ("icon-size",
                                                             ...
                                                             1, G_MAXINT, 16, ENDO_PARAM_READABLE));
}



static void
lunar_path_entry_editable_init (GtkEditableInterface *iface)
{
  lunar_path_entry_editable_parent_iface = g_type_interface_peek_parent (iface);

  iface->changed = lunar_path_entry_changed;
  iface->do_insert_text = lunar_path_entry_do_insert_text;
}



static void
lunar_path_entry_init (LunarPathEntry *path_entry)
{
  ...
  gtk_entry_completion_set_match_func (completion, lunar_path_entry_match_func, path_entry, NULL);
  g_signal_connect (G_OBJECT (completion), "match-selected", G_CALLBACK (lunar_path_entry_match_selected), path_entry);
  ...
  gtk_cell_layout_add_attribute (GTK_CELL_LAYOUT (completion), renderer, "file", LUNAR_COLUMN_FILE);

  /* add the text renderer to the entry completion */
  renderer = gtk_cell_renderer_text_new ();
  gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (completion), renderer, TRUE);
  gtk_cell_layout_add_attribute (GTK_CELL_LAYOUT (completion), renderer, "text", LUNAR_COLUMN_NAME);

  /* allocate a new list mode for the completion */
  store = lunar_list_model_new ();
  lunar_list_model_set_show_hidden (store, TRUE);
  lunar_list_model_set_folders_first (store, TRUE);
  gtk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (store), LUNAR_COLUMN_FILE_NAME, GTK_SORT_ASCENDING);
   ...
   */
  g_signal_connect (G_OBJECT (path_entry), "key-press-event", G_CALLBACK (lunar_path_entry_key_press_event), NULL);
  ...
  g_signal_connect (G_OBJECT (path_entry), "notify::cursor-position", G_CALLBACK (lunar_path_entry_clear_completion), NULL);
  g_signal_connect (G_OBJECT (path_entry), "notify::selection-bound", G_CALLBACK (lunar_path_entry_clear_completion), NULL);

  /* connect the icon signals */
  g_signal_connect (G_OBJECT (path_entry), "icon-press", G_CALLBACK (lunar_path_entry_icon_press_event), NULL);
  g_signal_connect (G_OBJECT (path_entry), "icon-release", G_CALLBACK (lunar_path_entry_icon_release_event), NULL);
}



static void
lunar_path_entry_finalize (GObject *object)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (object);
  ...
  if (G_LIKELY (path_entry->current_file != NULL))
    {
      g_signal_handlers_disconnect_by_func (G_OBJECT (path_entry->current_file), lunar_path_entry_set_current_file, path_entry);
  ...
  (*G_OBJECT_CLASS (lunar_path_entry_parent_class)->finalize) (object);
}



static void
lunar_path_entry_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CURRENT_FILE:
      g_value_set_object (value, lunar_path_entry_get_current_file (path_entry));
      break;
...
static void
lunar_path_entry_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CURRENT_FILE:
      lunar_path_entry_set_current_file (path_entry, g_value_get_object (value));
      break;
...
static gboolean
lunar_path_entry_focus (GtkWidget       *widget,
                         GtkDirectionType direction)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (widget);
  ...
  if ((direction == GTK_DIR_TAB_FORWARD) && (gtk_widget_has_focus (widget)) && !control_pressed)
    {
      ...
      if (!path_entry->has_completion && gtk_editable_get_position (GTK_EDITABLE (path_entry)) == gtk_entry_get_text_length (GTK_ENTRY (path_entry)))
        lunar_path_entry_common_prefix_append (path_entry, FALSE);
    ...
    return (*GTK_WIDGET_CLASS (lunar_path_entry_parent_class)->focus) (widget, direction);
}



static void
lunar_path_entry_icon_press_event (GtkEntry            *entry,
                                    GtkEntryIconPosition icon_pos,
                                    GdkEventButton      *event,
                                    gpointer             userdata)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (entry);
...
static void
lunar_path_entry_icon_release_event (GtkEntry            *entry,
                                      GtkEntryIconPosition icon_pos,
                                      GdkEventButton      *event,
                                      gpointer             user_data)
{
...
static gboolean
lunar_path_entry_motion_notify_event (GtkWidget      *widget,
                                       GdkEventMotion *event)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (widget);
  GdkDragContext  *context;
  ...
  if (path_entry->drag_button > 0
      ...
      && gtk_drag_check_threshold (widget, path_entry->drag_x, path_entry->drag_y, event->x, event->y))
    {
      ...
      icon = lunar_icon_factory_load_file_icon (path_entry->icon_factory,
  ...
  return (*GTK_WIDGET_CLASS (lunar_path_entry_parent_class)->motion_notify_event) (widget, event);
}



static gboolean
lunar_path_entry_key_press_event (GtkWidget   *widget,
                                   GdkEventKey *event)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (widget);
...
static void
lunar_path_entry_drag_data_get (GtkWidget        *widget,
                                 GdkDragContext   *context,
                                 GtkSelectionData *selection_data,
                                 guint             info,
                                 guint             timestamp)
  ...
  if (G_LIKELY (path_entry->current_file != NULL))
    {
      ...
      file_list.data = lunar_file_get_file (path_entry->current_file);

      /* setup the uri list for the drag selection */
      uris = lunar_g_file_list_to_stringv (&file_list);
...
static void
lunar_path_entry_activate (GtkEntry *entry)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (entry);
  ...
  (*GTK_ENTRY_CLASS (lunar_path_entry_parent_class)->activate) (entry);
}



static void
lunar_path_entry_changed (GtkEditable *editable)
{
  GtkEntryCompletion *completion;
  LunarPathEntry    *path_entry = LUNAR_PATH_ENTRY (editable);
  LunarFolder       *folder;
  GtkTreeModel       *model;
  const gchar        *text;
  gchar              *escaped_text;
  ...
  if (G_UNLIKELY (endo_str_looks_like_an_uri (text)))
    {
  ...
  else if (lunar_path_entry_parse (path_entry, &folder_part, &file_part, NULL))
    {
  ...
  current_folder = (folder_path != NULL) ? lunar_file_get (folder_path, NULL) : NULL;
  current_file = (file_path != NULL) ? lunar_file_get (file_path, NULL) : NULL;
  ...
  if (current_folder != path_entry->current_folder)
    {
      ...
      if (current_folder != NULL && lunar_file_is_directory (current_folder))
        folder = lunar_folder_get_for_file (current_folder);
      else
        folder = NULL;
       ...
       * updating the model being used (https://bugzilla.expidus.org/show_bug.cgi?id=1681).
       */
      model = gtk_entry_completion_get_model (completion);
      g_object_ref (G_OBJECT (model));
      gtk_entry_completion_set_model (completion, NULL);
      lunar_list_model_set_folder (LUNAR_LIST_MODEL (model), folder);
      ...
      if (G_UNLIKELY (current_file != NULL))
        {
          ...
          g_signal_connect_swapped (G_OBJECT (current_file), "changed", G_CALLBACK (lunar_path_entry_set_current_file), path_entry);
        }
  ...
  if (update_icon)
    lunar_path_entry_update_icon (path_entry);
...
static void
lunar_path_entry_update_icon (LunarPathEntry *path_entry)
{
  GdkPixbuf          *icon = NULL;
  ...
  if (path_entry->icon_factory == NULL)
    {
      ...
      path_entry->icon_factory = lunar_icon_factory_get_for_icon_theme (icon_theme);
    }
...
static void
lunar_path_entry_do_insert_text (GtkEditable *editable,
                                  const gchar *new_text,
                                  gint         new_text_length,
                                  gint        *position)
{
  ...
  (*lunar_path_entry_editable_parent_iface->do_insert_text) (editable, new_text, new_text_length, position);

  /* queue a completion check if this insert operation was triggered by the user */
  if (G_LIKELY (!path_entry->in_change))
    lunar_path_entry_queue_check_completion (path_entry);
}



static void
lunar_path_entry_clear_completion (LunarPathEntry *path_entry)
{
  /* reset the completion and apply the new text */
  if (G_UNLIKELY (path_entry->has_completion))
    {
      path_entry->has_completion = FALSE;
      lunar_path_entry_changed (GTK_EDITABLE (path_entry));
    }
}
...
static void
lunar_path_entry_common_prefix_append (LunarPathEntry *path_entry,
                                        gboolean         highlight)
{
  ...
  lunar_path_entry_common_prefix_lookup (path_entry, &prefix, &file);

  /* check if we should append a slash to the prefix */
  if (G_LIKELY (file != NULL))
    {
      /* we only append slashes for directories */
      if (lunar_file_is_directory (file) && file != path_entry->current_file)
        {
...
static gboolean
lunar_path_entry_has_prefix_casefolded (const gchar *string,
                                         const gchar *prefix)
{
...
static void
lunar_path_entry_common_prefix_lookup (LunarPathEntry *path_entry,
                                        gchar          **prefix_return,
                                        LunarFile     **file_return)
{
  GtkTreeModel *model;
      ...
      do
        {
          ...
          gtk_tree_model_get (model, &iter, LUNAR_COLUMN_FILE_NAME, &name, -1);

          /* check if we have a valid prefix here */
          if (lunar_path_entry_has_prefix_casefolded (name, text))
            {
              ...
              if (*prefix_return == NULL)
                {
                  ...
                  *prefix_return = g_strdup (name);
                  ...
                  gtk_tree_model_get (model, &iter, LUNAR_COLUMN_FILE, file_return, -1);
                }
              else
                {
...
static gboolean
lunar_path_entry_match_func (GtkEntryCompletion *completion,
                              const gchar        *key,
                              GtkTreeIter        *iter,
                              gpointer            user_data)
{
  GtkTreeModel    *model;
  LunarPathEntry *path_entry;
  const gchar     *last_slash;
  LunarFile      *file;
  gboolean         matched;
  ...
  /* leave if the model is null, we do this in lunar_path_entry_changed() to speed
   * things up, but that causes https://bugzilla.expidus.org/show_bug.cgi?id=4847. */
  if (G_UNLIKELY (model == NULL))
    return FALSE;

  /* leave if the auto completion highlight was not cleared yet, to prevent
   * https://bugzilla.expidus.org/show_bug.cgi?id=16267. */
  path_entry = LUNAR_PATH_ENTRY (user_data);
  if (G_UNLIKELY (path_entry->has_completion))
    return FALSE;
  ...
  if (G_UNLIKELY (last_slash != NULL && last_slash[1] == '\0'))
    {
      ...
      gtk_tree_model_get (model, iter, LUNAR_COLUMN_FILE, &file, -1);
      matched = !lunar_file_is_hidden (file);
      g_object_unref (G_OBJECT (file));
    }
  else
    {
      ...
      if (name_normalized != NULL)
        matched = lunar_path_entry_has_prefix_casefolded (name_normalized, last_slash);
      else
        matched = FALSE;
...
static gboolean
lunar_path_entry_match_selected (GtkEntryCompletion *completion,
                                  GtkTreeModel       *model,
                                  GtkTreeIter        *iter,
                                  gpointer            user_data)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (user_data);
  const gchar     *last_slash;
  const gchar     *text;
  LunarFile      *file;
  gchar           *real_name;
  ...
  gtk_tree_model_get (model, iter, LUNAR_COLUMN_FILE_NAME, &real_name, -1);
...
static gboolean
lunar_path_entry_parse (LunarPathEntry *path_entry,
                         gchar          **folder_part,
                         gchar          **file_part,
                         GError         **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_PATH_ENTRY (path_entry), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  _lunar_return_val_if_fail (folder_part != NULL, FALSE);
  _lunar_return_val_if_fail (file_part != NULL, FALSE);

  /* expand the filename */
  filename = lunar_util_expand_filename (gtk_entry_get_text (GTK_ENTRY (path_entry)),
  ...
  if (G_UNLIKELY (last_slash == NULL))
    {
      ...
      *file_part = g_filename_from_utf8 (filename, -1, NULL, NULL, error);
      if (G_LIKELY (*file_part != NULL))
        *folder_part = g_strdup (expidus_get_homedir ());
    }
  else
    {
      ...
      if (G_LIKELY (*folder_part != NULL))
        {
          ...
          if (G_UNLIKELY (**folder_part != G_DIR_SEPARATOR))
            {
              path = expidus_get_homefile (*folder_part, NULL);
              g_free (*folder_part);
              *folder_part = path;
            }
...
static void
lunar_path_entry_queue_check_completion (LunarPathEntry *path_entry)
{
  if (G_LIKELY (path_entry->check_completion_idle_id == 0))
    {
      path_entry->check_completion_idle_id = g_idle_add_full (G_PRIORITY_HIGH, lunar_path_entry_check_completion_idle,
                                                              path_entry, lunar_path_entry_check_completion_idle_destroy);
    }
}
...
static gboolean
lunar_path_entry_check_completion_idle (gpointer user_data)
{
  LunarPathEntry *path_entry = LUNAR_PATH_ENTRY (user_data);
  const gchar     *text;
  ...
  if (*text != '\0' && text[strlen (text) - 1] != '/')
    {
      ...
      lunar_path_entry_common_prefix_append (path_entry, TRUE);
    }
...
static void
lunar_path_entry_check_completion_idle_destroy (gpointer user_data)
{
  LUNAR_PATH_ENTRY (user_data)->check_completion_idle_id = 0;
}



/**
 * lunar_path_entry_new:
 *
 * Allocates a new #LunarPathEntry instance.
 *
 * Return value: the newly allocated #LunarPathEntry.
 **/
GtkWidget*
lunar_path_entry_new (void)
{
  return g_object_new (LUNAR_TYPE_PATH_ENTRY, NULL);
}



/**
 * lunar_path_entry_get_current_file:
 * @path_entry : a #LunarPathEntry.
 *
 * Returns the #LunarFile currently being displayed by
 * @path_entry or %NULL if @path_entry doesn't contain
 * a valid #LunarFile.
 *
 * Return value: the #LunarFile for @path_entry or %NULL.
 **/
LunarFile*
lunar_path_entry_get_current_file (LunarPathEntry *path_entry)
{
  _lunar_return_val_if_fail (LUNAR_IS_PATH_ENTRY (path_entry), NULL);
...
/**
 * lunar_path_entry_set_current_file:
 * @path_entry   : a #LunarPathEntry.
 * @current_file : a #LunarFile or %NULL.
 *
 * Sets the #LunarFile that should be displayed by
 * @path_entry to @current_file.
 **/
void
lunar_path_entry_set_current_file (LunarPathEntry *path_entry,
                                    LunarFile      *current_file)
{
  GFile    *file;
  gchar    *text;
  ...
  _lunar_return_if_fail (LUNAR_IS_PATH_ENTRY (path_entry));
  _lunar_return_if_fail (current_file == NULL || LUNAR_IS_FILE (current_file));

  file = (current_file != NULL) ? lunar_file_get_file (current_file) : NULL;
  if (G_UNLIKELY (file == NULL))
  ...
  else
    {
      ...
      if (lunar_file_is_directory (current_file)
...
/**
 * lunar_path_entry_set_working_directory:
 * @path_entry        : a #LunarPathEntry.
 * @working_directory : a #LunarFile or %NULL.
 *
 * Sets the #LunarFile that should be used as the
 * working directory for @path_entry.
 **/
void
lunar_path_entry_set_working_directory (LunarPathEntry *path_entry,
                                         LunarFile      *working_directory)
{
  _lunar_return_if_fail (LUNAR_IS_PATH_ENTRY (path_entry));
  _lunar_return_if_fail (working_directory == NULL || LUNAR_IS_FILE (working_directory));
  ...
  if (LUNAR_IS_FILE (working_directory))
    path_entry->working_directory = g_object_ref (lunar_file_get_file (working_directory));
}
