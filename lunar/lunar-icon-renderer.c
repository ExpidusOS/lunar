...
static void lunar_icon_renderer_finalize      (GObject                 *object);
static void lunar_icon_renderer_get_property  (GObject                 *object,
                                                guint                    prop_id,
                                                GValue                  *value,
                                                GParamSpec              *pspec);
static void lunar_icon_renderer_set_property  (GObject                 *object,
                                                guint                    prop_id,
                                                const GValue            *value,
                                                GParamSpec              *pspec);
static void lunar_icon_renderer_get_preferred_width (GtkCellRenderer   *renderer,
                                                GtkWidget               *widget,
                                                gint                    *minimum,
                                                gint                    *natural);
static void lunar_icon_renderer_get_preferred_height (GtkCellRenderer  *renderer,
                                                GtkWidget               *widget,
                                                gint                    *minimum,
                                                gint                    *natural);
static void lunar_icon_renderer_render        (GtkCellRenderer         *renderer,
                                                cairo_t                 *cr,
                                                GtkWidget               *widget,
...
G_DEFINE_TYPE (LunarIconRenderer, lunar_icon_renderer, GTK_TYPE_CELL_RENDERER)



static void
lunar_icon_renderer_class_init (LunarIconRendererClass *klass)
{
  GtkCellRendererClass *gtkcell_renderer_class;
  GObjectClass         *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_icon_renderer_finalize;
  gobject_class->get_property = lunar_icon_renderer_get_property;
  gobject_class->set_property = lunar_icon_renderer_set_property;

  gtkcell_renderer_class = GTK_CELL_RENDERER_CLASS (klass);
  gtkcell_renderer_class->get_preferred_width = lunar_icon_renderer_get_preferred_width;
  gtkcell_renderer_class->get_preferred_height = lunar_icon_renderer_get_preferred_height;
  gtkcell_renderer_class->render = lunar_icon_renderer_render;

  /**
   * LunarIconRenderer:drop-file:
   *
...
static void
lunar_icon_renderer_init (LunarIconRenderer *icon_renderer)
{
  /* use 1px padding */
...
static void
lunar_icon_renderer_finalize (GObject *object)
{
  LunarIconRenderer *icon_renderer = LUNAR_ICON_RENDERER (object);
  ...
  (*G_OBJECT_CLASS (lunar_icon_renderer_parent_class)->finalize) (object);
}



static void
lunar_icon_renderer_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
...
static void
lunar_icon_renderer_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
lunar_icon_renderer_get_preferred_width (GtkCellRenderer *renderer,
                                          GtkWidget       *widget,
                                          gint            *minimum,
                                          gint            *natural)
{
  LunarIconRenderer *icon_renderer = LUNAR_ICON_RENDERER (renderer);
  int xpad;
...
static void
lunar_icon_renderer_get_preferred_height (GtkCellRenderer *renderer,
                                           GtkWidget       *widget,
                                           gint            *minimum,
                                           gint            *natural)
{
...
static void
lunar_icon_renderer_color_insensitive (cairo_t   *cr,
                                        GtkWidget *widget)
{
...
static void
lunar_icon_renderer_color_selected (cairo_t   *cr,
                                     GtkWidget *widget)
{
  cairo_pattern_t *source;
  GtkStateFlags    state;
...
static void
lunar_icon_renderer_color_lighten (cairo_t   *cr,
                                    GtkWidget *widget)
{
  cairo_pattern_t *source;
...
static void
lunar_icon_renderer_render (GtkCellRenderer     *renderer,
                             cairo_t             *cr,
                             GtkWidget           *widget,
                             const GdkRectangle  *background_area,
                             const GdkRectangle  *cell_area,
                             GtkCellRendererState flags)
{
  ...
  icon = lunar_icon_factory_load_file_icon (icon_factory, icon_renderer->file, icon_state, icon_renderer->size);
  ...
  if (G_UNLIKELY (icon_state == LUNAR_FILE_ICON_STATE_DROP))
  ...
  if (G_UNLIKELY (icon_area.width > cell_area->width || icon_area.height > cell_area->height))
    {
      /* scale down to fit */
      temp = endo_gdk_pixbuf_scale_down (icon, TRUE, MAX (1, cell_area->width), MAX (1, cell_area->height));
  ...
  if (gdk_rectangle_intersect (&clip_area, &icon_area, NULL))
    {
      ...
      clipboard = lunar_clipboard_manager_get_for_display (gtk_widget_get_display (widget));
      if (lunar_clipboard_manager_has_cutted_file (clipboard, icon_renderer->file))
        {
          /* 50% translucent for cutted files */
          alpha = 0.50;
        }
      else if (lunar_file_is_hidden (icon_renderer->file))
        {
      ...
      lunar_gdk_cairo_set_source_pixbuf (cr, icon, icon_area.x, icon_area.y);
      ...
      if (G_UNLIKELY (gtk_widget_get_state_flags (widget) == GTK_STATE_FLAG_INSENSITIVE || !gtk_cell_renderer_get_sensitive (renderer)))
        lunar_icon_renderer_color_insensitive(cr,widget);

      /* paint the lighten mask */
      if (color_lighten)
        lunar_icon_renderer_color_lighten (cr, widget);

      /* paint the selected mask */
      if (color_selected)
        lunar_icon_renderer_color_selected (cr, widget);
    }
  ...
  if (G_LIKELY (icon_renderer->emblems))
    {
      ...
      emblems = lunar_file_get_emblem_names (icon_renderer->file);
          ...
          for (lp = emblems, position = 0; lp != NULL && position < max_emblems; lp = lp->next)
            {
              ...
              emblem = lunar_icon_factory_load_icon (icon_factory, lp->data, emblem_size, FALSE);
              ...
              if (G_UNLIKELY (MAX (emblem_area.width, emblem_area.height) > emblem_size))
                {
                  ...
                  temp = endo_gdk_pixbuf_scale_ratio (emblem, emblem_size);
              ...
              if (gdk_rectangle_intersect (&clip_area, &emblem_area, NULL))
                {
                  /* render the invalid parts of the icon */
                  lunar_gdk_cairo_set_source_pixbuf (cr, emblem, emblem_area.x, emblem_area.y);
                  cairo_paint (cr);
...
/**
 * lunar_icon_renderer_new:
 *
 * Creates a new #LunarIconRenderer. Adjust rendering
 ...
 *
 * Return value: the newly allocated #LunarIconRenderer.
 **/
GtkCellRenderer*
lunar_icon_renderer_new (void)
{
  return g_object_new (LUNAR_TYPE_ICON_RENDERER, NULL);
}
