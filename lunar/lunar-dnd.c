 ...
 * @folder    : the #LunarFile to which the @path_list is being dropped.
 ...
 **/
GdkDragAction
lunar_dnd_ask (GtkWidget    *widget,
                LunarFile   *folder,
                GList        *path_list,
                GdkDragAction dnd_actions)
{
  ...
  _lunar_return_val_if_fail (lunar_file_is_directory (folder), 0);
  _lunar_return_val_if_fail (GTK_IS_WIDGET (widget), 0);

  /* connect to the provider factory */
  factory = lunarx_provider_factory_get_default ();
      ...
      if (G_LIKELY (file_list != NULL && lp == NULL))
        {
          ...
          providers = lunarx_provider_factory_list_providers (factory, LUNARX_TYPE_MENU_PROVIDER);

          /* load the dnd menu items offered by the menu providers */
          for (lp = providers; lp != NULL; lp = lp->next)
            {
              /* merge the menu items from this provider */
              items = g_list_concat (items, lunarx_menu_provider_get_dnd_menu_items (lp->data, window, LUNARX_FILE_INFO (folder), file_list));
 ...
 * @file              : the #LunarFile on which the @file_list was dropped.
 ...
 **/
gboolean
lunar_dnd_perform (GtkWidget    *widget,
                    LunarFile   *file,
                    GList        *file_list,
                    ...
                    GClosure     *new_files_closure)
{
  ...
  _lunar_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (gtk_widget_get_realized (widget), FALSE);
  ...
  if (lunar_file_is_directory (file))
    {
      ...
      switch (action)
        ...
        case GDK_ACTION_COPY:
          lunar_application_copy_into (application, widget, file_list, lunar_file_get_file (file), new_files_closure);
          break;

        case GDK_ACTION_MOVE:
          lunar_application_move_into (application, widget, file_list, lunar_file_get_file (file), new_files_closure);
          break;

        case GDK_ACTION_LINK:
          lunar_application_link_into (application, widget, file_list, lunar_file_get_file (file), new_files_closure);
          break;
  ...
  else if (lunar_file_is_executable (file))
    {
      ...
      succeed = lunar_file_execute (file, NULL, widget, file_list, NULL, &error);
      if (G_UNLIKELY (!succeed))
        {
          /* display an error to the user */
          lunar_dialogs_show_error (widget, error, _("Failed to execute file \"%s\""), lunar_file_get_display_name (file));
