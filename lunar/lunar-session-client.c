...
static void     lunar_session_client_finalize            (GObject                  *object);
#ifdef HAVE_LIBSM
static gboolean lunar_session_client_connect             (LunarSessionClient      *session_client,
                                                           const gchar              *previous_id);
static void     lunar_session_client_restore             (LunarSessionClient      *session_client);
static void     lunar_session_client_die                 (SmcConn                   connection,
                                                           LunarSessionClient      *session_client);
static void     lunar_session_client_save_complete       (SmcConn                   connection,
                                                           LunarSessionClient      *session_client);
static void     lunar_session_client_save_yourself       (SmcConn                   connection,
                                                           LunarSessionClient      *session_client,
                                                           gint                      save_type,
                                                           Bool                      shutdown,
                                                           gint                      interact_style,
                                                           Bool                      fast);
static void     lunar_session_client_shutdown_cancelled  (SmcConn                   connection,
                                                           LunarSessionClient      *session_client);
#endif /* !HAVE_LIBSM */



struct _LunarSessionClientClass
{
  GObjectClass __parent__;
};

struct _LunarSessionClient
{
  GObject __parent__;
...
G_DEFINE_TYPE (LunarSessionClient, lunar_session_client, G_TYPE_OBJECT)



static void
lunar_session_client_class_init (LunarSessionClientClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_session_client_finalize;
}



static void
lunar_session_client_init (LunarSessionClient *session_client)
{
...
static void
lunar_session_client_finalize (GObject *object)
{
  LunarSessionClient *session_client = LUNAR_SESSION_CLIENT (object);
  ...
  (*G_OBJECT_CLASS (lunar_session_client_parent_class)->finalize) (object);
}
...
#ifdef HAVE_LIBSM
static gboolean
lunar_session_client_connect (LunarSessionClient *session_client,
                               const gchar         *previous_id)
{
  ...
  session_callbacks.die.callback = (SmcDieProc) lunar_session_client_die;
  session_callbacks.die.client_data = session_client;
  session_callbacks.save_complete.callback = (SmcSaveCompleteProc) lunar_session_client_save_complete;
  session_callbacks.save_complete.client_data = session_client;
  session_callbacks.save_yourself.callback = (SmcSaveYourselfProc) lunar_session_client_save_yourself;
  session_callbacks.save_yourself.client_data = session_client;
  session_callbacks.shutdown_cancelled.callback = (SmcShutdownCancelledProc) lunar_session_client_shutdown_cancelled;
  ...
  spec = g_strconcat ("sessions/Lunar-", session_client->id, NULL);
  session_client->path = expidus_resource_save_location (EXPIDUS_RESOURCE_CACHE, spec, TRUE);
  g_free (spec);
  ...
  value_restart[0].value = "Lunar";
  value_restart[0].length = sizeof ("Lunar") - 1;
...
static void
lunar_session_client_restore (LunarSessionClient *session_client)
{
  ...
  rc = expidus_rc_simple_open (session_client->path, TRUE);
  ...
  roles = expidus_rc_get_groups (rc);
  for (n = 0; roles[n] != NULL; ++n)
    {
      ...
      /* enter the group */
      expidus_rc_set_group (rc, roles[n]);

      /* determine the URI for the new window */
      uris = expidus_rc_read_list_entry (rc, "URI", ";");
      if (G_UNLIKELY (uris == NULL))
        continue;

      /* active tab */
      active_tab = expidus_rc_read_int_entry (rc, "PAGE", -1);

      /* open the new window */
      window = g_object_new (LUNAR_TYPE_WINDOW, "role", roles[n], NULL);
      lunar_application_take_window (application, GTK_WINDOW (window));
      gtk_widget_show (window);

      /* open tabs */
      if (!lunar_window_set_directories (LUNAR_WINDOW (window), uris, active_tab))
        {
          /* no tabs were opened */
...
static void
lunar_session_client_die (SmcConn              connection,
                           LunarSessionClient *session_client)
{
  LunarApplication* application;

  _lunar_return_if_fail (LUNAR_IS_SESSION_CLIENT (session_client));
  _lunar_return_if_fail (session_client->connection == connection);

  application = lunar_application_get ();
  lunar_application_quit (application);
}



static void
lunar_session_client_save_complete (SmcConn              connection,
                                     LunarSessionClient *session_client)
{
  _lunar_return_if_fail (LUNAR_IS_SESSION_CLIENT (session_client));
  _lunar_return_if_fail (session_client->connection == connection);
...
static void
lunar_session_client_save_yourself (SmcConn              connection,
                                     LunarSessionClient *session_client,
                                     gint                 save_type,
                                     Bool                 shutdown,
                                     gint                 interact_style,
              ...
              for (lp = windows; lp != NULL; lp = lp->next)
                {
                  ...
                  uris = lunar_window_get_directories (lp->data, &active_page);
                  if (G_UNLIKELY (uris == NULL))
                    continue;
...
static void
lunar_session_client_shutdown_cancelled (SmcConn              connection,
                                          LunarSessionClient *session_client)
{
  _lunar_return_if_fail (LUNAR_IS_SESSION_CLIENT (session_client));
  _lunar_return_if_fail (session_client->connection == connection);
...
/**
 * lunar_session_client_new:
 * @session_id : the previous session id, or %NULL.
 *
 * Allocates a new #LunarSessionClient with the specified
 ...
 *
 * Return value: the newly allocated #LunarSessionClient.
 **/
LunarSessionClient*
lunar_session_client_new (const gchar *session_id)
{
  LunarSessionClient *session_client;

  /* allocate a new session client */
  session_client = g_object_new (LUNAR_TYPE_SESSION_CLIENT, NULL);

#ifdef HAVE_LIBSM
  /* try to connect to the session manager and restore the previous session */
  if (lunar_session_client_connect (session_client, session_id) && session_id != NULL)
    lunar_session_client_restore (session_client);
#endif /* !HAVE_LIBSM */
