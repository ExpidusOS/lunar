...
#include <lunar/lunar-renamer-progress.h>
...
typedef enum
{
  LUNAR_RENAMER_ACTION_ADD_FILES,
  LUNAR_RENAMER_ACTION_REMOVE_FILES,
  LUNAR_RENAMER_ACTION_CLEAR,
  LUNAR_RENAMER_ACTION_ABOUT,

} LunarRenamerAction;



static void        lunar_renamer_dialog_dispose               (GObject                  *object);
static void        lunar_renamer_dialog_finalize              (GObject                  *object);
static void        lunar_renamer_dialog_get_property          (GObject                  *object,
                                                                guint                     prop_id,
                                                                GValue                   *value,
                                                                GParamSpec               *pspec);
static void        lunar_renamer_dialog_set_property          (GObject                  *object,
                                                                guint                     prop_id,
                                                                const GValue             *value,
                                                                GParamSpec               *pspec);
static void        lunar_renamer_dialog_realize               (GtkWidget                *widget);
static void        lunar_renamer_dialog_unrealize             (GtkWidget                *widget);
static void        lunar_renamer_dialog_response              (GtkDialog                *dialog,
                                                                gint                      response);
static void        lunar_renamer_dialog_context_menu          (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_help                  (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_save                  (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_action_add_files      (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_action_remove_files   (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_action_clear          (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_action_about          (LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_name_column_clicked   (GtkTreeViewColumn        *column,
                                                                LunarRenamerDialog      *renamer_dialog);
static gboolean    lunar_renamer_dialog_button_press_event    (GtkWidget                *tree_view,
                                                                GdkEventButton           *event,
                                                                LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_drag_data_received    (GtkWidget                *tree_view,
                                                                ...
                                                                guint                     timestamp,
                                                                LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_drag_leave            (GtkWidget                *tree_view,
                                                                GdkDragContext           *context,
                                                                guint                     timestamp,
                                                                LunarRenamerDialog      *renamer_dialog);
static gboolean    lunar_renamer_dialog_drag_motion           (GtkWidget                *tree_view,
                                                                GdkDragContext           *context,
                                                                gint                      x,
                                                                gint                      y,
                                                                guint                     timestamp,
                                                                LunarRenamerDialog      *renamer_dialog);
static gboolean    lunar_renamer_dialog_drag_drop             (GtkWidget                *tree_view,
                                                                GdkDragContext           *context,
                                                                gint                      x,
                                                                gint                      y,
                                                                guint                     timestamp,
                                                                LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_notify_page           (GtkNotebook              *notebook,
                                                                GParamSpec               *pspec,
                                                                LunarRenamerDialog      *renamer_dialog);
static gboolean    lunar_renamer_dialog_popup_menu            (GtkWidget                *tree_view,
                                                                LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_row_activated         (GtkTreeView              *tree_view,
                                                                GtkTreePath              *path,
                                                                GtkTreeViewColumn        *column,
                                                                LunarRenamerDialog      *renamer_dialog);
static void        lunar_renamer_dialog_selection_changed     (GtkTreeSelection         *selection,
                                                                LunarRenamerDialog      *renamer_dialog);
static LunarFile *lunar_renamer_dialog_get_current_directory (LunarRenamerDialog *renamer_dialog);
static void        lunar_renamer_dialog_set_current_directory (LunarRenamerDialog *renamer_dialog,
                                                                LunarFile          *current_directory);
static GList      *lunar_renamer_dialog_get_selected_files    (LunarRenamerDialog *renamer_dialog);
static gboolean    lunar_renamer_dialog_get_standalone        (LunarRenamerDialog *renamer_dialog);
static void        lunar_renamer_dialog_set_standalone        (LunarRenamerDialog *renamer_dialog,
                                                                gboolean             fixed);
static GtkWidget  *lunar_renamer_dialog_append_menu_item      (LunarRenamerDialog *renamer_dialog,
                                                                GtkMenuShell        *menu,
                                                                LunarRenamerAction  action);



struct _LunarRenamerDialogClass
{
  LunarAbstractDialogClass __parent__;
};

struct _LunarRenamerDialog
{
...
static ExpidusGtkActionEntry lunar_renamer_action_entries[] =
{
    { LUNAR_RENAMER_ACTION_ADD_FILES,    "<Actions>/LunarRenamerDialog/add-files",    "", EXPIDUS_GTK_IMAGE_MENU_ITEM, N_ ("_Add Files..."), N_ ("Include additional files in the list of files to be renamed"), "list-add",    G_CALLBACK (lunar_renamer_dialog_action_add_files),   },
    { LUNAR_RENAMER_ACTION_REMOVE_FILES, "<Actions>/LunarRenamerDialog/remove-files", "", EXPIDUS_GTK_IMAGE_MENU_ITEM, NULL,                 NULL,                                                               "list-remove", G_CALLBACK (lunar_renamer_dialog_action_remove_files),},
    { LUNAR_RENAMER_ACTION_CLEAR,        "",                                           "", EXPIDUS_GTK_IMAGE_MENU_ITEM, N_ ("Clear"),         N_ ("Clear the file list below"),                                   "edit-clear",  G_CALLBACK (lunar_renamer_dialog_action_clear),       },
    { LUNAR_RENAMER_ACTION_ABOUT,        "",                                           "", EXPIDUS_GTK_IMAGE_MENU_ITEM, N_ ("_About"),        N_ ("Display information about Lunar Bulk Rename"),                "help-about",  G_CALLBACK (lunar_renamer_dialog_action_about),       },
};

#define get_action_entry(id) expidus_gtk_get_action_entry_by_id(lunar_renamer_action_entries,G_N_ELEMENTS(lunar_renamer_action_entries),id)
...
G_DEFINE_TYPE (LunarRenamerDialog, lunar_renamer_dialog, LUNAR_TYPE_ABSTRACT_DIALOG)



static void
lunar_renamer_dialog_class_init (LunarRenamerDialogClass *klass)
{
  ...
  gobject_class->dispose = lunar_renamer_dialog_dispose;
  gobject_class->finalize = lunar_renamer_dialog_finalize;
  gobject_class->get_property = lunar_renamer_dialog_get_property;
  gobject_class->set_property = lunar_renamer_dialog_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->realize = lunar_renamer_dialog_realize;
  gtkwidget_class->unrealize = lunar_renamer_dialog_unrealize;

  gtkdialog_class = GTK_DIALOG_CLASS (klass);
  gtkdialog_class->response = lunar_renamer_dialog_response;

  /**
   * LunarRenamerDialog:current-directory:
   *
   * The current directory of the #LunarRenamerDialog,
  ...
  /**
   * LunarRenamerDialog:selected-files:
   *
   * The list of currently selected #LunarFile<!---->s
   * in this #LunarRenamerDialog.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_SELECTED_FILES,
  ...
  /**
   * LunarRenamerDialog:standalone:
   *
   * %TRUE if the #LunarRenamerDialog should display a
   * toolbar and add several actions which are only
   * useful in a standalone fashion.
   *
   * If the #LunarRenamerDialog is opened from within
   * Lunar this property will always be %FALSE.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_STANDALONE,
...
trd_renamer_compare (gconstpointer a,
                     gconstpointer b)
{
  return g_utf8_collate (lunarx_renamer_get_name (LUNARX_RENAMER (a)),
                         lunarx_renamer_get_name (LUNARX_RENAMER (b)));
}



static void
lunar_renamer_dialog_init (LunarRenamerDialog *renamer_dialog)
{
  ...
  renamer_dialog->model = lunar_renamer_model_new ();

  expidus_gtk_translate_action_entries (lunar_renamer_action_entries, G_N_ELEMENTS (lunar_renamer_action_entries));

  /* determine the list of available renamers (using the renamer providers) */
  provider_factory = lunarx_provider_factory_get_default ();
  providers = lunarx_provider_factory_list_providers (provider_factory, LUNARX_TYPE_RENAMER_PROVIDER);
  for (lp = providers; lp != NULL; lp = lp->next)
    {
      /* determine the renamers for this provider */
      renamers = g_list_concat (renamers, lunarx_renamer_provider_get_renamers (lp->data));
  ...
  endo_binding_new (G_OBJECT (renamer_dialog->model), "can-rename", G_OBJECT (button), "sensitive");
  ...
  renamer_dialog->launcher = g_object_new (LUNAR_TYPE_LAUNCHER, "widget", GTK_WIDGET (renamer_dialog), NULL);
  endo_binding_new (G_OBJECT (renamer_dialog), "selected-files", G_OBJECT (renamer_dialog->launcher), "selected-files");
  endo_binding_new (G_OBJECT (renamer_dialog), "current-directory", G_OBJECT (renamer_dialog->launcher), "current-directory");

  /* add the toolbar to the dialog */
  toolbar = gtk_toolbar_new ();
  expidus_gtk_tool_button_new_from_action_entry (get_action_entry (LUNAR_RENAMER_ACTION_ADD_FILES), G_OBJECT (renamer_dialog), GTK_TOOLBAR (toolbar));
  expidus_gtk_tool_button_new_from_action_entry (get_action_entry (LUNAR_RENAMER_ACTION_REMOVE_FILES), G_OBJECT (renamer_dialog), GTK_TOOLBAR (toolbar));
  ...
  expidus_gtk_tool_button_new_from_action_entry (get_action_entry (LUNAR_RENAMER_ACTION_CLEAR), G_OBJECT (renamer_dialog), GTK_TOOLBAR (toolbar));
  seperator = gtk_separator_tool_item_new ();
  gtk_container_add (GTK_CONTAINER (toolbar), GTK_WIDGET (seperator));
  gtk_widget_show (GTK_WIDGET (seperator));
  expidus_gtk_tool_button_new_from_action_entry (get_action_entry (LUNAR_RENAMER_ACTION_ABOUT), G_OBJECT (renamer_dialog), GTK_TOOLBAR (toolbar));
  endo_binding_new (G_OBJECT (renamer_dialog), "standalone", G_OBJECT (toolbar), "visible");
  ...
  gtk_tree_view_set_search_column (GTK_TREE_VIEW (renamer_dialog->tree_view), LUNAR_RENAMER_MODEL_COLUMN_OLDNAME);
  ...
  g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "button-press-event", G_CALLBACK (lunar_renamer_dialog_button_press_event), renamer_dialog);
  g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "popup-menu", G_CALLBACK (lunar_renamer_dialog_popup_menu), renamer_dialog);
  g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "row-activated", G_CALLBACK (lunar_renamer_dialog_row_activated), renamer_dialog);
  ...
  g_signal_connect (G_OBJECT (column), "clicked", G_CALLBACK (lunar_renamer_dialog_name_column_clicked), renamer_dialog);
  renderer = g_object_new (LUNAR_TYPE_ICON_RENDERER, "size", 16, NULL);
  gtk_tree_view_column_pack_start (column, renderer, FALSE);
  gtk_tree_view_column_set_attributes (column, renderer, "file", LUNAR_RENAMER_MODEL_COLUMN_FILE, NULL);
  renderer = g_object_new (GTK_TYPE_CELL_RENDERER_TEXT, "ellipsize", PANGO_ELLIPSIZE_END, NULL);
  gtk_tree_view_column_pack_start (column, renderer, TRUE);
  gtk_tree_view_column_set_attributes (column, renderer, "text", LUNAR_RENAMER_MODEL_COLUMN_OLDNAME, NULL);
  ...
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "foreground-set", LUNAR_RENAMER_MODEL_COLUMN_CONFLICT,
                                       "text", LUNAR_RENAMER_MODEL_COLUMN_NEWNAME,
                                       "weight", LUNAR_RENAMER_MODEL_COLUMN_CONFLICT_WEIGHT,
                                       NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (renamer_dialog->tree_view), column);
  ...
  renamer_dialog->progress = lunar_renamer_progress_new ();
  gtk_box_pack_start (GTK_BOX (vbox), renamer_dialog->progress, FALSE, FALSE, 0);
  endo_binding_new_with_negation (G_OBJECT (renamer_dialog->progress), "visible", G_OBJECT (rbox), "visible");
  endo_binding_new_with_negation (G_OBJECT (renamer_dialog->progress), "visible", G_OBJECT (swin), "sensitive");
  endo_binding_new_with_negation (G_OBJECT (renamer_dialog->progress), "visible", G_OBJECT (toolbar), "sensitive");
  endo_binding_new (G_OBJECT (renamer_dialog->progress), "visible", G_OBJECT (renamer_dialog->model), "frozen");
  ...
  endo_binding_new_with_negation (G_OBJECT (renamer_dialog->progress), "visible", G_OBJECT (frame), "sensitive");
  ...
  if (G_LIKELY (renamers != NULL))
    {
      ...
      /* open the config file, expidus_rc_config_open() should never return NULL */
      rc = expidus_rc_config_open (EXPIDUS_RESOURCE_CONFIG, "Lunar/renamerrc", TRUE);
      expidus_rc_set_group (rc, "Configuration");

      /* create the renamer combo box for the renamer selection */
      rcombo = gtk_combo_box_text_new ();
      for (lp = renamers; lp != NULL; lp = lp->next)
        gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (rcombo), lunarx_renamer_get_name (lp->data));
      ...
      g_signal_connect_swapped (G_OBJECT (button), "clicked", G_CALLBACK (lunar_renamer_dialog_help), renamer_dialog);
      ...
      klass = g_type_class_ref (LUNAR_TYPE_RENAMER_MODE);
      active_str = expidus_rc_read_entry_untranslated (rc, "LastActiveMode", "");
      for (active = 0, n = 0; n < klass->n_values; ++n)
        {
          if (endo_str_is_equal (active_str, klass->values[n].value_name))
            active = n;
          gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (mcombo), _(klass->values[n].value_nick));
        }
      endo_mutual_binding_new (G_OBJECT (renamer_dialog->model), "mode", G_OBJECT (mcombo), "active");
      ...
      g_signal_connect (G_OBJECT (notebook), "notify::page", G_CALLBACK (lunar_renamer_dialog_notify_page), renamer_dialog);
      gtk_container_add (GTK_CONTAINER (frame), notebook);
      gtk_widget_show (notebook);

      /* determine the last active renamer (as string) */
      active_str = expidus_rc_read_entry_untranslated (rc, "LastActiveRenamer", "");
      ...
      for (active = 0, lp = renamers; lp != NULL; lp = lp->next)
        {
          ...
          if (endo_str_is_equal (G_OBJECT_TYPE_NAME (lp->data), active_str))
            active = g_list_position (renamers, lp);

          /* try to load the settings for the renamer */
          entries = expidus_rc_get_entries (rc, G_OBJECT_TYPE_NAME (lp->data));
          if (G_LIKELY (entries != NULL))
            {
              /* add the settings to a hash table */
              expidus_rc_set_group (rc, G_OBJECT_TYPE_NAME (lp->data));
              settings = g_hash_table_new (g_str_hash, g_str_equal);
              for (n = 0; entries[n] != NULL; ++n)
                g_hash_table_insert (settings, entries[n], (gchar *) expidus_rc_read_entry_untranslated (rc, entries[n], NULL));

              /* let the renamer load the settings */
              lunarx_renamer_load (LUNARX_RENAMER (lp->data), settings);
      ...
      endo_binding_new (G_OBJECT (rcombo), "active", G_OBJECT (notebook), "page");
      ...
      g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "drag-data-received", G_CALLBACK (lunar_renamer_dialog_drag_data_received), renamer_dialog);
      g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "drag-leave", G_CALLBACK (lunar_renamer_dialog_drag_leave), renamer_dialog);
      g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "drag-motion", G_CALLBACK (lunar_renamer_dialog_drag_motion), renamer_dialog);
      g_signal_connect (G_OBJECT (renamer_dialog->tree_view), "drag-drop", G_CALLBACK (lunar_renamer_dialog_drag_drop), renamer_dialog);

      /* setup the tree selection */
      selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (renamer_dialog->tree_view));
      gtk_tree_selection_set_mode (selection, GTK_SELECTION_MULTIPLE);
      g_signal_connect (G_OBJECT (selection), "changed", G_CALLBACK (lunar_renamer_dialog_selection_changed), renamer_dialog);
      lunar_renamer_dialog_selection_changed (selection, renamer_dialog);
    }
  else
    {
      ...
      /* TRANSLATORS: You can test this string by temporarily removing lunar-sbr.* from $libdir/lunarx-2/,
       *              and opening the multi rename dialog by selecting multiple files and pressing F2.
       */
      label = gtk_label_new (_("No renamer modules were found on your system. Please check your\n"
                               "installation or contact your system administrator. If you install Lunar\n"
...
static void
lunar_renamer_dialog_dispose (GObject *object)
{
  LunarRenamerDialog *renamer_dialog = LUNAR_RENAMER_DIALOG (object);

  /* reset the "current-directory" property */
  lunar_renamer_dialog_set_current_directory (renamer_dialog, NULL);

  (*G_OBJECT_CLASS (lunar_renamer_dialog_parent_class)->dispose) (object);
}



static void
lunar_renamer_dialog_finalize (GObject *object)
{
  LunarRenamerDialog *renamer_dialog = LUNAR_RENAMER_DIALOG (object);
  ...
  lunarx_file_info_list_free (renamer_dialog->selected_files);

  (*G_OBJECT_CLASS (lunar_renamer_dialog_parent_class)->finalize) (object);
}



static void
lunar_renamer_dialog_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CURRENT_DIRECTORY:
      g_value_set_object (value, lunar_renamer_dialog_get_current_directory (renamer_dialog));
      break;

    case PROP_SELECTED_FILES:
      g_value_set_boxed (value, lunar_renamer_dialog_get_selected_files (renamer_dialog));
      break;

    case PROP_STANDALONE:
      g_value_set_boolean (value, lunar_renamer_dialog_get_standalone (renamer_dialog));
      break;
...
static void
lunar_renamer_dialog_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CURRENT_DIRECTORY:
      lunar_renamer_dialog_set_current_directory (renamer_dialog, g_value_get_object (value));
      break;

    case PROP_STANDALONE:
      lunar_renamer_dialog_set_standalone (renamer_dialog, g_value_get_boolean (value));
      break;
...
static void
lunar_renamer_dialog_realize (GtkWidget *widget)
{
  LunarRenamerDialog *renamer_dialog = LUNAR_RENAMER_DIALOG (widget);
  GtkIconTheme        *icon_theme;

  /* realize the widget */
  (*GTK_WIDGET_CLASS (lunar_renamer_dialog_parent_class)->realize) (widget);

  /* grab a reference on the icon factory for the screen */
  icon_theme = gtk_icon_theme_get_for_screen (gtk_widget_get_screen (widget));
  renamer_dialog->icon_factory = lunar_icon_factory_get_for_icon_theme (icon_theme);
}



static void
lunar_renamer_dialog_unrealize (GtkWidget *widget)
{
  LunarRenamerDialog *renamer_dialog = LUNAR_RENAMER_DIALOG (widget);
  ...
  (*GTK_WIDGET_CLASS (lunar_renamer_dialog_parent_class)->unrealize) (widget);
}



static void
lunar_renamer_dialog_response (GtkDialog *dialog,
                                gint       response)
{
  LunarRenamerDialog *renamer_dialog = LUNAR_RENAMER_DIALOG (dialog);
  GtkTreeIter          iter;
  LunarFile          *file;
  ...
  if (G_LIKELY (response == GTK_RESPONSE_ACCEPT))
    {
      ...
      lunar_renamer_dialog_save (renamer_dialog);
          ...
          do
            {
              ...
              gtk_tree_model_get (GTK_TREE_MODEL (renamer_dialog->model), &iter,
                                  LUNAR_RENAMER_MODEL_COLUMN_NEWNAME, &name,
                                  LUNAR_RENAMER_MODEL_COLUMN_FILE, &file,
                                  -1);

              /* check if this row should be renamed */
              if (G_LIKELY (name != NULL && *name != '\0'))
                pair_list = g_list_prepend (pair_list, lunar_renamer_pair_new (file, name));
      ...
      lunar_renamer_progress_run (LUNAR_RENAMER_PROGRESS (renamer_dialog->progress), pair_list);

      /* hide the rename progress bar */
      gtk_widget_hide (renamer_dialog->progress);

      /* release the pairs */
      lunar_renamer_pair_list_free (pair_list);

      /* check if we're in standalone mode */
      if (lunar_renamer_dialog_get_standalone (renamer_dialog))
        {
  ...
  else if (lunar_renamer_progress_running (LUNAR_RENAMER_PROGRESS (renamer_dialog->progress)))
    {
      /* cancel the rename operation */
      lunar_renamer_progress_cancel (LUNAR_RENAMER_PROGRESS (renamer_dialog->progress));
    }
  else
    {
      /* hide and destroy the dialog window */
...
/**
 * lunar_renamer_dialog_append_menu_item:
 * @renamer_dialog : Instance of a  #LunarRenamerDialog
 * @menu           : #GtkMenuShell to which the item should be added
 * @action         : #LunarRenamerAction to select which item should be added
 *
 ...
 **/
static GtkWidget*
lunar_renamer_dialog_append_menu_item (LunarRenamerDialog *renamer_dialog,
                                        GtkMenuShell        *menu,
                                        LunarRenamerAction  action)
{
  gchar                      *label_text;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), NULL);
  _lunar_return_val_if_fail (action_entry != NULL, NULL);

  switch (action)
    {
      case LUNAR_RENAMER_ACTION_REMOVE_FILES:
        ...
        return item;
      default:
        return expidus_gtk_menu_item_new_from_action_entry (action_entry, G_OBJECT (renamer_dialog), menu);
    }
  return NULL;
}
...
static void
lunar_renamer_dialog_context_menu (LunarRenamerDialog *renamer_dialog)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));

  /* grab an additional reference on the dialog */
  g_object_ref (G_OBJECT (renamer_dialog));

  menu = g_object_new (LUNAR_TYPE_MENU, "menu-type", LUNAR_MENU_TYPE_CONTEXT_RENAMER,
                                         "launcher", renamer_dialog->launcher,
                                         "tab-support-disabled", TRUE,
                                         "change_directory-support-disabled", TRUE, NULL);
  lunar_menu_add_sections (menu, LUNAR_MENU_SECTION_OPEN | LUNAR_MENU_SECTION_SENDTO);

  /* determine the active renamer */
  renamer = lunar_renamer_model_get_renamer (renamer_dialog->model);
  if (G_LIKELY (renamer != NULL))
    {
      /* determine the menu items provided by the active renamer */
      items = lunarx_renamer_get_menu_items (renamer, GTK_WINDOW (renamer_dialog), renamer_dialog->selected_files);
      if (items != NULL)
        {
          for(lp = items; lp != NULL; lp = lp->next)
              lunar_gtk_menu_lunarx_menu_item_new (lp->data, GTK_MENU_SHELL (menu));
          g_list_free (items);
          expidus_gtk_menu_append_seperator (GTK_MENU_SHELL (menu));
        }
    }
  lunar_renamer_dialog_append_menu_item (renamer_dialog, GTK_MENU_SHELL (menu), LUNAR_RENAMER_ACTION_ADD_FILES);
  lunar_renamer_dialog_append_menu_item (renamer_dialog, GTK_MENU_SHELL (menu), LUNAR_RENAMER_ACTION_REMOVE_FILES);
  expidus_gtk_menu_append_seperator (GTK_MENU_SHELL (menu));
  lunar_menu_add_sections (menu, LUNAR_MENU_SECTION_PROPERTIES);
  lunar_menu_hide_accel_labels (menu);
...
static void
lunar_renamer_dialog_help (LunarRenamerDialog *renamer_dialog)
{
  LunarxRenamer *renamer;
      ...
      if (!uri_launched)
        {
          ...
          lunar_dialogs_show_error (GTK_WIDGET (renamer_dialog), error, _("Failed to open the documentation browser"));
...
trd_save_foreach (gpointer key,
                  ...
                  gpointer user_data)
{
  expidus_rc_write_entry (user_data, key, value);
}



static void
lunar_renamer_dialog_save (LunarRenamerDialog *renamer_dialog)
{
  LunarxRenamer *renamer;
  GHashTable     *settings;
  GEnumClass     *klass;
  ...
  rc = expidus_rc_config_open (EXPIDUS_RESOURCE_CONFIG, "Lunar/renamerrc", FALSE);
  ...
  if (G_LIKELY (renamer != NULL))
    {
      ...
      klass = g_type_class_ref (LUNAR_TYPE_RENAMER_MODE);
      value = g_enum_get_value (klass, lunar_renamer_model_get_mode (renamer_dialog->model));
      if (G_LIKELY (value != NULL))
        expidus_rc_write_entry (rc, "LastActiveMode", value->value_name);
      g_type_class_unref (klass);

      /* remember renamer as last active */
      expidus_rc_write_entry (rc, "LastActiveRenamer", G_OBJECT_TYPE_NAME (renamer));

      /* save the renamer's settings */
      settings = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);
      expidus_rc_delete_group (rc, G_OBJECT_TYPE_NAME (renamer), TRUE);
      expidus_rc_set_group (rc, G_OBJECT_TYPE_NAME (renamer));
      lunarx_renamer_save (renamer, settings);
...
static void
lunar_renamer_dialog_action_add_files (LunarRenamerDialog *renamer_dialog)
{
  ...
  if (G_LIKELY (renamer_dialog->current_directory != NULL))
    {
      ...
      uri = lunarx_file_info_get_uri (LUNARX_FILE_INFO (renamer_dialog->current_directory));
      ...
      for (lp = uris; lp != NULL; lp = lp->next)
        {
          ...
          file = lunar_file_get_for_uri (lp->data, NULL);
          if (G_LIKELY (file != NULL))
            {
              /* append the file to the renamer model */
              lunar_renamer_model_append (renamer_dialog->model, file);
      ...
      if (G_LIKELY (uri != NULL))
        {
          ...
          file = lunar_file_get_for_uri (uri, NULL);
          if (G_LIKELY (file != NULL))
            {
              /* remember the current folder of the chooser as default */
              lunar_renamer_dialog_set_current_directory (renamer_dialog, file);
...
static void
lunar_renamer_dialog_action_remove_files (LunarRenamerDialog *renamer_dialog)
{
      ...
      if (G_LIKELY (path != NULL))
        {
          ...
          lunar_renamer_model_remove (LUNAR_RENAMER_MODEL (model), path);
...
static void
lunar_renamer_dialog_action_clear (LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));

  /* just clear the list of files in the model */
  lunar_renamer_model_clear (renamer_dialog->model);
}



static void
lunar_renamer_dialog_action_about (LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));

  /* just popup the about dialog */
  lunar_dialogs_show_about (GTK_WINDOW (renamer_dialog), _("Bulk Rename"),
                             _("Lunar Bulk Rename is a powerful and extensible\n"
...
static void
lunar_renamer_dialog_name_column_clicked (GtkTreeViewColumn   *column,
                                           LunarRenamerDialog *renamer_dialog)
{
  GtkSortType sort_order = GTK_SORT_ASCENDING;

  _lunar_return_if_fail (renamer_dialog->name_column == column);
  ...
  /* sort the model */
  lunar_renamer_model_sort (renamer_dialog->model, sort_order);
}



static gboolean
lunar_renamer_dialog_button_press_event (GtkWidget           *tree_view,
                                          GdkEventButton      *event,
                                          LunarRenamerDialog *renamer_dialog)
{
  GtkTreeSelection *selection;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), FALSE);
  _lunar_return_val_if_fail (GTK_IS_TREE_VIEW (tree_view), FALSE);
  ...
  if (event->window == gtk_tree_view_get_bin_window (GTK_TREE_VIEW (tree_view)) && event->type == GDK_BUTTON_PRESS && event->button == 3)
    {
      ...
      lunar_renamer_dialog_context_menu (renamer_dialog);
...
static void
lunar_renamer_dialog_drag_data_received (GtkWidget           *tree_view,
                                          GdkDragContext      *context,
                                          gint                 x,
                                          gint                 y,
                                          GtkSelectionData    *selection_data,
                                          ...
                                          LunarRenamerDialog *renamer_dialog)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));
  _lunar_return_if_fail (GTK_IS_TREE_VIEW (tree_view));
  ...
  if (info == TARGET_TEXT_URI_LIST && gtk_selection_data_get_format (selection_data) == 8 && gtk_selection_data_get_length (selection_data) > 0)
    {
      ...
      file_list = lunar_g_file_list_new_from_string ((const gchar *) gtk_selection_data_get_data (selection_data));
          ...
          if (G_LIKELY (file != NULL))
            {
              ...
              lunar_renamer_model_insert (renamer_dialog->model, file, position);
...
static void
lunar_renamer_dialog_drag_leave (GtkWidget           *tree_view,
                                  GdkDragContext      *context,
                                  guint                timestamp,
                                  LunarRenamerDialog *renamer_dialog)
{
...
static gboolean
lunar_renamer_dialog_drag_motion (GtkWidget           *tree_view,
                                   GdkDragContext      *context,
                                   gint                 x,
                                   gint                 y,
                                   guint                timestamp,
...
static gboolean
lunar_renamer_dialog_drag_drop (GtkWidget           *tree_view,
                                 GdkDragContext      *context,
                                 gint                 x,
                                 gint                 y,
                                 guint                timestamp,
      ...
      if (G_LIKELY (rows != NULL))
        {
          ...
          if (G_LIKELY (rows != NULL))
            {
              lunar_renamer_model_reorder (renamer_dialog->model, rows, position);
...
static void
lunar_renamer_dialog_notify_page (GtkNotebook         *notebook,
                                   GParamSpec          *pspec,
                                   LunarRenamerDialog *renamer_dialog)
{
  GtkWidget *renamer;
  ...
  if (G_LIKELY (page >= 0))
    {
      ...
      lunar_renamer_model_set_renamer (renamer_dialog->model, LUNARX_RENAMER (renamer));
    }
}
...
static gboolean
lunar_renamer_dialog_popup_menu (GtkWidget           *tree_view,
                                  LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_val_if_fail (GTK_IS_TREE_VIEW (tree_view), FALSE);
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), FALSE);
...
static void
lunar_renamer_dialog_row_activated (GtkTreeView         *tree_view,
                                     GtkTreePath         *path,
                                     GtkTreeViewColumn   *column,
                                     LunarRenamerDialog *renamer_dialog)
{
  ...
  lunar_launcher_activate_selected_files (renamer_dialog->launcher, LUNAR_LAUNCHER_OPEN_AS_NEW_WINDOW, NULL);
}



static void
lunar_renamer_dialog_selection_changed (GtkTreeSelection    *selection,
                                         LunarRenamerDialog *renamer_dialog)
{
  GtkTreeModel *model;
  GtkTreeIter   iter;
  ...
  _lunar_return_if_fail (GTK_IS_TREE_SELECTION (selection));
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));
      ...
      if (gtk_tree_model_get_iter (model, &iter, lp->data))
        {
          ...
          gtk_tree_model_get (model, &iter, LUNAR_RENAMER_MODEL_COLUMN_FILE, &file, -1);
          if (G_LIKELY (file != NULL))
...
/**
 * lunar_renamer_dialog_get_current_directory:
 * @renamer_dialog : a #LunarRenamerDialog.
 *
 ...
 **/
static LunarFile*
lunar_renamer_dialog_get_current_directory (LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), NULL);
...
/**
 * lunar_renamer_dialog_set_current_directory:
 * @renamer_dialog    : a #LunarRenamerDialog.
 * @current_directory : a #LunarFile or %NULL.
 *
 ...
 **/
static void
lunar_renamer_dialog_set_current_directory (LunarRenamerDialog *renamer_dialog,
                                             LunarFile          *current_directory)
{
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));
...
/**
 * lunar_renamer_dialog_get_selected_files:
 * @renamer_dialog : a #LunarRenamerDialog.
 *
 * Returns the #GList of the currently selected #LunarFile<!---->s
 * in @renamer_dialog.
 *
 * Return value: the list of currently selected files.
 **/
static GList*
lunar_renamer_dialog_get_selected_files (LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), NULL);
...
/**
 * lunar_renamer_dialog_get_standalone:
 * @renamer_dialog : a #LunarRenamerDialog.
 *
 ...
 **/
static gboolean
lunar_renamer_dialog_get_standalone (LunarRenamerDialog *renamer_dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog), FALSE);
...
/**
 * lunar_renamer_dialog_set_standalone:
 * @renamer_dialog : a #LunarRenamerDialog.
 ...
 **/
static void
lunar_renamer_dialog_set_standalone (LunarRenamerDialog *renamer_dialog,
                                      gboolean             standalone)
{
  _lunar_return_if_fail (LUNAR_IS_RENAMER_DIALOG (renamer_dialog));
...
/**
 * lunar_show_renamer_dialog:
 * @parent            : the #GtkWidget or the #GdkScreen on which to open the
 *                      dialog. May also be %NULL in which case the default
 *                      #GdkScreen will be used.
 * @current_directory : the current directory for the #LunarRenamerDialog,
 ...
 * @files             : the list of #LunarFile<!---->s to rename.
 ...
 *
 * Convenience function to display a #LunarRenamerDialog with
 * the given parameters.
 **/
void
lunar_show_renamer_dialog (gpointer     parent,
                            LunarFile  *current_directory,
                            GList       *files,
                            gboolean     standalone,
                            const gchar *startup_id)
{
  ...
  dialog = g_object_new (LUNAR_TYPE_RENAMER_DIALOG,
  ...
  for (lp = files; lp != NULL; lp = lp->next)
    lunar_renamer_model_append (LUNAR_RENAMER_DIALOG (dialog)->model, lp->data);
