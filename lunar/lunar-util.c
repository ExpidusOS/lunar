 ...
 **/
static inline gchar *
lunar_util_strrchr_offset (const gchar *str,
                            const gchar *offset,
                            gchar        c)
{
  const gchar *p;
...
**/
gchar *
lunar_util_str_get_extension (const gchar *filename)
{
      ...
      if (strcasecmp (ext, compressed[i]) == 0)
        {
          ...
          dot2 = lunar_util_strrchr_offset (filename, dot - 1, '.');
          if (dot2 != NULL
              && dot2 != filename)
            {
...
void
lunar_util_load_bookmarks (GFile               *bookmarks_file,
                            ...
                            gpointer             user_data)
{
  ...
  _lunar_return_if_fail (G_IS_FILE (bookmarks_file));
  _lunar_return_if_fail (g_file_is_native (bookmarks_file));
  _lunar_return_if_fail (foreach_func != NULL);
 ...
 **/
gchar *
lunar_util_expand_filename (const gchar  *filename,
  ...
  if (*filename == '~')
    {
      ...
      if (G_UNLIKELY (*remainder == '\0'))
        return g_strdup (expidus_get_homedir ());
      ...
      if (G_LIKELY (slash == remainder))
        {
          ...
          replacement = expidus_get_homedir ();
        }
      else
        {
...
/**
 * lunar_util_humanize_file_time:
 * @file_time         : a #guint64 timestamp.
 * @date_style        : the #LunarDateFormat used to humanize the @file_time.
 ...
 **/
gchar*
lunar_util_humanize_file_time (guint64          file_time,
  ...
  if (G_LIKELY (file_time != 0))
    {
      ...
      if (date_style == LUNAR_DATE_STYLE_SIMPLE || date_style == LUNAR_DATE_STYLE_SHORT)
        {
          ...
          if (diff == 0)
            {
              if (date_style == LUNAR_DATE_STYLE_SIMPLE)
                {
                  /* TRANSLATORS: file was modified less than one day ago */
                  return g_strdup (_("Today"));
                }
              else /* if (date_style == LUNAR_DATE_STYLE_SHORT) */
                {
                  /* TRANSLATORS: file was modified less than one day ago */
                  return endo_strdup_strftime (_("Today at %X"), &tfile);
                }
            }
          else if (diff == 1)
            {
              if (date_style == LUNAR_DATE_STYLE_SIMPLE)
                ...
                {
                  /* TRANSLATORS: file was modified less than two days ago */
                  return endo_strdup_strftime (_("Yesterday at %X"), &tfile);
                }
            }
          else
            {
              if (diff > 1 && diff < 7)
                {
                  /* Days from last week */
                  date_format = (date_style == LUNAR_DATE_STYLE_SIMPLE) ? "%A" : _("%A at %X");
                }
              else
                {
                  /* Any other date */
                  date_format = (date_style == LUNAR_DATE_STYLE_SIMPLE) ? "%x" : _("%x at %X");
                }

              /* format the date string accordingly */
              return endo_strdup_strftime (date_format, &tfile);
            }
        }
      else if (date_style == LUNAR_DATE_STYLE_LONG)
        {
          /* use long, date(1)-like format string */
          return endo_strdup_strftime ("%c", &tfile);
        }
      else if (date_style == LUNAR_DATE_STYLE_YYYYMMDD)
        {
          return endo_strdup_strftime ("%Y-%m-%d %H:%M:%S", &tfile);
        }
      else if (date_style == LUNAR_DATE_STYLE_MMDDYYYY)
        {
          return endo_strdup_strftime ("%m-%d-%Y %H:%M:%S", &tfile);
        }
      else if (date_style == LUNAR_DATE_STYLE_DDMMYYYY)
        {
          return endo_strdup_strftime ("%d-%m-%Y %H:%M:%S", &tfile);
        }
      else /* if (date_style == LUNAR_DATE_STYLE_CUSTOM) */
        {
          if (date_custom_style == NULL)
            return g_strdup ("");

          /* use custom date formatting */
          return endo_strdup_strftime (date_custom_style, &tfile);
        }
    }
 ...
 **/
GdkScreen*
lunar_util_parse_parent (gpointer    parent,
 ...
 **/
time_t
lunar_util_time_from_rfc3339 (const gchar *date_string)
{
  struct tm tm;
...
gchar *
lunar_util_change_working_directory (const gchar *new_directory)
{
  gchar *old_directory;

  _lunar_return_val_if_fail (new_directory != NULL && *new_directory != '\0', NULL);
...
void
lunar_setup_display_cb (gpointer data)
{
