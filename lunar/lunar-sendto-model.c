...
static void lunar_sendto_model_finalize   (GObject                *object);
static void lunar_sendto_model_load       (LunarSendtoModel      *sendto_model);
static void lunar_sendto_model_event      (GFileMonitor           *monitor,
                                            GFile                  *file,
                                            GFile                  *other_file,
...
struct _LunarSendtoModelClass
{
  GObjectClass __parent__;
};

struct _LunarSendtoModel
{
  GObject __parent__;
  GList  *monitors;
  GList  *handlers;
...
G_DEFINE_TYPE (LunarSendtoModel, lunar_sendto_model, G_TYPE_OBJECT)



static void
lunar_sendto_model_class_init (LunarSendtoModelClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_sendto_model_finalize;
}



static void
lunar_sendto_model_init (LunarSendtoModel *sendto_model)
{
...
static void
lunar_sendto_model_finalize (GObject *object)
{
  LunarSendtoModel *sendto_model = LUNAR_SENDTO_MODEL (object);
  GList             *lp;
  ...
  (*G_OBJECT_CLASS (lunar_sendto_model_parent_class)->finalize) (object);
}
...
static void
lunar_sendto_model_load (LunarSendtoModel *sendto_model)
{
#ifdef HAVE_GIO_UNIX
  ...
  specs = expidus_resource_match (EXPIDUS_RESOURCE_DATA, "Lunar/sendto/*.desktop", TRUE);
  for (n = 0; specs[n] != NULL; ++n)
    {
      /* lookup the absolute path to the .desktop file */
      path = expidus_resource_lookup (EXPIDUS_RESOURCE_DATA, specs[n]);
...
static void
lunar_sendto_model_event (GFileMonitor     *monitor,
                           GFile            *file,
                           GFile            *other_file,
                           GFileMonitorEvent event_type,
                           gpointer          user_data)
{
  LunarSendtoModel *sendto_model = LUNAR_SENDTO_MODEL (user_data);
  ...
  lunar_sendto_model_load (sendto_model);
}



/**
 * lunar_sendto_model_get_default:
 *
 * Returns a reference to the default #LunarSendtoModel
 ...
 **/
LunarSendtoModel*
lunar_sendto_model_get_default (void)
{
  static LunarSendtoModel *sendto_model = NULL;

  if (G_UNLIKELY (sendto_model == NULL))
    {
      sendto_model = g_object_new (LUNAR_TYPE_SENDTO_MODEL, NULL);
...
/**
 * lunar_sendto_model_get_matching:
 * @sendto_model : a #LunarSendtoModel.
 * @files        : a #GList of #LunarFile<!---->s.
 *
 ...
 **/
GList*
lunar_sendto_model_get_matching (LunarSendtoModel *sendto_model,
                                  GList             *files)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_SENDTO_MODEL (sendto_model), NULL);
  ...
  if (G_UNLIKELY (sendto_model->monitors == NULL))
    {
      ...
      datadirs = expidus_resource_dirs (EXPIDUS_RESOURCE_DATA);
      for (n = 0; datadirs[n] != NULL; ++n)
        {
          /* determine the path to the sendto directory */
          dir = g_build_filename (datadirs[n], "Lunar", "sendto", NULL);
          ...
          if (G_LIKELY (monitor != NULL))
            {
              g_signal_connect (monitor, "changed", G_CALLBACK (lunar_sendto_model_event), sendto_model);
      ...
      if (!g_app_info_supports_uris (hp->data))
        {
          ...
          for (fp = files; fp != NULL; fp = fp->next)
            if (!lunar_file_is_local (fp->data))
              break;
          ...
          for (fp = files; fp != NULL; fp = fp->next)
            {
              ...
              for (n = 0; mime_types[n] != NULL; ++n)
                {
                  content_type = lunar_file_get_content_type (fp->data);
