...
static GList *
_tij_collect_nofollow (LunarJob *job,
                       ...
                       GError   **error)
{
  ...
  for (lp = base_file_list;
       err == NULL && lp != NULL && !endo_job_is_cancelled (ENDO_JOB (job));
       lp = lp->next)
    {
      /* try to scan the directory */
      child_file_list = lunar_io_scan_directory (job, lp->data,
                                                  G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                                  TRUE, unlinking, FALSE, &err);

      /* prepend the new files to the existing list */
      file_list = lunar_g_file_list_prepend (file_list, lp->data);
      file_list = g_list_concat (child_file_list, file_list);
    }

  /* check if we failed */
  if (err != NULL || endo_job_is_cancelled (ENDO_JOB (job)))
    {
...
static gboolean
_lunar_io_jobs_create (LunarJob  *job,
                        GArray     *param_values,
                        GError    **error)
{
  GFileOutputStream *stream;
  LunarJobResponse  response = LUNAR_JOB_RESPONSE_CANCEL;
  GFileInfo         *info;
  GError            *err = NULL;
  GList             *file_list;
  GList             *lp;
  ...
  _lunar_return_val_if_fail (param_values != NULL, FALSE);
  _lunar_return_val_if_fail (param_values->len == 2, FALSE);
  ...
  lunar_job_set_total_files (LUNAR_JOB (job), file_list);

  /* check if we need to open the template */
  if (template_file != NULL)
    {
      /* open read stream to feed in the new files */
      template_stream = g_file_read (template_file, endo_job_get_cancellable (ENDO_JOB (job)), &err);
  ...
  for (lp = file_list;
       ...
       lp = lp->next, n_processed++)
    {
      ...
      lunar_job_processing_file (LUNAR_JOB (job), lp, n_processed);
...
again:
      ...
      if (endo_job_is_cancelled (ENDO_JOB (job)))
        break;
          ...
          if (err->code == G_IO_ERROR_EXISTS)
            {
              ...
              response = lunar_job_ask_overwrite (LUNAR_JOB (job),
              ...
              if (response == LUNAR_JOB_RESPONSE_REPLACE)
                {
                  ...
                  if (_tij_delete_file (lp->data, endo_job_get_cancellable (ENDO_JOB (job)), &err))
                    goto again;
                }
          ...
          else
            {
              ...
              response = lunar_job_ask_skip (LUNAR_JOB (job),
              ...
              if (response == LUNAR_JOB_RESPONSE_RETRY)
                goto again;
            }
        }
      else
  ...
  lunar_job_new_files (LUNAR_JOB (job), file_list);
...
LunarJob *
lunar_io_jobs_create_files (GList *file_list,
                             GFile *template_file)
{
  return lunar_simple_job_launch (_lunar_io_jobs_create, 2,
                                   LUNAR_TYPE_G_FILE_LIST, file_list,
...
static gboolean
_lunar_io_jobs_mkdir (LunarJob  *job,
                       GArray     *param_values,
                       GError    **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_JOB (job), FALSE);
  _lunar_return_val_if_fail (param_values != NULL, FALSE);
  _lunar_return_val_if_fail (param_values->len == 1, FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
...
again:
      ...
      if (!g_file_make_directory (lp->data, endo_job_get_cancellable (ENDO_JOB (job)), &err))
        {
          if (err->code == G_IO_ERROR_EXISTS)
            {
              g_error_free (err);
...
LunarJob *
lunar_io_jobs_make_directories (GList *file_list)
{
  return lunar_simple_job_launch (_lunar_io_jobs_mkdir, 1,
                                   LUNAR_TYPE_G_FILE_LIST, file_list);
}
...
static gboolean
_lunar_io_jobs_unlink (LunarJob  *job,
                        GArray     *param_values,
                        GError    **error)
{
  ...
  endo_job_info_message (ENDO_JOB (job), _("Preparing..."));
  ...
  for (lp = file_list;
       lp != NULL && !endo_job_is_cancelled (ENDO_JOB (job));
       lp = lp->next, n_processed++)
    {
      g_assert (G_IS_FILE (lp->data));

      /* skip root folders which cannot be deleted anyway */
      if (lunar_g_file_is_root (lp->data))
        continue;
      ...
      if (_tij_delete_file (lp->data, endo_job_get_cancellable (ENDO_JOB (job)), &err))
        {
           ...
           * be deleted now */
          lunar_thumbnail_cache_delete_file (thumbnail_cache, lp->data);
        }
      else
        {
...
LunarJob *
lunar_io_jobs_unlink_files (GList *file_list)
{
  return lunar_simple_job_launch (_lunar_io_jobs_unlink, 1,
                                   LUNAR_TYPE_G_FILE_LIST, file_list);
}
...
LunarJob *
lunar_io_jobs_move_files (GList *source_file_list,
                           GList *target_file_list)
{
  LunarJob *job;

  _lunar_return_val_if_fail (source_file_list != NULL, NULL);
  _lunar_return_val_if_fail (target_file_list != NULL, NULL);
  _lunar_return_val_if_fail (g_list_length (source_file_list) == g_list_length (target_file_list), NULL);

  job = lunar_transfer_job_new (source_file_list, target_file_list,
                                 LUNAR_TRANSFER_JOB_MOVE);
  lunar_job_set_pausable (job, TRUE);

  return LUNAR_JOB (endo_job_launch (ENDO_JOB (job)));
}



LunarJob *
lunar_io_jobs_copy_files (GList *source_file_list,
                           GList *target_file_list)
{
  LunarJob *job;
...
static GFile *
_lunar_io_jobs_link_file (LunarJob *job,
                           GFile     *source_file,
                           GFile     *target_file,
                           GError   **error)
{
  ...
  while (err == NULL)
    {
      if (!g_file_equal (source_file, target_file))
      ...
      else
        {
          for (n = 1; err == NULL; ++n)
            {
              GFile *duplicate_file = lunar_io_jobs_util_next_duplicate_file (job,
                                                                               source_file,
                                                                               FALSE, n,
                                                                               &err);
      ...
      if (err->domain == G_IO_ERROR && err->code == G_IO_ERROR_EXISTS)
        {
          ...
          response = lunar_job_ask_overwrite (job, "%s", err->message);
          ...
          if (response == LUNAR_JOB_RESPONSE_REPLACE)
            {
              ...
              _tij_delete_file (target_file, endo_job_get_cancellable (ENDO_JOB (job)), &err);
            }
...
static gboolean
_lunar_io_jobs_link (LunarJob  *job,
                      GArray     *param_values,
                      GError    **error)
{
  LunarThumbnailCache *thumbnail_cache;
  ...
  lunar_job_set_total_files (LUNAR_JOB (job), source_file_list);
  ...
  for (sp = source_file_list, tp = target_file_list;
    ...
    {
      _lunar_assert (G_IS_FILE (sp->data));
      _lunar_assert (G_IS_FILE (tp->data));

      /* update progress information */
      lunar_job_processing_file (LUNAR_JOB (job), sp, n_processed);

      /* try to create the symbolic link */
      real_target_file = _lunar_io_jobs_link_file (job, sp->data, tp->data, &err);
      if (real_target_file != NULL)
        {
          /* queue the file for the folder update unless it was skipped */
          if (sp->data != real_target_file)
            {
              new_files_list = lunar_g_file_list_prepend (new_files_list,
                                                           real_target_file);

              /* notify the thumbnail cache that we need to copy the original
               * thumbnail for the symlink to have one too */
              lunar_thumbnail_cache_copy_file (thumbnail_cache, sp->data,
                                                real_target_file);
...
LunarJob *
lunar_io_jobs_link_files (GList *source_file_list,
                           GList *target_file_list)
{
  _lunar_return_val_if_fail (source_file_list != NULL, NULL);
  _lunar_return_val_if_fail (target_file_list != NULL, NULL);
  ...
  return lunar_simple_job_launch (_lunar_io_jobs_link, 2,
                                   LUNAR_TYPE_G_FILE_LIST, source_file_list,
                                   LUNAR_TYPE_G_FILE_LIST, target_file_list);
}



static gboolean
_lunar_io_jobs_trash (LunarJob  *job,
                       GArray     *param_values,
                       GError    **error)
{
  LunarThumbnailCache *thumbnail_cache;
  ...
  for (lp = file_list; err == NULL && lp != NULL; lp = lp->next)
    {
      _lunar_assert (G_IS_FILE (lp->data));

      /* trash the file or folder */
      g_file_trash (lp->data, endo_job_get_cancellable (ENDO_JOB (job)), &err);

      if (err != NULL)
        {
          response = lunar_job_ask_delete (job, "%s", err->message);

          g_clear_error (&err);

          if (response == LUNAR_JOB_RESPONSE_CANCEL)
            break;

          if (response == LUNAR_JOB_RESPONSE_YES)
            _tij_delete_file (lp->data, endo_job_get_cancellable (ENDO_JOB (job)), &err);
        }

      /* update the thumbnail cache */
      lunar_thumbnail_cache_cleanup_file (thumbnail_cache, lp->data);
    }
...
LunarJob *
lunar_io_jobs_trash_files (GList *file_list)
{
  _lunar_return_val_if_fail (file_list != NULL, NULL);

  return lunar_simple_job_launch (_lunar_io_jobs_trash, 1,
                                   LUNAR_TYPE_G_FILE_LIST, file_list);
}
...
LunarJob *
lunar_io_jobs_restore_files (GList *source_file_list,
                              GList *target_file_list)
{
  LunarJob *job;
...
static gboolean
_lunar_io_jobs_chown (LunarJob  *job,
                       GArray     *param_values,
                       GError    **error)
{
  LunarJobResponse response;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_JOB (job), FALSE);
  _lunar_return_val_if_fail (param_values != NULL, FALSE);
  _lunar_return_val_if_fail (param_values->len == 4, FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  _lunar_assert ((uid >= 0 || gid >= 0) && !(uid >= 0 && gid >= 0));
      ...
      if (err != NULL && !endo_job_is_cancelled (ENDO_JOB (job)))
        {
          ...
          response = lunar_job_ask_skip (LUNAR_JOB (job), message,
...
LunarJob *
lunar_io_jobs_change_group (GList    *files,
                             guint32   gid,
                             gboolean  recursive)
{
  _lunar_return_val_if_fail (files != NULL, NULL);

  /* files are released when the list if destroyed */
  g_list_foreach (files, (GFunc) (void (*)(void)) g_object_ref, NULL);

  return lunar_simple_job_launch (_lunar_io_jobs_chown, 4,
                                   LUNAR_TYPE_G_FILE_LIST, files,
                                   G_TYPE_INT, -1,
                                   G_TYPE_INT, (gint) gid,
...
static gboolean
_lunar_io_jobs_chmod (LunarJob  *job,
                       GArray     *param_values,
                       GError    **error)
{
  LunarJobResponse response;
  ...
  LunarFileMode    mask;
  LunarFileMode    mode;
  LunarFileMode    old_mode;
  LunarFileMode    new_mode;

  _lunar_return_val_if_fail (LUNAR_IS_JOB (job), FALSE);
  _lunar_return_val_if_fail (param_values != NULL, FALSE);
  _lunar_return_val_if_fail (param_values->len == 6, FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
...
retry_chown:
      ...
      if (err != NULL && !endo_job_is_cancelled (ENDO_JOB (job)))
        {
          /* ask the user whether to skip/retry this file */
          response = lunar_job_ask_skip (job,
...
LunarJob *
lunar_io_jobs_change_mode (GList         *files,
                            LunarFileMode dir_mask,
                            LunarFileMode dir_mode,
                            LunarFileMode file_mask,
                            LunarFileMode file_mode,
                            gboolean       recursive)
{
  ...
  return lunar_simple_job_launch (_lunar_io_jobs_chmod, 6,
                                   LUNAR_TYPE_G_FILE_LIST, files,
                                   LUNAR_TYPE_FILE_MODE, dir_mask,
                                   LUNAR_TYPE_FILE_MODE, dir_mode,
                                   LUNAR_TYPE_FILE_MODE, file_mask,
                                   LUNAR_TYPE_FILE_MODE, file_mode,
                                   G_TYPE_BOOLEAN, recursive);
}
...
static gboolean
_lunar_io_jobs_ls (LunarJob  *job,
                    GArray     *param_values,
                    GError    **error)
{
  GError *err = NULL;
  ...
  _lunar_assert (G_IS_FILE (directory));

  /* collect directory contents (non-recursively) */
  file_list = lunar_io_scan_directory (job, directory,
                                        G_FILE_QUERY_INFO_NONE,
  ...
  else if (endo_job_set_error_if_cancelled (ENDO_JOB (job), &err))
    {
      g_propagate_error (error, err);
      return FALSE;
    }
  ...
  if (G_LIKELY (file_list != NULL))
    {
      ...
      if (!lunar_job_files_ready (LUNAR_JOB (job), file_list))
        {
...
LunarJob *
lunar_io_jobs_list_directory (GFile *directory)
{
  _lunar_return_val_if_fail (G_IS_FILE (directory), NULL);

  return lunar_simple_job_launch (_lunar_io_jobs_ls, 1, G_TYPE_FILE, directory);
}



static gboolean
_lunar_io_jobs_rename_notify (gpointer user_data)
{
  LunarFile *file = user_data;

  /* tell the associated folder that the file was renamed */
  lunarx_file_info_renamed (LUNARX_FILE_INFO (file));
...
static gboolean
_lunar_io_jobs_rename (LunarJob  *job,
                        GArray     *param_values,
                        GError    **error)
{
  ...
  _lunar_return_val_if_fail (param_values != NULL, FALSE);
  _lunar_return_val_if_fail (param_values->len == 2, FALSE);
  _lunar_return_val_if_fail (G_VALUE_HOLDS (&g_array_index (param_values, GValue, 0), LUNAR_TYPE_FILE), FALSE);
  _lunar_return_val_if_fail (G_VALUE_HOLDS_STRING (&g_array_index (param_values, GValue, 1)), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (lunar_file_rename (file, display_name, endo_job_get_cancellable (ENDO_JOB (job)), TRUE, &err))
    {
      endo_job_send_to_mainloop (ENDO_JOB (job),
                                _lunar_io_jobs_rename_notify,
...
LunarJob *
lunar_io_jobs_rename_file (LunarFile  *file,
                            const gchar *display_name)
{
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), NULL);
  _lunar_return_val_if_fail (g_utf8_validate (display_name, -1, NULL), NULL);

  return lunar_simple_job_launch (_lunar_io_jobs_rename, 2,
                                   LUNAR_TYPE_FILE, file,
