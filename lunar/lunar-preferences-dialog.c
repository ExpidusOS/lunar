...
static void lunar_preferences_dialog_finalize   (GObject                      *object);
static void lunar_preferences_dialog_response   (GtkDialog                    *dialog,
                                                  gint                          response);
static void lunar_preferences_dialog_configure  (LunarPreferencesDialog     *dialog);



struct _LunarPreferencesDialogClass
{
  ExpidusTitledDialogClass __parent__;
};

struct _LunarPreferencesDialog
{
...
G_DEFINE_TYPE (LunarPreferencesDialog, lunar_preferences_dialog, EXPIDUS_TYPE_TITLED_DIALOG)
                              ...
                              gpointer      user_data)
{
  ...
  klass = g_type_class_ref (LUNAR_TYPE_ICON_SIZE);
                                ...
                                gpointer      user_data)
{
  ...
  if (type == LUNAR_TYPE_ICON_VIEW)
    g_value_set_int (dst_value, 0);
  else if (type == LUNAR_TYPE_DETAILS_VIEW)
    g_value_set_int (dst_value, 1);
  else if (type == LUNAR_TYPE_COMPACT_VIEW)
                                ...
                                gpointer      user_data)
{
  switch (g_value_get_int (src_value))
    {
    case 0:
      g_value_set_static_string (dst_value, g_type_name (LUNAR_TYPE_ICON_VIEW));
      break;

    case 1:
      g_value_set_static_string (dst_value, g_type_name (LUNAR_TYPE_DETAILS_VIEW));
      break;

    case 2:
      g_value_set_static_string (dst_value, g_type_name (LUNAR_TYPE_COMPACT_VIEW));
      break;
                                   ...
                                   gpointer      user_data)
{
  ...
  klass = g_type_class_ref (LUNAR_TYPE_THUMBNAIL_MODE);
  for (n = 0; n < klass->n_values; ++n)
    if (klass->values[n].value == g_value_get_enum (src_value))
      g_value_set_int (dst_value, n);
  g_type_class_unref (klass);
                                       ...
                                       gpointer      user_data)
{
  ...
  klass = g_type_class_ref (LUNAR_TYPE_PARALLEL_COPY_MODE);
  for (n = 0; n < klass->n_values; ++n)
    if (klass->values[n].value == g_value_get_enum (src_value))
      g_value_set_int (dst_value, n);
  g_type_class_unref (klass);
...
static void
lunar_preferences_dialog_class_init (LunarPreferencesDialogClass *klass)
{
  GtkDialogClass *gtkdialog_class;
  GObjectClass   *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_preferences_dialog_finalize;

  gtkdialog_class = GTK_DIALOG_CLASS (klass);
  gtkdialog_class->response = lunar_preferences_dialog_response;
}
                        ...
                        GtkWidget *customFormat)
{
  ...
  _lunar_return_if_fail (GTK_IS_COMBO_BOX (combobox));
  _lunar_return_if_fail (GTK_IS_WIDGET (customFormat));

  if (gtk_combo_box_get_active (combobox) == LUNAR_DATE_STYLE_CUSTOM)
...
static void
lunar_preferences_dialog_init (LunarPreferencesDialog *dialog)
{
  ...
  gtk_window_set_icon_name (GTK_WINDOW (dialog), "com.expidus.lunar");
  gtk_window_set_resizable (GTK_WINDOW (dialog), FALSE);
  gtk_window_set_title (GTK_WINDOW (dialog), _("File Manager Preferences"));

#if LIBEXPIDUS1UI_CHECK_VERSION (4, 15, 1)
...
#if LIBEXPIDUS1UI_CHECK_VERSION (4, 15, 1)
  expidus_titled_dialog_add_action_widget (EXPIDUS_TITLED_DIALOG (dialog), button, GTK_RESPONSE_CLOSE);
#else
  ...
  endo_mutual_binding_new_full (G_OBJECT (dialog->preferences), "default-view", G_OBJECT (combo), "active",
                               transform_view_string_to_index, transform_view_index_to_string, NULL, NULL);
  gtk_widget_set_hexpand (combo, TRUE);
  gtk_grid_attach (GTK_GRID (grid), combo, 1, 0, 1, 1);
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label), combo);
  ...
  endo_mutual_binding_new_full (G_OBJECT (dialog->preferences), "misc-thumbnail-mode", G_OBJECT (combo), "active",
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-directory-specific-settings", G_OBJECT (button), "active");
  gtk_widget_set_tooltip_text (button,
                               _("Select this option to remember view type, sort column, and sort order individually for each folder"));
  gtk_widget_set_hexpand (button, TRUE);
  gtk_grid_attach (GTK_GRID (grid), button, 0, 2, 1, 1);
  gtk_widget_show (button);
  if (!lunar_g_vfs_metadata_is_supported ())
    {
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-thumbnail-draw-frames", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-folders-first", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-file-size-binary", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-text-beside-icons", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-change-window-icon", G_OBJECT (button), "active");
  ...
  combo = gtk_combo_box_text_new ();
  for (date_style = LUNAR_DATE_STYLE_SIMPLE; date_style <= LUNAR_DATE_STYLE_DDMMYYYY; ++date_style)
    {
      date = lunar_util_humanize_file_time (time (NULL), date_style, NULL);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-date-style", G_OBJECT (combo), "active");
  gtk_widget_set_hexpand (combo, TRUE);
  gtk_grid_attach (GTK_GRID (grid), combo, 1, 0, 1, 1);
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label), combo);
  gtk_label_set_mnemonic_widget (GTK_LABEL (label), combo);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-date-custom-style", G_OBJECT (entry), "text");
  g_signal_connect (G_OBJECT (combo), "changed", G_CALLBACK (on_date_format_changed), entry);
  gtk_grid_attach (GTK_GRID (grid), entry, 1, 1, 1, 1);
  if (gtk_combo_box_get_active (GTK_COMBO_BOX (combo)) == LUNAR_DATE_STYLE_CUSTOM)
  ...
  endo_mutual_binding_new_full (G_OBJECT (dialog->preferences), "shortcuts-icon-size", G_OBJECT (combo), "active",
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "shortcuts-icon-emblems", G_OBJECT (button), "active");
  ...
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _("192px"));
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _("256px"));
  endo_mutual_binding_new_full (G_OBJECT (dialog->preferences), "tree-icon-size", G_OBJECT (combo), "active",
                               transform_icon_size_to_index, transform_index_to_icon_size, NULL, NULL);
  gtk_widget_set_hexpand (combo, TRUE);
  gtk_grid_attach (GTK_GRID (grid), combo, 1, 0, 1, 1);
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label), combo);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "tree-icon-emblems", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-single-click", G_OBJECT (button), "active");
  ...
  endo_binding_new (G_OBJECT (button), "active", G_OBJECT (ibox), "sensitive");
  gtk_widget_set_hexpand (ibox, TRUE);
  ...
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label), range);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-single-click-timeout", G_OBJECT (adjustment), "value");
  ...
  endo_mutual_binding_new_with_negation (G_OBJECT (dialog->preferences), "misc-single-click", G_OBJECT (button), "active");
  g_signal_connect (G_OBJECT (button), "toggled", G_CALLBACK (g_object_notify), "active");
  gtk_widget_set_hexpand (button, TRUE);
  gtk_grid_attach (GTK_GRID (grid), button, 0, 2, 1, 1);
  gtk_widget_show (button);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-middle-click-in-tab", G_OBJECT (button), "active");
  gtk_widget_set_tooltip_text (button, _("Select this option to open a new tab on middle click instead of a new window"));
  gtk_grid_attach (GTK_GRID (grid), button, 0, 0, 1, 1);
  gtk_widget_show (button);

  button = gtk_check_button_new_with_mnemonic (_("Open new lunar instances as tabs"));
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-open-new-window-as-tab", G_OBJECT (button), "active");
  gtk_widget_set_tooltip_text (button, _("Select this option to open new lunar instances as tabs in an existing lunar window"));
  gtk_widget_set_hexpand (button, TRUE);
  ...
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _("Never"));
  endo_mutual_binding_new_full (G_OBJECT (dialog->preferences), "misc-parallel-copy-mode", G_OBJECT (combo), "active",
  ...
  if (lunar_g_vfs_is_uri_scheme_supported ("trash"))
    {
      ...
      endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-show-delete-action", G_OBJECT (button), "active");
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-recursive-permissions", G_OBJECT (combo), "active");
  gtk_widget_set_hexpand (combo, TRUE);
  ...
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-exec-shell-scripts-by-default", G_OBJECT (button), "active");
  ...
  /* check if "lunar-volman" is found */
  path = g_find_program_in_path ("lunar-volman");

  /* add check button to enable/disable auto mounting */
  button = gtk_check_button_new_with_mnemonic (_("Enable _Volume Management"));
  endo_mutual_binding_new (G_OBJECT (dialog->preferences), "misc-volume-management", G_OBJECT (button), "active");
  gtk_widget_set_hexpand (button, TRUE);
  gtk_grid_attach (GTK_GRID (grid), button, 0, 0, 1, 1);
  gtk_widget_show (button);
  ...
  gtk_label_set_xalign (GTK_LABEL (label), 0.0f);
  endo_binding_new (G_OBJECT (button), "active", G_OBJECT (label), "sensitive");
  g_signal_connect_swapped (G_OBJECT (label), "activate-link", G_CALLBACK (lunar_preferences_dialog_configure), dialog);
  ...
  if (!lunar_g_vfs_is_uri_scheme_supported ("trash")    ||
      !lunar_g_vfs_is_uri_scheme_supported ("computer") || /* support for removable media */
      !lunar_g_vfs_is_uri_scheme_supported ("sftp") )
    {
      frame = g_object_new (GTK_TYPE_FRAME, "border-width", 0, "shadow-type", GTK_SHADOW_NONE, NULL);
      gtk_box_pack_start (GTK_BOX (vbox), frame, FALSE, TRUE, 0);
      gtk_widget_show (frame);
      ...
      gtk_label_set_markup (GTK_LABEL (label), _("It looks like <a href=\"https://wiki.gnome.org/Projects/gvfs\">gvfs</a> is not available.\n"
                                                 ...
                                                 "will not work. <a href=\"https://docs.expidus.com/expidus/lunar/unix-filesystem#gnome_virtual_file_system\">[Read more]</a>"));
...
static void
lunar_preferences_dialog_finalize (GObject *object)
{
  LunarPreferencesDialog *dialog = LUNAR_PREFERENCES_DIALOG (object);

  /* release our reference on the preferences */
  g_object_unref (G_OBJECT (dialog->preferences));

  (*G_OBJECT_CLASS (lunar_preferences_dialog_parent_class)->finalize) (object);
}



static void
lunar_preferences_dialog_response (GtkDialog *dialog,
                                    gint       response)
{
...
static void
lunar_preferences_dialog_configure (LunarPreferencesDialog *dialog)
{
  GError    *err = NULL;
  gchar     *argv[3];
  GdkScreen *screen;
  ...
  _lunar_return_if_fail (LUNAR_IS_PREFERENCES_DIALOG (dialog));

  /* prepare the argument vector */
  argv[0] = (gchar *) "lunar-volman";
  ...
  /* invoke the configuration interface of lunar-volman */
  if (!g_spawn_async (NULL, argv, NULL, G_SPAWN_SEARCH_PATH, lunar_setup_display_cb, display, NULL, &err))
    {
      /* tell the user that we failed to come up with the lunar-volman configuration dialog */
      lunar_dialogs_show_error (dialog, err, _("Failed to display the volume management settings"));
      g_error_free (err);
    }
...
/**
 * lunar_preferences_dialog_new:
 * @parent : a #GtkWindow or %NULL.
 *
 * Allocates a new #LunarPreferencesDialog widget.
 *
 * Return value: the newly allocated #LunarPreferencesDialog.
 **/
GtkWidget*
lunar_preferences_dialog_new (GtkWindow *parent)
{
  GtkWidget    *dialog;
  gint          root_x, root_y;
  ...
  dialog = g_object_new (LUNAR_TYPE_PREFERENCES_DIALOG, NULL);
