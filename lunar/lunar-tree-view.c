...
#include <lunar/lunar-shortcuts-icon-renderer.h>
...
#define LUNAR_TREE_VIEW_EXPAND_TIMEOUT (750)
...
static void                     lunar_tree_view_navigator_init               (LunarNavigatorIface    *iface);
static void                     lunar_tree_view_finalize                     (GObject                 *object);
static void                     lunar_tree_view_get_property                 (GObject                 *object,
                                                                               guint                    prop_id,
                                                                               GValue                  *value,
                                                                               GParamSpec              *pspec);
static void                     lunar_tree_view_set_property                 (GObject                 *object,
                                                                               guint                    prop_id,
                                                                               const GValue            *value,
                                                                               GParamSpec              *pspec);
static LunarFile              *lunar_tree_view_get_current_directory        (LunarNavigator         *navigator);
static void                     lunar_tree_view_set_current_directory        (LunarNavigator         *navigator,
                                                                               LunarFile              *current_directory);
static void                     lunar_tree_view_realize                      (GtkWidget               *widget);
static void                     lunar_tree_view_unrealize                    (GtkWidget               *widget);
static gboolean                 lunar_tree_view_button_press_event           (GtkWidget               *widget,
                                                                               GdkEventButton          *event);
static gboolean                 lunar_tree_view_button_release_event         (GtkWidget               *widget,
                                                                               GdkEventButton          *event);
static gboolean                 lunar_tree_view_key_press_event              (GtkWidget               *widget,
                                                                               GdkEventKey             *event);
static void                     lunar_tree_view_drag_data_received           (GtkWidget               *widget,
                                                                               ...
                                                                               guint                    info,
                                                                               guint                    time);
static gboolean                 lunar_tree_view_drag_drop                    (GtkWidget               *widget,
                                                                               GdkDragContext          *context,
                                                                               gint                     x,
                                                                               gint                     y,
                                                                               guint                    time);
static gboolean                 lunar_tree_view_drag_motion                  (GtkWidget               *widget,
                                                                               GdkDragContext          *context,
                                                                               gint                     x,
                                                                               gint                     y,
                                                                               guint                    time);
static void                     lunar_tree_view_drag_leave                   (GtkWidget               *widget,
                                                                               GdkDragContext          *context,
                                                                               guint                    time);
static gboolean                 lunar_tree_view_popup_menu                   (GtkWidget               *widget);
static void                     lunar_tree_view_row_activated                (GtkTreeView             *tree_view,
                                                                               GtkTreePath             *path,
                                                                               GtkTreeViewColumn       *column);
static gboolean                 lunar_tree_view_test_expand_row              (GtkTreeView             *tree_view,
                                                                               GtkTreeIter             *iter,
                                                                               GtkTreePath             *path);
static void                     lunar_tree_view_row_collapsed                (GtkTreeView             *tree_view,
                                                                               GtkTreeIter             *iter,
                                                                               GtkTreePath             *path);
static gboolean                 lunar_tree_view_delete_selected_files        (LunarTreeView          *view);
static void                     lunar_tree_view_context_menu                 (LunarTreeView          *view,
                                                                               GtkTreeModel            *model,
                                                                               GtkTreeIter             *iter);
static GdkDragAction            lunar_tree_view_get_dest_actions             (LunarTreeView          *view,
                                                                               GdkDragContext          *context,
                                                                               gint                     x,
                                                                               gint                     y,
                                                                               guint                    time,
                                                                               LunarFile             **file_return);
static LunarFile              *lunar_tree_view_get_selected_file            (LunarTreeView          *view);
static LunarDevice            *lunar_tree_view_get_selected_device          (LunarTreeView          *view);
static void                     lunar_tree_view_action_unlink_selected_folder(LunarTreeView          *view,
                                                                               gboolean                 permanently);
static void                     lunar_tree_view_action_move_to_trash         (LunarTreeView          *view);
static void                     lunar_tree_view_action_delete                (LunarTreeView          *view);
static void                     lunar_tree_view_action_open                  (LunarTreeView          *view);
static void                     lunar_tree_view_open_selection               (LunarTreeView          *view);
static void                     lunar_tree_view_select_files                 (LunarTreeView          *view,
                                                                               GList                   *files_to_selected);
static gboolean                 lunar_tree_view_visible_func                 (LunarTreeModel         *model,
                                                                               LunarFile              *file,
                                                                               gpointer                 user_data);
static gboolean                 lunar_tree_view_selection_func               (GtkTreeSelection        *selection,
                                                                               GtkTreeModel            *model,
                                                                               GtkTreePath             *path,
                                                                               gboolean                 path_currently_selected,
                                                                               gpointer                 user_data);
static gboolean                 lunar_tree_view_cursor_idle                  (gpointer                 user_data);
static void                     lunar_tree_view_cursor_idle_destroy          (gpointer                 user_data);
static gboolean                 lunar_tree_view_drag_scroll_timer            (gpointer                 user_data);
static void                     lunar_tree_view_drag_scroll_timer_destroy    (gpointer                 user_data);
static gboolean                 lunar_tree_view_expand_timer                 (gpointer                 user_data);
static void                     lunar_tree_view_expand_timer_destroy         (gpointer                 user_data);
static gboolean                 lunar_tree_view_get_show_hidden              (LunarTreeView          *view);
static void                     lunar_tree_view_set_show_hidden              (LunarTreeView          *view,
                                                                               gboolean                 show_hidden);
static GtkTreePath             *lunar_tree_view_get_preferred_toplevel_path  (LunarTreeView          *view,
                                                                               LunarFile              *file);



struct _LunarTreeViewClass
{
  GtkTreeViewClass __parent__;

  /* signals */
  gboolean (*delete_selected_files) (LunarTreeView *view);
};

struct _LunarTreeView
{
...
G_DEFINE_TYPE_WITH_CODE (LunarTreeView, lunar_tree_view, GTK_TYPE_TREE_VIEW,
    G_IMPLEMENT_INTERFACE (LUNAR_TYPE_NAVIGATOR, lunar_tree_view_navigator_init))



static void
lunar_tree_view_class_init (LunarTreeViewClass *klass)
{
  ...
  gobject_class->finalize = lunar_tree_view_finalize;
  gobject_class->get_property = lunar_tree_view_get_property;
  gobject_class->set_property = lunar_tree_view_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->realize = lunar_tree_view_realize;
  gtkwidget_class->unrealize = lunar_tree_view_unrealize;
  gtkwidget_class->button_press_event = lunar_tree_view_button_press_event;
  gtkwidget_class->button_release_event = lunar_tree_view_button_release_event;
  gtkwidget_class->drag_data_received = lunar_tree_view_drag_data_received;
  gtkwidget_class->drag_drop = lunar_tree_view_drag_drop;
  gtkwidget_class->drag_motion = lunar_tree_view_drag_motion;
  gtkwidget_class->drag_leave = lunar_tree_view_drag_leave;
  gtkwidget_class->popup_menu = lunar_tree_view_popup_menu;

  gtktree_view_class = GTK_TREE_VIEW_CLASS (klass);
  gtktree_view_class->row_activated = lunar_tree_view_row_activated;
  gtktree_view_class->test_expand_row = lunar_tree_view_test_expand_row;
  gtktree_view_class->row_collapsed = lunar_tree_view_row_collapsed;

  klass->delete_selected_files = lunar_tree_view_delete_selected_files;

  /* Override LunarNavigator's properties */
  g_object_class_override_property (gobject_class, PROP_CURRENT_DIRECTORY, "current-directory");

  /**
   * LunarTreeView:show-hidden:
   *
   * Whether to display hidden and backup folders
   * in this #LunarTreeView.
   **/
  ...
  /**
   * LunarTreeView::delete-selected-files:
   * @tree_view : a #LunarTreeView.
   *
  ...
  tree_view_signals[DELETE_SELECTED_FILES] =
    g_signal_new (I_("delete-selected-files"),
                  ...
                  G_STRUCT_OFFSET (LunarTreeViewClass, delete_selected_files),
...
static void
lunar_tree_view_navigator_init (LunarNavigatorIface *iface)
{
  iface->get_current_directory = lunar_tree_view_get_current_directory;
  iface->set_current_directory = lunar_tree_view_set_current_directory;
}



static void
lunar_tree_view_init (LunarTreeView *view)
{
  GtkTreeViewColumn *column;
  ...
  view->model = g_object_new (LUNAR_TYPE_TREE_MODEL, NULL);

  /* synchronize the the global "misc-case-sensitive" preference */
  g_object_set_data_full (G_OBJECT (view->model), I_("lunar-preferences"), view->preferences, g_object_unref);
  endo_binding_new (G_OBJECT (view->preferences), "misc-case-sensitive", G_OBJECT (view->model), "case-sensitive");

  lunar_tree_model_set_visible_func (view->model, lunar_tree_view_visible_func, view);
  ...
  view->icon_renderer = lunar_shortcuts_icon_renderer_new ();
  ...
  gtk_tree_view_column_set_attributes (column, view->icon_renderer,
                                       "file", LUNAR_TREE_MODEL_COLUMN_FILE,
                                       "device", LUNAR_TREE_MODEL_COLUMN_DEVICE,
                                       NULL);
   ...
   */
  endo_binding_new (G_OBJECT (view->preferences), "tree-icon-size", G_OBJECT (view->icon_renderer), "size");
  endo_binding_new (G_OBJECT (view->preferences), "tree-icon-emblems", G_OBJECT (view->icon_renderer), "emblems");

  /* allocate the text renderer */
  renderer = gtk_cell_renderer_text_new ();
  gtk_tree_view_column_pack_start (column, renderer, TRUE);
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "attributes", LUNAR_TREE_MODEL_COLUMN_ATTR,
                                       "text", LUNAR_TREE_MODEL_COLUMN_NAME,
                                       NULL);

  /* setup the tree selection */
  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (view));
  gtk_tree_selection_set_mode (selection, GTK_SELECTION_SINGLE);
  gtk_tree_selection_set_select_function (selection, lunar_tree_view_selection_func, view, NULL);

  /* custom keyboard handler for better navigation */
  g_signal_connect (GTK_WIDGET (view), "key_press_event", G_CALLBACK (lunar_tree_view_key_press_event), NULL);

  /* enable drop support for the tree view */
  gtk_drag_dest_set (GTK_WIDGET (view), 0, drop_targets, G_N_ELEMENTS (drop_targets),
                     GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_MOVE);

  view->new_files_closure = g_cclosure_new_swap (G_CALLBACK (lunar_tree_view_select_files), view, NULL);
  g_closure_ref (view->new_files_closure);
  g_closure_sink (view->new_files_closure);

  view->launcher =  g_object_new (LUNAR_TYPE_LAUNCHER, "widget", GTK_WIDGET (view),
                                                        "select-files-closure", view->new_files_closure, NULL);

  g_signal_connect_swapped (G_OBJECT (view->launcher), "change-directory", G_CALLBACK (lunar_tree_view_action_open), view);
  g_signal_connect_swapped (G_OBJECT (view->launcher), "open-new-tab", G_CALLBACK (lunar_navigator_open_new_tab), view);
  endo_binding_new (G_OBJECT (view), "current-directory", G_OBJECT (view->launcher), "current-directory");
}



static void
lunar_tree_view_finalize (GObject *object)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (object);

  /* release drop path list (if drag_leave wasn't called) */
  lunar_g_file_list_free (view->drop_file_list);
  ...
  lunar_navigator_set_current_directory (LUNAR_NAVIGATOR (view), NULL);
  ...
  (*G_OBJECT_CLASS (lunar_tree_view_parent_class)->finalize) (object);
}



static void
lunar_tree_view_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  switch (prop_id)
    ...
    case PROP_SHOW_HIDDEN:
      g_value_set_boolean (value, lunar_tree_view_get_show_hidden (LUNAR_TREE_VIEW (object)));
      break;
...
static void
lunar_tree_view_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
  switch (prop_id)
    ...
    case PROP_SHOW_HIDDEN:
      lunar_tree_view_set_show_hidden (LUNAR_TREE_VIEW (object), g_value_get_boolean (value));
      break;
...
static LunarFile*
lunar_tree_view_get_current_directory (LunarNavigator *navigator)
{
  return LUNAR_TREE_VIEW (navigator)->current_directory;
}



static void
lunar_tree_view_set_current_directory (LunarNavigator *navigator,
                                        LunarFile      *current_directory)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (navigator);
  LunarFile     *file;
  LunarFile     *file_parent;
      ...
      if (!view->show_hidden)
        {
          ...
          for (file = LUNAR_FILE (g_object_ref (G_OBJECT (view->current_directory))); file != NULL; file = file_parent)
            {
              ...
              /* get the file parent */
              file_parent = lunar_file_get_parent (file, NULL);
  ...
  if (G_LIKELY (current_directory != NULL))
    {
      ...
      if (!needs_refiltering && !view->show_hidden)
        {
          /* look if the file or one of it's parents is hidden */
          for (file = LUNAR_FILE (g_object_ref (G_OBJECT (current_directory))); file != NULL; file = file_parent)
            {
              /* check if this file is hidden */
              if (lunar_file_is_hidden (file))
                {
                  /* update the filter */
                  lunar_tree_model_refilter (view->model);
      ...
      if (G_LIKELY (view->cursor_idle_id == 0))
        view->cursor_idle_id = g_idle_add_full (G_PRIORITY_LOW, lunar_tree_view_cursor_idle, view, lunar_tree_view_cursor_idle_destroy);
    }
...
static void
lunar_tree_view_realize (GtkWidget *widget)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (widget);
  GdkDisplay     *display;

  /* let the parent class realize the widget */
  (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->realize) (widget);

  /* query the clipboard manager for the display */
  display = gtk_widget_get_display (widget);
  view->clipboard = lunar_clipboard_manager_get_for_display (display);
}



static void
lunar_tree_view_unrealize (GtkWidget *widget)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (widget);
  ...
  (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->unrealize) (widget);
}



static gboolean
lunar_tree_view_button_press_event (GtkWidget      *widget,
                                     GdkEventButton *event)
{
  LunarTreeView    *view = LUNAR_TREE_VIEW (widget);
  LunarDevice      *device;
  ...
  result = (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->button_press_event) (widget, event);
          ...
          if (gtk_tree_model_get_iter (GTK_TREE_MODEL (view->model), &iter, path))
            {
              ...
              lunar_tree_view_context_menu (view, GTK_TREE_MODEL (view->model), &iter);
      ...
      else if (event->button == 2)
        {
          ...
          if (path != NULL && gtk_tree_model_get_iter (GTK_TREE_MODEL (view->model), &iter, path))
            gtk_tree_model_get (GTK_TREE_MODEL (view->model), &iter,
                                LUNAR_TREE_MODEL_COLUMN_FILE, &file,
                                LUNAR_TREE_MODEL_COLUMN_DEVICE, &device, -1);

          if ((device != NULL && lunar_device_is_mounted (device)) ||
              (file != NULL && lunar_file_is_mounted (file)))
            {
              view->pressed_button = event->button;
            }
          else
...
static gboolean
lunar_tree_view_button_release_event (GtkWidget      *widget,
                                       GdkEventButton *event)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (widget);
  gboolean        in_tab;
  ...
  if (G_LIKELY (view->pressed_button == (gint) event->button))
    {
      ...
      if (G_LIKELY (event->button == 1))
        {
          lunar_tree_view_action_open (view);
        }
      else if (G_UNLIKELY (event->button == 2))
        {
          ...
          lunar_launcher_open_selected_folders (view->launcher, in_tab);
  ...
  return (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->button_release_event) (widget, event);
}



static gboolean
lunar_tree_view_key_press_event(GtkWidget   *widget,
                                 GdkEventKey *event)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (widget);
  LunarDevice   *device = NULL;
  GtkTreePath    *path;
  GtkTreeIter     iter;
  ...
  switch (event->keyval)
    ...
    case GDK_KEY_KP_Down:
      ...
      GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->key_press_event (widget, event);

      /* sync with new tree view selection */
      gtk_tree_path_free (path);
      gtk_tree_view_get_cursor (GTK_TREE_VIEW (view), &path, NULL);
      lunar_tree_view_open_selection (view);
      ...
      if (gtk_tree_view_row_expanded (GTK_TREE_VIEW (view), path))
        ...
        if (gtk_tree_path_get_depth (path) > 1 && gtk_tree_path_up (path))
        ...
        else if (gtk_tree_path_get_depth (path) == 1)
          {
            ...
            if (device != NULL)
              if (lunar_device_is_mounted (device) && lunar_device_can_unmount (device))
                {
                  ...
                  lunar_launcher_action_unmount (view->launcher);
    ...
    case GDK_KEY_KP_Right:
      ...
      if (gtk_tree_model_get_iter (GTK_TREE_MODEL (view->model), &iter, path))
        gtk_tree_model_get (GTK_TREE_MODEL (view->model), &iter,
                            LUNAR_TREE_MODEL_COLUMN_DEVICE, &device, -1);
     if (device != NULL && lunar_device_is_mounted (device) == FALSE)
       {
         g_object_set (G_OBJECT (view->launcher), "selected-device", device, NULL);
         g_object_set (G_OBJECT (view->launcher), "selected-files", NULL, "current-directory", NULL, NULL);
         lunar_launcher_action_mount (view->launcher);
       }
     else
       {
...
static void
lunar_tree_view_drag_data_received (GtkWidget        *widget,
                                     GdkDragContext   *context,
                                     gint              x,
                                     gint              y,
                                     GtkSelectionData *selection_data,
  ...
  if (G_LIKELY (!view->drop_data_ready))
    {
      ...
      if (info == TARGET_TEXT_URI_LIST && gtk_selection_data_get_format (selection_data) == 8 && gtk_selection_data_get_length (selection_data) > 0)
        view->drop_file_list = lunar_g_file_list_new_from_string ((const gchar *) gtk_selection_data_get_data (selection_data));
  ...
  if (G_UNLIKELY (view->drop_occurred))
    {
      ...
      actions = lunar_tree_view_get_dest_actions (view, context, x, y, timestamp, &file);
      if (G_LIKELY ((actions & (GDK_ACTION_COPY | GDK_ACTION_MOVE | GDK_ACTION_LINK)) != 0))
        {
          ...
          action = (gdk_drag_context_get_selected_action (context) == GDK_ACTION_ASK)
                 ? lunar_dnd_ask (GTK_WIDGET (view), file, view->drop_file_list, actions)
                 : gdk_drag_context_get_selected_action (context);

          /* perform the requested action */
          if (G_LIKELY (action != 0))
            succeed = lunar_dnd_perform (GTK_WIDGET (view), file, view->drop_file_list, action, NULL);
        }
      ...
      lunar_tree_view_drag_leave (GTK_WIDGET (view), context, timestamp);
    }
}
...
static gboolean
lunar_tree_view_drag_drop (GtkWidget      *widget,
                            GdkDragContext *context,
                            gint            x,
                            gint            y,
                            guint           timestamp)
...
static gboolean
lunar_tree_view_drag_motion (GtkWidget      *widget,
                              GdkDragContext *context,
                              gint            x,
                              gint            y,
                              guint           timestamp)
{
  ...
  if (G_UNLIKELY (!view->drop_data_ready))
  ...
  else
    {
      ...
      lunar_tree_view_get_dest_actions (view, context, x, y, timestamp, NULL);
    }
  ...
  if (G_UNLIKELY (view->drag_scroll_timer_id == 0))
    {
      ...
      view->drag_scroll_timer_id = g_timeout_add_full (G_PRIORITY_LOW, 50, lunar_tree_view_drag_scroll_timer,
                                                       view, lunar_tree_view_drag_scroll_timer_destroy);
    }
...
static void
lunar_tree_view_drag_leave (GtkWidget      *widget,
                             GdkDragContext *context,
                             guint           timestamp)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (widget);
  ...
  (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->drag_leave) (widget, context, timestamp);
}



static gboolean
lunar_tree_view_popup_menu (GtkWidget *widget)
{
  GtkTreeSelection *selection;
  LunarTreeView   *view = LUNAR_TREE_VIEW (widget);
  GtkTreeModel     *model;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      /* popup the context menu */
      lunar_tree_view_context_menu (view, model, &iter);
      return TRUE;
    }
  else if (GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->popup_menu != NULL)
    {
      /* call the parent's "popup-menu" handler */
      return (*GTK_WIDGET_CLASS (lunar_tree_view_parent_class)->popup_menu) (widget);
    }
...
static void
lunar_tree_view_row_activated (GtkTreeView       *tree_view,
                                GtkTreePath       *path,
                                GtkTreeViewColumn *column)
{
  /* call the parent's "row-activated" handler */
  if (GTK_TREE_VIEW_CLASS (lunar_tree_view_parent_class)->row_activated != NULL)
    (*GTK_TREE_VIEW_CLASS (lunar_tree_view_parent_class)->row_activated) (tree_view, path, column);
      ...
      if (gtk_tree_view_expand_row (tree_view, path, FALSE))
        {
          ...
          lunar_tree_view_action_open (LUNAR_TREE_VIEW (tree_view));
        }
    }
}
...
static gboolean
lunar_tree_view_test_expand_row (GtkTreeView *tree_view,
                                  GtkTreeIter *iter,
                                  GtkTreePath *path)
{
  LunarTreeView          *view = LUNAR_TREE_VIEW (tree_view);
  gboolean                 expandable = TRUE;
  LunarDevice            *device;

  /* determine the device for the iterator */
  gtk_tree_model_get (GTK_TREE_MODEL (view->model), iter, LUNAR_TREE_MODEL_COLUMN_DEVICE, &device, -1);
...
static void
lunar_tree_view_row_collapsed (GtkTreeView *tree_view,
                                GtkTreeIter *iter,
                                GtkTreePath *path)
{
  /* schedule a cleanup of the tree model */
  lunar_tree_model_cleanup (LUNAR_TREE_VIEW (tree_view)->model);
}



static gboolean
lunar_tree_view_delete_selected_files (LunarTreeView *view)
{
  GtkAccelKey     key;
  GdkModifierType state;

  _lunar_return_val_if_fail (LUNAR_IS_TREE_VIEW (view), FALSE);
  ...
  if (gtk_accel_map_lookup_entry ("<Actions>/LunarStandardView/move-to-trash", &key)
  ...
  if ((gtk_get_current_event_state (&state) && (state & GDK_SHIFT_MASK) != 0) ||
      (gtk_get_current_event_state (&state) && !lunar_g_vfs_is_uri_scheme_supported ("trash")))
    lunar_tree_view_action_delete (view);
  else
    lunar_tree_view_action_move_to_trash (view);
...
static void
lunar_tree_view_context_menu (LunarTreeView *view,
                               GtkTreeModel   *model,
                               GtkTreeIter    *iter)
{
  LunarDevice *device;
  ...
  gtk_tree_model_get (model, iter,
                      LUNAR_TREE_MODEL_COLUMN_FILE, &file,
                      LUNAR_TREE_MODEL_COLUMN_DEVICE, &device,
                      -1);

  context_menu = g_object_new (LUNAR_TYPE_MENU, "menu-type", LUNAR_MENU_TYPE_CONTEXT_TREE_VIEW,
                                                 "launcher", view->launcher,
                                                 "force-section-open", TRUE, NULL);

  g_object_set (G_OBJECT (view->launcher), "selected-device", device, NULL);

  file_is_available = (device == NULL || lunar_device_is_mounted (device));
  if (file_is_available)
    {
      ...
      if(lunar_g_file_is_trash    (lunar_file_get_file (file)) ||
         lunar_g_file_is_computer (lunar_file_get_file (file)) ||
         lunar_g_file_is_network  (lunar_file_get_file (file)))
        {
          lunar_launcher_append_menu_item (view->launcher, GTK_MENU_SHELL (context_menu), LUNAR_LAUNCHER_ACTION_OPEN, TRUE);
          lunar_launcher_append_menu_item (view->launcher, GTK_MENU_SHELL (context_menu), LUNAR_LAUNCHER_ACTION_OPEN_IN_TAB, TRUE);
          lunar_launcher_append_menu_item (view->launcher, GTK_MENU_SHELL (context_menu), LUNAR_LAUNCHER_ACTION_OPEN_IN_WINDOW, TRUE);
          expidus_gtk_menu_append_seperator (GTK_MENU_SHELL (context_menu));
          lunar_menu_add_sections (context_menu, LUNAR_MENU_SECTION_EMPTY_TRASH);
        }
      else
        {
          lunar_menu_add_sections (context_menu, LUNAR_MENU_SECTION_OPEN);
          lunar_menu_add_sections (context_menu, LUNAR_MENU_SECTION_SENDTO
  ...
  lunar_menu_add_sections (context_menu, LUNAR_MENU_SECTION_MOUNTABLE);
  lunar_menu_add_sections (context_menu, LUNAR_MENU_SECTION_PROPERTIES);
...
lunar_tree_view_get_dest_actions (LunarTreeView *view,
                                   GdkDragContext *context,
                                   gint            x,
                                   gint            y,
                                   guint           timestamp,
      ...
      if (gtk_tree_model_get_iter (GTK_TREE_MODEL (view->model), &iter, path))
        {
          ...
          gtk_tree_model_get (GTK_TREE_MODEL (view->model), &iter, LUNAR_TREE_MODEL_COLUMN_FILE, &file, -1);
          if (G_LIKELY (file != NULL))
            {
              /* check if the file accepts the drop */
              actions = lunar_file_accepts_drop (file, view->drop_file_list, context, &action);
  ...
  if (G_LIKELY (path != NULL))
    {
      ...
      view->expand_timer_id = g_timeout_add_full (G_PRIORITY_LOW, LUNAR_TREE_VIEW_EXPAND_TIMEOUT,
                                                  lunar_tree_view_expand_timer, view,
                                                  lunar_tree_view_expand_timer_destroy);
    }
...
static LunarFile*
lunar_tree_view_get_selected_file (LunarTreeView *view)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreeIter       iter;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
    gtk_tree_model_get (model, &iter, LUNAR_TREE_MODEL_COLUMN_FILE, &file, -1);
...
static LunarDevice*
lunar_tree_view_get_selected_device (LunarTreeView *view)
{
  GtkTreeSelection *selection;
  LunarDevice     *device = NULL;
  GtkTreeModel     *model;
  ...
  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (view));
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
    gtk_tree_model_get (model, &iter, LUNAR_TREE_MODEL_COLUMN_DEVICE, &device, -1);
...
static void
lunar_tree_view_action_unlink_selected_folder (LunarTreeView *view,
                                                gboolean        permanently)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_TREE_VIEW (view));

  /* determine the selected file */
  file = lunar_tree_view_get_selected_file (view);
  if (G_LIKELY (file != NULL))
    {
      if (lunar_file_can_be_trashed (file))
        {
          /* fake a file list */
          file_list.data = file;
          file_list.next = NULL;
          ...
          lunar_application_unlink_files (application, GTK_WIDGET (view), &file_list, permanently);
...
static void
lunar_tree_view_action_move_to_trash (LunarTreeView *view)
{
  lunar_tree_view_action_unlink_selected_folder (view, FALSE);
}



static void
lunar_tree_view_action_delete (LunarTreeView *view)
{
  lunar_tree_view_action_unlink_selected_folder (view, TRUE);
}



static void
lunar_tree_view_action_open (LunarTreeView *view)
{
  LunarFile   *file;
  LunarDevice *device;

  device = lunar_tree_view_get_selected_device (view);
  file = lunar_tree_view_get_selected_file (view);
...
static void
lunar_tree_view_open_selection (LunarTreeView *view)
{
  LunarFile *file;
  ...
  if (G_LIKELY (file != NULL))
    {
      ...
      lunar_navigator_change_directory (LUNAR_NAVIGATOR (view), file);
      g_object_unref (file);
    }
}
...
static void
lunar_tree_view_select_files (LunarTreeView *view,
                               GList          *files_to_selected)
{
  LunarFile *file = NULL;
  ...
  file = lunar_file_get (G_FILE (files_to_selected->data), NULL);
  if (G_LIKELY (file != NULL))
    {
...
static gboolean
lunar_tree_view_visible_func (LunarTreeModel *model,
                               LunarFile      *file,
                               gpointer         user_data)
{
  LunarTreeView *view;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_TREE_VIEW (user_data), FALSE);

  /* if show_hidden is TRUE, nothing is filtered */
  view = LUNAR_TREE_VIEW (user_data);
  if (G_LIKELY (!view->show_hidden))
    {
      /* we display all non-hidden file and hidden files that are ancestors of the current directory */
      visible = !lunar_file_is_hidden (file) || (view->current_directory == file)
                || (view->current_directory != NULL && lunar_file_is_ancestor (view->current_directory, file));
    }
...
static gboolean
lunar_tree_view_selection_func (GtkTreeSelection *selection,
                                 GtkTreeModel     *model,
                                 GtkTreePath      *path,
                                 gboolean          path_currently_selected,
                                 gpointer          user_data)
...
static gboolean
lunar_tree_view_cursor_idle (gpointer user_data)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (user_data);
  GtkTreePath    *path;
  GtkTreeIter     iter;
  LunarFile     *file;
  LunarFolder   *folder;
...
LUNAR_THREADS_ENTER
  ...
  path = lunar_tree_view_get_preferred_toplevel_path (view, view->current_directory);
  ...
  /* collect all LunarFiles in the path of current_directory in a List. root is on the very left side */
  for (file = view->current_directory; file != NULL; file = lunar_file_get_parent (file, NULL))
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (view->model), &iter, LUNAR_TREE_MODEL_COLUMN_FILE, &file_in_tree, -1);
  for (lp = path_as_list; lp != NULL; lp = lp->next)
    {
      if (LUNAR_FILE (lp->data) == file_in_tree)
          break;
    }
  if (file_in_tree)
  ...
  for (; lp != NULL; lp = lp->next)
    {
      ...
      if (folder != NULL && lunar_folder_get_loading (folder))
        {
      ...
      if (!gtk_tree_model_iter_children (GTK_TREE_MODEL (view->model), &child_iter, &iter))
        {
          ...
          file_info = lunar_file_get_info (file);
          if (file_info != NULL)
            {
              ...
              if (!g_file_info_get_attribute_boolean (file_info, G_FILE_ATTRIBUTE_ACCESS_CAN_READ))
                {
                  ...
                  lunar_tree_model_add_child (view->model, iter.user_data,  LUNAR_FILE (lp->next->data));
                }
            }
          break; /* we dont have a valid child_iter by now, so we cannot continue.                         */
                 /* Since done is FALSE, the next iteration on lunar_tree_view_cursor_idle will go deeper */
        }
...
static void
lunar_tree_view_cursor_idle_destroy (gpointer user_data)
{
  LUNAR_TREE_VIEW (user_data)->cursor_idle_id = 0;
}



static gboolean
lunar_tree_view_drag_scroll_timer (gpointer user_data)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (user_data);
...
static void
lunar_tree_view_drag_scroll_timer_destroy (gpointer user_data)
{
  LUNAR_TREE_VIEW (user_data)->drag_scroll_timer_id = 0;
}



static gboolean
lunar_tree_view_expand_timer (gpointer user_data)
{
  LunarTreeView *view = LUNAR_TREE_VIEW (user_data);
  GtkTreePath    *path;
...
static void
lunar_tree_view_expand_timer_destroy (gpointer user_data)
{
  LUNAR_TREE_VIEW (user_data)->expand_timer_id = 0;
}



/**
 * lunar_tree_view_get_show_hidden:
 * @view : a #LunarTreeView.
 *
 ...
 **/
static gboolean
lunar_tree_view_get_show_hidden (LunarTreeView *view)
{
  _lunar_return_val_if_fail (LUNAR_IS_TREE_VIEW (view), FALSE);
...
/**
 * lunar_tree_view_set_show_hidden:
 * @view        : a #LunarTreeView.
 ...
 **/
static void
lunar_tree_view_set_show_hidden (LunarTreeView *view,
                                  gboolean        show_hidden)
{
  _lunar_return_if_fail (LUNAR_IS_TREE_VIEW (view));
  _lunar_return_if_fail (LUNAR_IS_TREE_MODEL (view->model));
...
/**
 * lunar_tree_view_get_preferred_toplevel_path:
 * @view        : a #LunarTreeView.
 * @file        : the #LunarFile we want the toplevel path for
 *
 ...
 **/
static GtkTreePath *
lunar_tree_view_get_preferred_toplevel_path (LunarTreeView *view,
                                              LunarFile     *file)
{
  GtkTreeModel *model = GTK_TREE_MODEL (view->model);
  GtkTreePath  *path = NULL;
  GtkTreeIter   iter;
  LunarFile   *toplevel_file;
  GFile        *home;
  GFile        *root;
  GFile        *best_match;
      ...
      if (gtk_tree_model_get_iter (GTK_TREE_MODEL (view->model), &iter, path))
        {
          ...
          gtk_tree_model_get (GTK_TREE_MODEL (view->model), &iter, LUNAR_TREE_MODEL_COLUMN_FILE, &toplevel_file, -1);
          if (toplevel_file)
            {
              /* check if the toplevel file is an ancestor */
              if (lunar_file_is_ancestor (file, toplevel_file))
                {
  ...
  if (lunar_file_is_gfile_ancestor (file, home))
    best_match = home;
  else if (lunar_file_is_gfile_ancestor (file, root))
    best_match = root;
  else
    best_match = NULL;
  ...
  do
    {
      gtk_tree_model_get (model, &iter, LUNAR_TREE_MODEL_COLUMN_FILE, &toplevel_file, -1);
      ...
      if (g_file_equal (lunar_file_get_file (file),
                        lunar_file_get_file (toplevel_file)))
        {
          gtk_tree_path_free (path);
      ...
      if (lunar_file_is_ancestor (file, toplevel_file))
        {
          ...
          if (!g_file_equal (lunar_file_get_file (toplevel_file), home) &&
              !g_file_equal (lunar_file_get_file (toplevel_file), root))
            {
              gtk_tree_path_free (path);
              g_object_unref (toplevel_file);
              path = gtk_tree_model_get_path (model, &iter);
          ...
          if (best_match != NULL &&
              g_file_equal (lunar_file_get_file (toplevel_file), best_match))
            {
              gtk_tree_path_free (path);
              g_object_unref (toplevel_file);
              path = gtk_tree_model_get_path (model, &iter);
 ...
 * lunar_tree_view_new:
 *
 * Allocates a new #LunarTreeView instance.
 *
 * Return value: the newly allocated #LunarTreeView instance.
 **/
GtkWidget*
lunar_tree_view_new (void)
{
  return g_object_new (LUNAR_TYPE_TREE_VIEW, NULL);
}
