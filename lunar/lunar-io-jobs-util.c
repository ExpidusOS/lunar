...
/**
 * lunar_io_jobs_util_next_duplicate_file:
 * @job   : a #LunarJob.
 ...
 **/
GFile *
lunar_io_jobs_util_next_duplicate_file (LunarJob *job,
                                         GFile     *file,
                                         gboolean   copy,
                                         guint      n,
                                         GError   **error)
{
  ...
  _lunar_return_val_if_fail (0 < n, NULL);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, NULL);
  _lunar_return_val_if_fail (!lunar_g_file_is_root (file), NULL);
  ...
  info = g_file_query_info (file, G_FILE_ATTRIBUTE_STANDARD_TYPE ","
                            ...
                            endo_job_get_cancellable (ENDO_JOB (job)), &err);
  ...
  if (copy)
    {
      ...
      if (g_file_info_get_file_type (info) != G_FILE_TYPE_DIRECTORY)
        dot = lunar_util_str_get_extension (old_display_name);
...
/**
 * lunar_io_jobs_util_next_renamed_file:
 * @job       : a #LunarJob.
 ...
 **/
GFile *
lunar_io_jobs_util_next_renamed_file (LunarJob *job,
                                       GFile     *src_file,
                                       GFile     *tgt_file,
                                       guint      n,
                                       GError   **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_JOB (job), NULL);
  _lunar_return_val_if_fail (G_IS_FILE (src_file), NULL);
  _lunar_return_val_if_fail (G_IS_FILE (tgt_file), NULL);
  _lunar_return_val_if_fail (0 < n, NULL);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, NULL);
  _lunar_return_val_if_fail (!lunar_g_file_is_root (src_file), NULL);
  _lunar_return_val_if_fail (!lunar_g_file_is_root (tgt_file), NULL);
