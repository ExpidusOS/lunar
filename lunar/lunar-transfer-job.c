...
#include <lunar/lunar-io-jobs-util.h>
...
typedef struct _LunarTransferNode LunarTransferNode;



static void     lunar_transfer_job_get_property (GObject    *object,
                                                  guint       prop_id,
                                                  GValue     *value,
                                                  GParamSpec *pspec);

static void     lunar_transfer_job_set_property (GObject      *object,
                                                  guint         prop_id,
                                                  const GValue *value,
                                                  GParamSpec   *pspec);

static void     lunar_transfer_job_finalize     (GObject                *object);
static gboolean lunar_transfer_job_execute      (EndoJob                 *job,
                                                  GError                **error);
static void     lunar_transfer_node_free        (gpointer                data);



struct _LunarTransferJobClass
{
  LunarJobClass __parent__;
};

struct _LunarTransferJob
{
  LunarJob               __parent__;

  LunarTransferJobType   type;
  ...
  LunarParallelCopyMode  parallel_copy_mode;
};

struct _LunarTransferNode
{
  LunarTransferNode *next;
  LunarTransferNode *children;
  GFile              *source_file;
...
G_DEFINE_TYPE (LunarTransferJob, lunar_transfer_job, LUNAR_TYPE_JOB)



static void
lunar_transfer_job_class_init (LunarTransferJobClass *klass)
{
  GObjectClass *gobject_class;
  EndoJobClass  *endojob_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_transfer_job_finalize;
  gobject_class->get_property = lunar_transfer_job_get_property;
  gobject_class->set_property = lunar_transfer_job_set_property;

  endojob_class = ENDO_JOB_CLASS (klass);
  endojob_class->execute = lunar_transfer_job_execute;
  ...
  /**
   * LunarPropertiesDialog:parallel_copy_mode:
   *
...
static void
lunar_transfer_job_init (LunarTransferJob *job)
{
  job->preferences = lunar_preferences_get ();
  endo_binding_new (G_OBJECT (job->preferences), "misc-file-size-binary",
                   G_OBJECT (job), "file-size-binary");
  endo_binding_new (G_OBJECT (job->preferences), "misc-parallel-copy-mode",
...
static void
lunar_transfer_job_finalize (GObject *object)
{
  LunarTransferJob *job = LUNAR_TRANSFER_JOB (object);

  g_list_free_full (job->source_node_list, lunar_transfer_node_free);
  ...
  lunar_g_file_list_free (job->target_file_list);

  g_object_unref (job->preferences);

  (*G_OBJECT_CLASS (lunar_transfer_job_parent_class)->finalize) (object);
}



static void
lunar_transfer_job_get_property (GObject     *object,
                                  guint        prop_id,
                                  GValue      *value,
                                  GParamSpec  *pspec)
{
...
static void
lunar_transfer_job_set_property (GObject      *object,
                                  guint         prop_id,
                                  const GValue *value,
                                  GParamSpec   *pspec)
{
...
static void
lunar_transfer_job_check_pause (LunarTransferJob *job)
{
  _lunar_return_if_fail (LUNAR_IS_TRANSFER_JOB (job));
  while (lunar_job_is_paused (LUNAR_JOB (job)) && !endo_job_is_cancelled (ENDO_JOB (job)))
    {
...
static void
lunar_transfer_job_progress (goffset  current_num_bytes,
                              goffset  total_num_bytes,
                              gpointer user_data)
{
  LunarTransferJob *job = user_data;
  ...
  lunar_transfer_job_check_pause (job);
      ...
      if (expired_time > (500 * 1000))
        {
          ...
          endo_job_percent (ENDO_JOB (job), new_percentage);
...
static gboolean
lunar_transfer_job_collect_node (LunarTransferJob  *job,
                                  LunarTransferNode *node,
                                  GError            **error)
{
  LunarTransferNode *child_node;
  GFileInfo          *info;
  GError             *err = NULL;
  GList              *file_list;
  GList              *lp;

  _lunar_return_val_if_fail (LUNAR_IS_TRANSFER_JOB (job), FALSE);
  _lunar_return_val_if_fail (node != NULL && G_IS_FILE (node->source_file), FALSE);
  ...
  info = g_file_query_info (node->source_file,
                            ...
                            endo_job_get_cancellable (ENDO_JOB (job)),
                            &err);
  ...
  if (g_file_info_get_file_type (info) == G_FILE_TYPE_DIRECTORY)
    {
      ...
      file_list = lunar_io_scan_directory (LUNAR_JOB (job), node->source_file,
                                            G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
      ...
      for (lp = file_list; err == NULL && lp != NULL; lp = lp->next)
        {
          ...
          child_node = g_slice_new0 (LunarTransferNode);
          ...
          lunar_transfer_job_collect_node (job, child_node, &err);
        }
...
static gboolean
ttj_copy_file (LunarTransferJob *job,
               GFile             *source_file,
               GFile             *target_file,
               ...
               GError           **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_TRANSFER_JOB (job), FALSE);
  _lunar_return_val_if_fail (G_IS_FILE (source_file), FALSE);
  _lunar_return_val_if_fail (G_IS_FILE (target_file), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (target_type == G_FILE_TYPE_SYMBOLIC_LINK && (copy_flags & G_FILE_COPY_OVERWRITE) != 0)
    {
      ...
      if (!g_file_delete (target_file, endo_job_get_cancellable (ENDO_JOB (job)), &err))
        {
          g_propagate_error (error, err);
          return FALSE;
        }
  ...
  g_file_copy (source_file, target_file, copy_flags,
               endo_job_get_cancellable (ENDO_JOB (job)),
               lunar_transfer_job_progress, job, &err);
  ...
  if (G_UNLIKELY (err != NULL && err->domain == G_IO_ERROR))
    {
      if (err->code == G_IO_ERROR_WOULD_MERGE
      ...
      else if (err->code == G_IO_ERROR_WOULD_RECURSE)
        {
          ...
          if (!endo_job_set_error_if_cancelled (ENDO_JOB (job), &err))
            {
              if (target_exists)
                {
...
/**
 * lunar_transfer_job_copy_file:
 * @job                : a #LunarTransferJob.
 ...
 **/
static GFile *
lunar_transfer_job_copy_file (LunarTransferJob *job,
                               GFile             *source_file,
                               GFile             *target_file,
                               gboolean           replace_confirmed,
                               gboolean           rename_confirmed,
                               GError           **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_TRANSFER_JOB (job), NULL);
  _lunar_return_val_if_fail (G_IS_FILE (source_file), NULL);
  _lunar_return_val_if_fail (G_IS_FILE (target_file), NULL);
  ...
  while (err == NULL)
    {
      ...
      if (G_LIKELY (!g_file_equal (source_file, dest_file)))
      ...
      else
        {
          for (n = 1; err == NULL; ++n)
            {
              GFile *duplicate_file = lunar_io_jobs_util_next_duplicate_file (LUNAR_JOB (job),
                                                                               source_file,
                                                                               TRUE, n,
                                                                               &err);
      ...
      if (err->domain == G_IO_ERROR && err->code == G_IO_ERROR_EXISTS)
        {
          ...
          if (replace_confirmed)
            ...
            response = lunar_job_ask_replace (LUNAR_JOB (job), source_file,
                                               dest_file, &err);
          ...
          if (response == LUNAR_JOB_RESPONSE_REPLACE)
            {
              copy_flags |= G_FILE_COPY_OVERWRITE;
              continue;
            }
          else if (response == LUNAR_JOB_RESPONSE_RENAME)
            {
              GFile *renamed_file;
              renamed_file = lunar_io_jobs_util_next_renamed_file (LUNAR_JOB (job),
                                                                    source_file,
                                                                    dest_file,
                                                                    ++n_rename, &err);
          ...
          if (response == LUNAR_JOB_RESPONSE_SKIP)
            return g_object_ref (source_file);
        }
    }

  _lunar_assert (err != NULL);
...
static void
lunar_transfer_job_copy_node (LunarTransferJob  *job,
                               LunarTransferNode *node,
                               GFile              *target_file,
                               ...
                               GError            **error)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_TRANSFER_JOB (job));
  _lunar_return_if_fail (node != NULL && G_IS_FILE (node->source_file));
  _lunar_return_if_fail (target_file == NULL || node->next == NULL);
  _lunar_return_if_fail ((target_file == NULL && target_parent_file != NULL) || (target_file != NULL && target_parent_file == NULL));
  _lunar_return_if_fail (error == NULL || *error == NULL);
  ...
  for (; err == NULL && node != NULL; node = node->next)
    {
      ...
      endo_job_info_message (ENDO_JOB (job), "%s", g_file_info_get_display_name (info));

retry_copy:
      lunar_transfer_job_check_pause (job);

      /* copy the item specified by this node (not recursively) */
      real_target_file = lunar_transfer_job_copy_file (job, node->source_file,
                                                        target_file,
          ...
          if (G_LIKELY (node->source_file != real_target_file))
            {
              ...
              lunar_thumbnail_cache_copy_file (thumbnail_cache,
                                                node->source_file,
                                                real_target_file);
              ...
              if (node->children != NULL)
                {
                  ...
                  lunar_transfer_job_copy_node (job, node->children, NULL, real_target_file, NULL, &err);

                  /* free resources allocted for the children */
                  lunar_transfer_node_free (node->children);
                  node->children = NULL;
                }
              ...
              if (G_LIKELY (target_file_list_return != NULL))
                {
                  *target_file_list_return =
                    lunar_g_file_list_prepend (*target_file_list_return,
                                                real_target_file);
                }
...
retry_remove:
              ...
              if (job->type == LUNAR_TRANSFER_JOB_MOVE)
                {
                  if (g_file_delete (node->source_file,
                                     ...
                                     &err))
                    {
                      ...
                      lunar_thumbnail_cache_delete_file (thumbnail_cache,
                                                          node->source_file);
                    }
                  else
                    {
                      /* ask the user to retry */
                      response = lunar_job_ask_skip (LUNAR_JOB (job), "%s",
                                                      err->message);
                      ...
                      if (G_UNLIKELY (response == LUNAR_JOB_RESPONSE_RETRY))
                        goto retry_remove;
                    }
                }
            }
          ...
          if (err->domain != G_IO_ERROR || err->code != G_IO_ERROR_NO_SPACE)
            {
              ...
              response = lunar_job_ask_skip (LUNAR_JOB (job), "%s", err->message);
...
static gboolean
lunar_transfer_job_verify_destination (LunarTransferJob  *transfer_job,
                                        GError            **error)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_TRANSFER_JOB (transfer_job), FALSE);
  ...
  /* for all actions in lunar use the same target directory so
  ...
  filesystem_info = g_file_query_filesystem_info (dest, LUNARX_FILESYSTEM_INFO_NAMESPACE,
                                                  endo_job_get_cancellable (ENDO_JOB (transfer_job)),
                                                  NULL);
      ...
      if (transfer_job->total_size > free_space)
        {
          ...
          succeed = lunar_job_ask_no_size (LUNAR_JOB (transfer_job),
   ...
   * error message.
   * More details: https://bugzilla.expidus.org/show_bug.cgi?id=15367#c16 */
...
static gboolean
lunar_transfer_job_prepare_untrash_file (EndoJob     *job,
                                          GFileInfo  *info,
                                          GFile      *file,
                                          GError    **error)
{
  ...
  /* update progress information */
  endo_job_info_message (job, _("Trying to restore \"%s\""),
  ...
  if (target_parent != NULL)
    parent_exists = g_file_query_exists (target_parent, endo_job_get_cancellable (job));
  else
    parent_exists = FALSE;
  ...
  if (target_parent != NULL && !parent_exists)
    {
      ...
      response = lunar_job_ask_create (LUNAR_JOB (job),
...
static gboolean
lunar_transfer_job_move_file_with_rename (EndoJob             *job,
                                           LunarTransferNode *node,
                                           GList              *tp,
  ...
  while (TRUE)
    {
      ...
      move_rename_successful = g_file_move (node->source_file,
                                            ...
                                            endo_job_get_cancellable (job),
                                            NULL, NULL, error);
      if (!move_rename_successful && !endo_job_is_cancelled (job) && ((*error)->code == G_IO_ERROR_EXISTS))
        continue;
...
static gboolean
lunar_transfer_job_move_file (EndoJob                *job,
                               GFileInfo             *info,
                               GList                 *sp,
                               LunarTransferNode    *node,
                               GList                 *tp,
                               ...
                               GError               **error)
{
  LunarTransferJob *transfer_job = LUNAR_TRANSFER_JOB (job);
  LunarJobResponse  response;
  gboolean           move_successful;

  /* update progress information */
  endo_job_info_message (job, _("Trying to move \"%s\""),
                        g_file_info_get_display_name (info));
  ...
  if (!move_successful && (*error)->code == G_IO_ERROR_EXISTS)
    {
      g_clear_error (error);
      response = lunar_job_ask_replace (LUNAR_JOB (job), node->source_file, tp->data, NULL);
      ...
      /* if the user chose to rename then try to do so */
        ...
        {
          move_successful = lunar_transfer_job_move_file_with_rename (job, node, tp, move_flags, error);
        }
      /* if the user chose to cancel then abort all remaining file moves */
      else if (response == LUNAR_JOB_RESPONSE_CANCEL)
        {
          ...
          g_list_free_full (transfer_job->source_node_list, lunar_transfer_node_free);
      ...
      /* if the user chose not to replace nor rename the file, so that response == LUNAR_JOB_RESPONSE_SKIP,
      ...
      if (move_successful)
        {
          ...
          lunar_thumbnail_cache_move_file (thumbnail_cache,
                                            node->source_file,
                                            tp->data);

          /* add the target file to the new files list */
          *new_files_list_p = lunar_g_file_list_prepend (*new_files_list_p, tp->data);
        }

      /* release source and target files */
      lunar_transfer_node_free (node);
  ...
  else if (!endo_job_is_cancelled (job) &&
           (
...
static GList *
lunar_transfer_job_filter_running_jobs (GList     *jobs,
                                         LunarJob *own_job)
{
  LunarJob *job;
  GList     *run_jobs = NULL;

  _lunar_return_val_if_fail (LUNAR_IS_TRANSFER_JOB (own_job), NULL);
  ...
  for (GList *ljobs = jobs; ljobs != NULL; ljobs = ljobs->next)
    {
      ...
      if (job == own_job)
        continue;
      if (!endo_job_is_cancelled (ENDO_JOB (job)) && !lunar_job_is_paused (job) && !lunar_job_is_frozen (job))
        {
...
static gboolean
lunar_transfer_job_device_id_in_job_list (const char *device_fs_id,
                                           GList      *jobs)
{
  LunarTransferJob *job;

  for (GList *ljobs = jobs; device_fs_id != NULL && ljobs != NULL; ljobs = ljobs->next)
    {
      if (LUNAR_IS_TRANSFER_JOB (ljobs->data))
        {
          job = LUNAR_TRANSFER_JOB (ljobs->data);
...
/**
 * lunar_transfer_job_is_file_on_local_device:
 ...
 **/
static gboolean
lunar_transfer_job_is_file_on_local_device (GFile *file)
{
  gboolean  is_local;
  GFile    *target_file;
  GFile    *target_parent;
  ...
  _lunar_return_val_if_fail (file != NULL, TRUE);
...
static void
lunar_transfer_job_fill_source_device_info (LunarTransferJob *transfer_job,
                                             GFile             *file)
{
  ...
  transfer_job->is_source_device_local = lunar_transfer_job_is_file_on_local_device (file);
}


static void
lunar_transfer_job_fill_target_device_info (LunarTransferJob *transfer_job,
                                             GFile             *file)
{
  /*
  ...
  g_object_unref (target_file);
  transfer_job->is_target_device_local = lunar_transfer_job_is_file_on_local_device (file);
}



static void
lunar_transfer_job_determine_copy_behavior (LunarTransferJob *transfer_job,
                                             ...
                                             gboolean          *should_freeze_on_any_other_job_p)
{
  ...
  *should_freeze_on_any_other_job_p = FALSE;
  if (transfer_job->parallel_copy_mode == LUNAR_PARALLEL_COPY_MODE_ALWAYS)
    /* never freeze, always parallel copies */
    *always_parallel_copy_p = TRUE;
  else if (transfer_job->parallel_copy_mode == LUNAR_PARALLEL_COPY_MODE_ONLY_LOCAL)
    {
  ...
  else if (transfer_job->parallel_copy_mode == LUNAR_PARALLEL_COPY_MODE_ONLY_LOCAL_SAME_DEVICES)
    {
      /* always parallel copy if:
      ...
      else /* same as LUNAR_PARALLEL_COPY_MODE_ONLY_LOCAL */
        {
          *always_parallel_copy_p = transfer_job->is_source_device_local && transfer_job->is_target_device_local;
          *freeze_if_src_busy_p = ! transfer_job->is_source_device_local;
          *freeze_if_tgt_busy_p = ! transfer_job->is_target_device_local;
  ...
  else /* LUNAR_PARALLEL_COPY_MODE_NEVER */
    {
...
/**
 * lunar_transfer_job_freeze_optional:
 * @job : a #LunarTransferJob.
 *
 * Based on lunar setting, will block until all running jobs
 * doing IO on the source files or target files devices are completed.
 * The unblocking could be forced by the user in the UI.
 *
 **/
static void
lunar_transfer_job_freeze_optional (LunarTransferJob *transfer_job)
{
  ...
  _lunar_return_if_fail (LUNAR_IS_TRANSFER_JOB (transfer_job));

  /* no source node list nor target file list */
  if (transfer_job->source_node_list == NULL || transfer_job->target_file_list == NULL)
    return;
  /* first source file */
  lunar_transfer_job_fill_source_device_info (transfer_job, ((LunarTransferNode*) transfer_job->source_node_list->data)->source_file);
  /* first target file */
  lunar_transfer_job_fill_target_device_info (transfer_job, G_FILE (transfer_job->target_file_list->data));
  lunar_transfer_job_determine_copy_behavior (transfer_job,
                                               &freeze_if_src_busy,
                                               &freeze_if_tgt_busy,
  ...
  while (TRUE)
    {
      GList *jobs = lunar_job_ask_jobs (LUNAR_JOB (transfer_job));
      GList *other_jobs = lunar_transfer_job_filter_running_jobs (jobs, LUNAR_JOB (transfer_job));
      ...
      if
        (
          ...
          (freeze_if_src_busy && lunar_transfer_job_device_id_in_job_list (transfer_job->source_device_fs_id, other_jobs)) ||
          /* should freeze because target is busy and target device id appears in another job */
          (freeze_if_tgt_busy && lunar_transfer_job_device_id_in_job_list (transfer_job->target_device_fs_id, other_jobs))
        )
      ...
      if (endo_job_is_cancelled (ENDO_JOB (transfer_job)))
        break;
      if (!lunar_job_is_frozen (LUNAR_JOB (transfer_job)))
        {
          if (been_frozen)
          ...
          else
            {
              ...
              lunar_job_freeze (LUNAR_JOB (transfer_job));
            }
        }
      g_usleep(500 * 1000); /* pause for 500ms */
    }
  if (lunar_job_is_frozen (LUNAR_JOB (transfer_job)))
    lunar_job_unfreeze (LUNAR_JOB (transfer_job));
}



static gboolean
lunar_transfer_job_execute (EndoJob  *job,
                             GError **error)
{
  LunarThumbnailCache *thumbnail_cache;
  LunarTransferNode   *node;
  ...
  endo_job_info_message (job, _("Collecting files..."));
  ...
  for (sp = transfer_job->source_node_list, tp = transfer_job->target_file_list;
       ...
       sp = snext, tp = tnext)
    {
      lunar_transfer_job_check_pause (transfer_job);
      ...
      if (transfer_job->type == LUNAR_TRANSFER_JOB_MOVE
          && lunar_g_file_is_trashed (node->source_file))
        {
          if (!lunar_transfer_job_prepare_untrash_file (job, info, tp->data, &err))
            break;
          if (!lunar_transfer_job_move_file (job, info, sp, node, tp,
                                              G_FILE_COPY_NOFOLLOW_SYMLINKS | G_FILE_COPY_ALL_METADATA,
                                              thumbnail_cache, &new_files_list, &err))
            break;
        }
      else if (transfer_job->type == LUNAR_TRANSFER_JOB_MOVE)
        {
          if (!lunar_transfer_job_move_file (job, info, sp, node, tp,
      ...
      else if (transfer_job->type == LUNAR_TRANSFER_JOB_COPY)
        {
          if (!lunar_transfer_job_collect_node (LUNAR_TRANSFER_JOB (job), node, &err))
            break;
        }
  ...
  if (G_LIKELY (err == NULL))
    {
      /* check destination */
      if (!lunar_transfer_job_verify_destination (transfer_job, &err))
        {
          if (err != NULL)
            {
              g_propagate_error (error, err);
      ...
      lunar_transfer_job_freeze_optional (transfer_job);
      ...
      for (sp = transfer_job->source_node_list, tp = transfer_job->target_file_list;
        ...
        {
          lunar_transfer_job_copy_node (transfer_job, sp->data, tp->data, NULL,
                                         &new_files_list, &err);
        }
    }
  ...
  if (G_UNLIKELY (err != NULL))
    ...
    {
      lunar_job_new_files (LUNAR_JOB (job), new_files_list);
      lunar_g_file_list_free (new_files_list);
      return TRUE;
    }
}
...
static void
lunar_transfer_node_free (gpointer data)
{
  LunarTransferNode *node = data;
  LunarTransferNode *next;
  ...
  while (node != NULL)
    {
      ...
      g_slice_free (LunarTransferNode, node);
...
LunarJob *
lunar_transfer_job_new (GList                *source_node_list,
                         GList                *target_file_list,
                         LunarTransferJobType type)
{
  LunarTransferNode *node;
  ...
  _lunar_return_val_if_fail (source_node_list != NULL, NULL);
  _lunar_return_val_if_fail (target_file_list != NULL, NULL);
  _lunar_return_val_if_fail (g_list_length (source_node_list) == g_list_length (target_file_list), NULL);

  job = g_object_new (LUNAR_TYPE_TRANSFER_JOB, NULL);
  job->type = type;
  ...
  for (sp = source_node_list, tp = target_file_list;
       ...
       sp = sp->next, tp = tp->next)
    {
      ...
      if (G_UNLIKELY (lunar_g_file_is_root (sp->data) || lunar_g_file_is_root (tp->data)))
        continue;

      /* only process non-equal pairs unless we're copying */
      if (G_LIKELY (type != LUNAR_TRANSFER_JOB_MOVE || !g_file_equal (sp->data, tp->data)))
        {
          /* append transfer node for this source file */
          node = g_slice_new0 (LunarTransferNode);
          ...
          /* append target file */
          job->target_file_list = lunar_g_file_list_append (job->target_file_list, tp->data);
        }
    }

  /* make sure we didn't mess things up */
  _lunar_assert (g_list_length (job->source_node_list) == g_list_length (job->target_file_list));

  return LUNAR_JOB (job);
}



gchar *
lunar_transfer_job_get_status (LunarTransferJob *job)
{
