...
static void     lunar_progress_dialog_dispose            (GObject              *object);
static void     lunar_progress_dialog_finalize           (GObject              *object);
static void     lunar_progress_dialog_shown              (LunarProgressDialog *dialog);
static gboolean lunar_progress_dialog_closed             (LunarProgressDialog *dialog);
static gboolean lunar_progress_dialog_toggled            (LunarProgressDialog *dialog,
                                                           GdkEventButton       *button,
                                                           GtkStatusIcon        *status_icon);
static void     lunar_progress_dialog_update_status_icon (LunarProgressDialog *dialog);



struct _LunarProgressDialogClass
{
  GtkWindowClass __parent__;
};

struct _LunarProgressDialog
{
...
G_DEFINE_TYPE (LunarProgressDialog, lunar_progress_dialog, GTK_TYPE_WINDOW);



static void
lunar_progress_dialog_class_init (LunarProgressDialogClass *klass)
{
  GObjectClass *gobject_class;

  /* Determine parent type class */
  lunar_progress_dialog_parent_class = g_type_class_peek_parent (klass);

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->dispose = lunar_progress_dialog_dispose;
  gobject_class->finalize = lunar_progress_dialog_finalize;
}



static void
lunar_progress_dialog_init (LunarProgressDialog *dialog)
{
  dialog->views = NULL;
  ...
  g_signal_connect_swapped (dialog, "show",
                            G_CALLBACK (lunar_progress_dialog_shown), dialog);

  g_signal_connect (dialog, "delete-event",
                    G_CALLBACK (lunar_progress_dialog_closed), dialog);
...
static void
lunar_progress_dialog_dispose (GObject *object)
{
  (*G_OBJECT_CLASS (lunar_progress_dialog_parent_class)->dispose) (object);
}



static void
lunar_progress_dialog_finalize (GObject *object)
{
  LunarProgressDialog *dialog = LUNAR_PROGRESS_DIALOG (object);
...
G_GNUC_END_IGNORE_DEPRECATIONS
  ...
  (*G_OBJECT_CLASS (lunar_progress_dialog_parent_class)->finalize) (object);
}



static void
lunar_progress_dialog_shown (LunarProgressDialog *dialog)
{
  _lunar_return_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog));
...
G_GNUC_END_IGNORE_DEPRECATIONS
      ...
      lunar_progress_dialog_update_status_icon (dialog);
      g_signal_connect_swapped (dialog->status_icon, "button-press-event",
                                G_CALLBACK (lunar_progress_dialog_toggled),
                                GTK_WIDGET (dialog));
    }
}
...
static gboolean
lunar_progress_dialog_closed (LunarProgressDialog *dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog), FALSE);
...
static gboolean
lunar_progress_dialog_toggled (LunarProgressDialog *dialog,
                                GdkEventButton       *event,
                                GtkStatusIcon        *status_icon)
{
  _lunar_return_val_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog), FALSE);
  _lunar_return_val_if_fail (GTK_IS_STATUS_ICON (status_icon), FALSE);
...
static void
lunar_progress_dialog_view_needs_attention (LunarProgressDialog *dialog,
                                             LunarProgressView   *view)
{
  _lunar_return_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog));
...
static void
lunar_progress_dialog_job_finished (LunarProgressDialog *dialog,
                                     LunarProgressView   *view)
{
  guint n_views;
...
#if LIBEXPIDUS1UI_CHECK_VERSION (4, 13, 2)
      expidus_widget_reparent (dialog->content_box, dialog->vbox);
#else
...
static void
lunar_progress_dialog_update_status_icon (LunarProgressDialog *dialog)
{
  gchar *tooltip_text;
  guint  n_views;

  _lunar_return_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog));
  _lunar_return_if_fail (GTK_IS_STATUS_ICON (dialog->status_icon));
...
GtkWidget*
lunar_progress_dialog_new (void)
{
  return g_object_new (LUNAR_TYPE_PROGRESS_DIALOG, NULL);
}



/**
 * lunar_progress_dialog_list_jobs:
 * @dialog       : a #LunarProgressDialog instance
 *
 * Return a list of non-cancelled #LunarJob
 *
 * The caller is responsible to free the returned list using
 * g_list_free() when no longer needed.
 *
 * Return value: non-cancelled #LunarJob list or %NULL if
 *               the list is empty.
 **/
GList *
lunar_progress_dialog_list_jobs (LunarProgressDialog *dialog)
{
  GList              *jobs = NULL;
  GList              *l;
  LunarProgressView *view;
  ...
  _lunar_return_val_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog), NULL);

  for (l = dialog->views; l != NULL; l = l->next)
    {
      view = LUNAR_PROGRESS_VIEW (l->data);
      job = lunar_progress_view_get_job (view);
      if (job != NULL && !endo_job_is_cancelled (ENDO_JOB (job)))
        {
...
void
lunar_progress_dialog_add_job (LunarProgressDialog *dialog,
                                LunarJob            *job,
                                ...
                                const gchar          *title)
{
  ...
  _lunar_return_if_fail (g_utf8_validate (title, -1, NULL));

  view = lunar_progress_view_new_with_job (job);
  lunar_progress_view_set_icon_name (LUNAR_PROGRESS_VIEW (view), icon_name);
  lunar_progress_view_set_title (LUNAR_PROGRESS_VIEW (view), title);
...
#if LIBEXPIDUS1UI_CHECK_VERSION (4, 13, 2)
      expidus_widget_reparent (dialog->content_box, viewport);
#else
  ...
  g_signal_connect_swapped (view, "need-attention",
                            G_CALLBACK (lunar_progress_dialog_view_needs_attention), dialog);

  g_signal_connect_swapped (view, "finished",
                            G_CALLBACK (lunar_progress_dialog_job_finished), dialog);

  g_signal_connect_swapped (job, "ask-jobs",
                            G_CALLBACK (lunar_progress_dialog_list_jobs), dialog);
...
gboolean
lunar_progress_dialog_has_jobs (LunarProgressDialog *dialog)
{
  _lunar_return_val_if_fail (LUNAR_IS_PROGRESS_DIALOG (dialog), FALSE);
