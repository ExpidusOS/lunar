...
/*-
 * Copyright (c) 2005-2011 Benedikt Meurer <benny@expidus.org>
...
static void          lunar_dialogs_select_filename      (GtkWidget  *entry,
                                                          LunarFile *file);



/**
 * lunar_dialogs_show_create:
 ...
 *
 * Displays a Lunar create dialog  with the specified
 ...
 **/
gchar*
lunar_dialogs_show_create (gpointer     parent,
                            ...
                            const gchar *title)
{
  ...
  _lunar_return_val_if_fail (parent == NULL || GDK_IS_SCREEN (parent) || GTK_IS_WIDGET (parent), NULL);
  ...
  filename_input = g_object_new (EXPIDUS_TYPE_FILENAME_INPUT, "original-filename", filename, NULL);
  ...
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label),
                                      GTK_WIDGET (expidus_filename_input_get_entry (filename_input)));
  gtk_widget_show_all ( GTK_WIDGET (filename_input));

  /* ensure that the sensitivity of the Create button is set correctly */
  expidus_filename_input_check (filename_input);
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
    {
      ...
      filename = expidus_filename_input_get_text (filename_input);
      ...
      if (G_UNLIKELY (name == NULL))
        {
          ...
          lunar_dialogs_show_error (dialog, error, _("Cannot convert filename \"%s\" to the local encoding"), filename);
...
/**
 * lunar_dialogs_show_rename_file:
 ...
 * @file   : the #LunarFile we're going to rename.
 *
 * Displays the Lunar rename dialog for a single file rename.
 *
 * Return value: The #LunarJob responsible for renaming the file or
 *               %NULL if there was no renaming required.
 **/
LunarJob *
lunar_dialogs_show_rename_file (gpointer    parent,
                                 LunarFile *file)
{
  ...
  _lunar_return_val_if_fail (parent == NULL || GDK_IS_SCREEN (parent) || GTK_IS_WINDOW (parent), FALSE);
  ...
  filename = lunar_file_get_display_name (file);
  ...
  icon = lunar_icon_factory_load_file_icon (icon_factory, file, LUNAR_FILE_ICON_STATE_DEFAULT, 48);
  ...
  lunar_dialogs_select_filename (GTK_WIDGET (expidus_filename_input_get_entry (filename_input)), file);

  /* get the size the entry requires to render the full text */
  layout = gtk_entry_get_layout (expidus_filename_input_get_entry (filename_input));
  pango_layout_get_pixel_size (layout, &layout_width, NULL);
  gtk_entry_get_layout_offsets (expidus_filename_input_get_entry (filename_input), &layout_offset, NULL);
  ...
  if (G_LIKELY (response == GTK_RESPONSE_OK))
    {
      ...
      text = expidus_filename_input_get_text (filename_input);

      /* check if we have a new name here */
      if (G_LIKELY (!endo_str_is_equal (filename, text)))
        {
          /* try to rename the file */
          job = lunar_io_jobs_rename_file (file, text);
        }
    }
 ...
 *
 * Displays the Lunar about dialog with @format as main text.
 **/
void
lunar_dialogs_show_about (GtkWindow   *parent,
                           const gchar *title,
                           const gchar *format,
                           ...)
{
  ...
  {
    "Benedikt Meurer <benny@expidus.org>",
    "Jannis Pohlmann <jannis@expidus.org>",
    "Nick Schermer <nick@expidus.org>",
    NULL,
  };
  ...
  _lunar_return_if_fail (parent == NULL || GTK_IS_WINDOW (parent));
  ...
  gtk_show_about_dialog (parent,
                         ...
                         "logo-icon-name", "com.expidus.lunar",
                         "program-name", title,
                         "translator-credits", _("translator-credits"),
                         "version", PACKAGE_VERSION,
                         "website", "https://docs.expidus.com/expidus/lunar/start",
                         NULL);
 ...
 **/
void
lunar_dialogs_show_error (gpointer      parent,
                           const GError *error,
                           const gchar  *format,
                           ...)
{
...
/**
 * lunar_dialogs_show_job_ask:
 * @parent   : the parent #GtkWindow or %NULL.
 * @question : the question text.
 * @choices  : possible responses.
 *
 * Utility function to display a question dialog for the LunarJob::ask
 * signal.
 *
 * Return value: the #LunarJobResponse.
 **/
LunarJobResponse
lunar_dialogs_show_job_ask (GtkWindow        *parent,
                             ...
                             LunarJobResponse choices)
{
  ...
  _lunar_return_val_if_fail (parent == NULL || GTK_IS_WINDOW (parent), LUNAR_JOB_RESPONSE_CANCEL);
  _lunar_return_val_if_fail (g_utf8_validate (question, -1, NULL), LUNAR_JOB_RESPONSE_CANCEL);
  ...
  for (n = LUNAR_JOB_RESPONSE_MAX_INT; n >= 0; --n)
    {
      ...
      switch (response)
        ...
        case LUNAR_JOB_RESPONSE_REPLACE:
          mnemonic = _("_Replace");
          break;
        ...
        case LUNAR_JOB_RESPONSE_RENAME:
          mnemonic = _("Re_name");
          break;

        case LUNAR_JOB_RESPONSE_RENAME_ALL:
          mnemonic = _("Rena_me All");
          break;

        case LUNAR_JOB_RESPONSE_NO:
          mnemonic = _("_No");
          break;
  ...
  if (G_UNLIKELY (response <= 0))
    response = LUNAR_JOB_RESPONSE_CANCEL;
...
static void lunar_dialogs_show_job_ask_replace_callback (GtkWidget *button,
                                                          gpointer   user_data)
{
  gint response;

  _lunar_return_if_fail (GTK_IS_DIALOG (user_data));
...
/**
 * lunar_dialogs_show_job_ask_replace:
 * @parent   : the parent #GtkWindow or %NULL.
 ...
 *
 * Return value: the selected #LunarJobResponse.
 **/
LunarJobResponse
lunar_dialogs_show_job_ask_replace (GtkWindow  *parent,
                                     LunarFile *src_file,
                                     LunarFile *dst_file)
{
  LunarIconFactory *icon_factory;
  ...
  gtk_dialog_set_default_response (GTK_DIALOG (dialog), LUNAR_JOB_RESPONSE_REPLACE);
  ...
  g_signal_connect (cancel_button,      "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (skipall_button,     "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (skip_button,        "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (replaceall_button,  "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (replace_button,     "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (renameall_button,   "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);
  g_signal_connect (rename_button,      "clicked", G_CALLBACK (lunar_dialogs_show_job_ask_replace_callback), dialog);

  g_object_set_data (G_OBJECT (cancel_button),     "response-id", GINT_TO_POINTER (GTK_RESPONSE_CANCEL));
  g_object_set_data (G_OBJECT (skipall_button),    "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_SKIP_ALL));
  g_object_set_data (G_OBJECT (skip_button),       "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_SKIP));
  g_object_set_data (G_OBJECT (replaceall_button), "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_REPLACE_ALL));
  g_object_set_data (G_OBJECT (replace_button),    "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_REPLACE));
  g_object_set_data (G_OBJECT (renameall_button),  "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_RENAME_ALL));
  g_object_set_data (G_OBJECT (rename_button),     "response-id", GINT_TO_POINTER (LUNAR_JOB_RESPONSE_RENAME));
  ...
  if (lunar_file_is_symlink (dst_file))
    {
      text = g_strdup_printf (_("This folder already contains a symbolic link \"%s\"."),
                              lunar_file_get_display_name (dst_file));
    }
  else if (lunar_file_is_directory (dst_file))
    {
  ...
  gtk_label_set_xalign (GTK_LABEL (label), 0.0f);
  gtk_label_set_attributes (GTK_LABEL (label), lunar_pango_attr_list_big ());
  gtk_widget_set_hexpand (label, TRUE);
  ...
  icon = lunar_icon_factory_load_file_icon (icon_factory, dst_file, LUNAR_FILE_ICON_STATE_DEFAULT, 48);
  image = gtk_image_new_from_pixbuf (icon);
  gtk_widget_set_margin_start (GTK_WIDGET(image), 6);
  gtk_widget_set_margin_end (GTK_WIDGET(image), 6);
  gtk_widget_set_margin_top (GTK_WIDGET(image), 6);
  ...
  size_string = lunar_file_get_size_string_long (dst_file, file_size_binary);
  date_string = lunar_file_get_date_string (dst_file, LUNAR_FILE_DATE_MODIFIED, date_style, date_custom_style);
  ...
  if (lunar_file_is_symlink (src_file))
    /* TRANSLATORS: Second part of replace dialog sentence */
    text = g_strdup_printf (_("with the following link?"));
  else if (lunar_file_is_directory (src_file))
    /* TRANSLATORS: Second part of replace dialog sentence */
  ...
  icon = lunar_icon_factory_load_file_icon (icon_factory, src_file, LUNAR_FILE_ICON_STATE_DEFAULT, 48);
  image = gtk_image_new_from_pixbuf (icon);
  gtk_widget_set_margin_start (GTK_WIDGET(image), 6);
  gtk_widget_set_margin_end (GTK_WIDGET(image), 6);
  gtk_widget_set_margin_top (GTK_WIDGET(image), 6);
  ...
  size_string = lunar_file_get_size_string_long (src_file, file_size_binary);
  date_string = lunar_file_get_date_string (src_file, LUNAR_FILE_DATE_MODIFIED, date_style, date_custom_style);
  text = g_strdup_printf ("%s %s\n%s %s", _("Size:"), size_string, _("Modified:"), date_string);
  label = gtk_label_new (text);
  gtk_label_set_xalign (GTK_LABEL (label), 0.0f);
  gtk_widget_set_hexpand (label, TRUE);
...
/**
 * lunar_dialogs_show_job_error:
 * @parent : the parent #GtkWindow or %NULL.
 * @error  : the #GError provided by the #LunarJob.
 *
 * Utility function to display a message dialog for the
 * LunarJob::error signal.
 **/
void
lunar_dialogs_show_job_error (GtkWindow *parent,
                               GError    *error)
{
  const gchar *separator;
  GtkWidget   *message;
...
gboolean
lunar_dialogs_show_insecure_program (gpointer     parent,
                                      const gchar *primary,
                                      LunarFile  *file,
                                      const gchar *command)
{
  ...
  _lunar_return_val_if_fail (LUNAR_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (g_utf8_validate (command, -1, NULL), FALSE);
  ...
  if (endo_str_looks_like_an_uri (command))
  ...
  if (lunar_file_is_chmodable (file))
  ...
  if (response == GTK_RESPONSE_APPLY)
    {
      ...
      info = g_file_query_info (lunar_file_get_file (file),
          ...
          if (g_file_info_has_attribute (info, G_FILE_ATTRIBUTE_UNIX_MODE))
            {
              ...
              new_mode = old_mode | LUNAR_FILE_MODE_USR_EXEC | LUNAR_FILE_MODE_GRP_EXEC | LUNAR_FILE_MODE_OTH_EXEC;

              if (old_mode != new_mode)
                {
                  g_file_set_attribute_uint32 (lunar_file_get_file (file),
      ...
      if (err != NULL)
        {
          lunar_dialogs_show_error (parent, err, ("Unable to mark launcher executable"));
          g_error_free (err);
        }
                                            ...
                                            LunarFile *file)
{
  ...
  ext = lunar_util_str_get_extension (filename);
