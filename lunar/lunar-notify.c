...
#ifdef HAVE_LIBNOTIFY
static gboolean lunar_notify_initted = FALSE;



static gboolean
lunar_notify_init (void)
{
  gchar *spec_version = NULL;

  if (!lunar_notify_initted
      && notify_init (PACKAGE_NAME))
    {
      ...
      lunar_notify_initted = TRUE;
    }

  return lunar_notify_initted;
}



static void
lunar_notify_show (LunarDevice *device,
                    const gchar  *summary,
                    const gchar  *message)
{
  ...
  icon = lunar_device_get_icon (device);
  if (icon != NULL)
    {
  ...
  g_object_set_data_full (G_OBJECT (device), I_("lunar-notification"),
...
static gboolean
lunar_notify_device_readonly (LunarDevice *device)
{
  GFile     *mount_point;
...
void
lunar_notify_unmount (LunarDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  gchar       *name;
  const gchar *summary;
  ...
  if (!lunar_notify_init ())
    return;

  name = lunar_device_get_name (device);

  if (lunar_notify_device_readonly (device))
    {
  ...
  lunar_notify_show (device, summary, message);
...
void
lunar_notify_eject (LunarDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  gchar       *name;
  const gchar *summary;
...
void
lunar_notify_finish (LunarDevice *device)
{
#ifdef HAVE_LIBNOTIFY
  NotifyNotification *notification;
  ...
  notification = g_object_get_data (G_OBJECT (device), I_("lunar-notification"));
  if (notification != NULL)
    {
      ...
      g_object_set_data (G_OBJECT (device), I_("lunar-notification"), NULL);
    }
#endif
}
