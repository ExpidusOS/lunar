...
GFile *
lunar_g_file_new_for_home (void)
{
  return g_file_new_for_path (expidus_get_homedir ());
}



GFile *
lunar_g_file_new_for_root (void)
{
...
GFile *
lunar_g_file_new_for_trash (void)
{
...
GFile *
lunar_g_file_new_for_desktop (void)
{
...
GFile *
lunar_g_file_new_for_bookmarks (void)
{
  gchar *filename;
  GFile *bookmarks;
...
gboolean
lunar_g_file_is_root (GFile *file)
{
  GFile   *parent;
...
gboolean
lunar_g_file_is_trashed (GFile *file)
{
...
gboolean
lunar_g_file_is_home (GFile *file)
{
  GFile   *home;
...
gboolean
lunar_g_file_is_trash (GFile *file)
{
  char *uri;
...
gboolean
lunar_g_file_is_computer (GFile *file)
{
  char *uri;
...
gboolean
lunar_g_file_is_network (GFile *file)
{
  char *uri;
...
GKeyFile *
lunar_g_file_query_key_file (GFile              *file,
                              ...
                              GError            **error)
{
  ...
  _lunar_return_val_if_fail (cancellable == NULL || G_IS_CANCELLABLE (cancellable), NULL);
...
gboolean
lunar_g_file_write_key_file (GFile        *file,
                              GKeyFile     *key_file,
                              GCancellable *cancellable,
                              GError      **error)
{
  ...
  _lunar_return_val_if_fail (G_IS_FILE (file), FALSE);
  _lunar_return_val_if_fail (key_file != NULL, FALSE);
...
gchar *
lunar_g_file_get_location (GFile *file)
{
  gchar *location;
...
gchar *
lunar_g_file_get_display_name (GFile *file)
{
  gchar *base_name;
  gchar *display_name;
  ...
  if (G_LIKELY (base_name != NULL))
    {
      ...
      else if (lunar_g_file_is_trash (file))
...
gchar *
lunar_g_file_get_display_name_remote (GFile *mount_point)
{
  gchar       *scheme;
  gchar       *parse_name;
  const gchar *p;
  ...
  _lunar_return_val_if_fail (G_IS_FILE (mount_point), NULL);
  ...
  if (display_name == NULL)
    display_name = lunar_g_file_get_display_name (mount_point);
...
gboolean
lunar_g_vfs_is_uri_scheme_supported (const gchar *scheme)
{
  ...
  _lunar_return_val_if_fail (scheme != NULL && *scheme != '\0', FALSE);
 ...
 **/
gboolean
lunar_g_file_get_free_space (GFile   *file,
...
gchar *
lunar_g_file_get_free_space_string (GFile *file, gboolean file_size_binary)
{
  gchar             *fs_free_str;
  gchar             *fs_size_str;
  guint64            fs_free;
  ...
  if (lunar_g_file_get_free_space (file, &fs_free, &fs_size)
      && fs_size > 0)
    {
...
GType
lunar_g_file_list_get_type (void)
{
  static GType type = G_TYPE_INVALID;

  if (G_UNLIKELY (type == G_TYPE_INVALID))
    {
      type = g_boxed_type_register_static (I_("LunarGFileList"),
                                           (GBoxedCopyFunc) lunar_g_file_list_copy,
                                           (GBoxedFreeFunc) lunar_g_file_list_free);
    }
...
/**
 * lunar_g_file_list_new_from_string:
 ...
 **/
GList *
lunar_g_file_list_new_from_string (const gchar *string)
{
  GList  *list = NULL;
  gchar **uris;
  gsize   n;
 ...
 **/
gchar **
lunar_g_file_list_to_stringv (GList *list)
{
  gchar **uris;
  guint   n;
  GList  *lp;
 ...
 **/
GList*
lunar_g_file_list_get_parents (GList *file_list)
{
  GList    *lp_file_list;
...
gboolean
lunar_g_app_info_launch (GAppInfo          *info,
                          ...
                          GError           **error)
{
  ...
  _lunar_return_val_if_fail (G_IS_APP_INFO (info), FALSE);
  _lunar_return_val_if_fail (working_directory == NULL || G_IS_FILE (working_directory), FALSE);
  _lunar_return_val_if_fail (path_list != NULL, FALSE);
  _lunar_return_val_if_fail (G_IS_APP_LAUNCH_CONTEXT (context), FALSE);
  _lunar_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  ...
  if (working_directory != NULL)
    {
      ...
      if (new_path != NULL)
        {
          /* switch to the desired working directory, remember that of Lunar itself */
          old_path = lunar_util_change_working_directory (new_path);
      ...
      for (lp = path_list; lp != NULL; lp = lp->next)
        {
          ...
          default_app_info = lunar_file_get_default_handler (file);
  ...
  /* check if we need to reset the working directory to the one Lunar was
   * opened from */
  if (old_path != NULL)
    {
      /* switch to Lunar's original working directory */
      new_path = lunar_util_change_working_directory (old_path);
...
gboolean
lunar_g_app_info_should_show (GAppInfo *info)
{
#ifdef HAVE_GIO_UNIX
  _lunar_return_val_if_fail (G_IS_APP_INFO (info), FALSE);
...
gboolean
lunar_g_vfs_metadata_is_supported (void)
{
  GFile                  *root;
