...
static void lunar_column_editor_finalize           (GObject                  *object);
static void lunar_column_editor_help_clicked       (GtkWidget                *button,
                                                     LunarColumnEditor       *column_editor);
static void lunar_column_editor_move_down          (GtkWidget                *button,
                                                     LunarColumnEditor       *column_editor);
static void lunar_column_editor_move_up            (GtkWidget                *button,
                                                     LunarColumnEditor       *column_editor);
static void lunar_column_editor_toggled            (GtkCellRendererToggle    *cell_renderer,
                                                     const gchar              *path_string,
                                                     LunarColumnEditor       *column_editor);
static void lunar_column_editor_toggle_visibility  (GtkWidget                *button,
                                                     LunarColumnEditor       *column_editor);
static void lunar_column_editor_update_buttons     (LunarColumnEditor       *column_editor);
static void lunar_column_editor_use_defaults       (GtkWidget                *button,
                                                     LunarColumnEditor       *column_editor);



struct _LunarColumnEditorClass
{
  LunarAbstractDialogClass __parent__;
};

struct _LunarColumnEditor
{
...
G_DEFINE_TYPE (LunarColumnEditor, lunar_column_editor, LUNAR_TYPE_ABSTRACT_DIALOG)



static void
lunar_column_editor_class_init (LunarColumnEditorClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_column_editor_finalize;
}



static void
lunar_column_editor_init (LunarColumnEditor *column_editor)
{
  ...
  column_editor->preferences = lunar_preferences_get ();

  /* grab a reference on the shared column model */
  column_editor->column_model = lunar_column_model_get_default ();
  g_signal_connect_data (G_OBJECT (column_editor->column_model), "row-changed", G_CALLBACK (lunar_column_editor_update_buttons),
                         column_editor, NULL, G_CONNECT_AFTER | G_CONNECT_SWAPPED);
  g_signal_connect_data (G_OBJECT (column_editor->column_model), "rows-reordered", G_CALLBACK (lunar_column_editor_update_buttons),
                         column_editor, NULL, G_CONNECT_AFTER | G_CONNECT_SWAPPED);
  ...
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (lunar_column_editor_help_clicked), column_editor);
  gtk_box_pack_start (GTK_BOX (endo_gtk_dialog_get_action_area (GTK_DIALOG (column_editor))), button, FALSE, FALSE, 0);
  gtk_button_box_set_child_secondary (GTK_BUTTON_BOX (endo_gtk_dialog_get_action_area (GTK_DIALOG (column_editor))), button, TRUE);
  ...
  g_signal_connect (G_OBJECT (renderer), "toggled", G_CALLBACK (lunar_column_editor_toggled), column_editor);
  gtk_tree_view_column_pack_start (column, renderer, FALSE);
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "active", LUNAR_COLUMN_MODEL_COLUMN_VISIBLE,
                                       "activatable", LUNAR_COLUMN_MODEL_COLUMN_MUTABLE,
                                       NULL);
  ...
  gtk_tree_view_column_set_attributes (column, renderer,
                                       "text", LUNAR_COLUMN_MODEL_COLUMN_NAME,
                                       NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (column_editor->tree_view), column);

  /* create the "Move Up" button */
  column_editor->up_button = gtk_button_new_with_mnemonic (_("Move _Up"));
  g_signal_connect (G_OBJECT (column_editor->up_button), "clicked", G_CALLBACK (lunar_column_editor_move_up), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->down_button), "clicked", G_CALLBACK (lunar_column_editor_move_down), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->show_button), "clicked", G_CALLBACK (lunar_column_editor_toggle_visibility), column_editor);
  ...
  g_signal_connect (G_OBJECT (column_editor->hide_button), "clicked", G_CALLBACK (lunar_column_editor_toggle_visibility), column_editor);
  ...
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (lunar_column_editor_use_defaults), column_editor);
  ...
  endo_mutual_binding_new_with_negation (G_OBJECT (column_editor->preferences), "last-details-view-fixed-columns", G_OBJECT (button), "active");
  gtk_grid_attach (GTK_GRID (grid), button, 0, 1, 1, 1);
  lunar_gtk_label_set_a11y_relation (GTK_LABEL (label), button);
  gtk_widget_show (button);

  /* setup the tree selection */
  selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (column_editor->tree_view));
  g_signal_connect_swapped (G_OBJECT (selection), "changed", G_CALLBACK (lunar_column_editor_update_buttons), column_editor);
...
static void
lunar_column_editor_finalize (GObject *object)
{
  LunarColumnEditor *column_editor = LUNAR_COLUMN_EDITOR (object);
  ...
  (*G_OBJECT_CLASS (lunar_column_editor_parent_class)->finalize) (object);
}



static void
lunar_column_editor_help_clicked (GtkWidget          *button,
                                   LunarColumnEditor *column_editor)
{
  _lunar_return_if_fail (LUNAR_IS_COLUMN_EDITOR (column_editor));
  _lunar_return_if_fail (GTK_IS_BUTTON (button));

  /* open the user manual */
  expidus_dialog_show_help (GTK_WINDOW (gtk_widget_get_toplevel (button)),
                         "lunar",
...
static void
lunar_column_editor_move_down (GtkWidget          *button,
                                LunarColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter1))
    {
      ...
      if (gtk_tree_model_iter_next (model, &iter2))
        lunar_column_model_exchange (LUNAR_COLUMN_MODEL (model), &iter1, &iter2);
    }
}
...
static void
lunar_column_editor_move_up (GtkWidget          *button,
                              LunarColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
...
static void
lunar_column_editor_toggled (GtkCellRendererToggle *cell_renderer,
                              const gchar           *path_string,
                              LunarColumnEditor    *column_editor)
{
  LunarColumn column;
  ...
  _lunar_return_if_fail (GTK_IS_CELL_RENDERER_TOGGLE (cell_renderer));
  _lunar_return_if_fail (LUNAR_IS_COLUMN_EDITOR (column_editor));
  _lunar_return_if_fail (path_string != NULL);
  ...
  if (gtk_tree_model_get_iter (GTK_TREE_MODEL (column_editor->column_model), &iter, path))
    {
      ...
      column = lunar_column_model_get_column_for_iter (column_editor->column_model, &iter);

      /* ...determine the existing visbility setting... */
      visible = lunar_column_model_get_column_visible (column_editor->column_model, column);

      /* ...and change the visibility of the column */
      lunar_column_model_set_column_visible (column_editor->column_model, column, !visible);
    }
  gtk_tree_path_free (path);
}
...
static void
lunar_column_editor_toggle_visibility (GtkWidget          *button,
                                        LunarColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  LunarColumn      column;
...
static void
lunar_column_editor_update_buttons (LunarColumnEditor *column_editor)
{
  GtkTreeSelection *selection;
  GtkTreeModel     *model;
  GtkTreePath      *path;
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      ...
      gtk_tree_model_get (model, &iter, LUNAR_COLUMN_MODEL_COLUMN_MUTABLE, &mutable, LUNAR_COLUMN_MODEL_COLUMN_VISIBLE, &visible, -1);
...
static void
lunar_column_editor_use_defaults (GtkWidget          *button,
                                   LunarColumnEditor *column_editor)
{
 ...
 *
 * Convenience function to display a #LunarColumnEditor.
 *
 ...
 **/
void
lunar_show_column_editor (gpointer parent)
{
  ...
  dialog = g_object_new (LUNAR_TYPE_COLUMN_EDITOR, NULL);
