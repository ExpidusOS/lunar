...
/**
 * lunar_gtk_label_set_a11y_relation:
 * @label  : a #GtkLabel.
 * @widget : a #GtkWidget.
 *
 ...
 **/
void
lunar_gtk_label_set_a11y_relation (GtkLabel  *label,
                                    GtkWidget *widget)
{
  ...
  _lunar_return_if_fail (GTK_IS_LABEL (label));
...
/**
 * lunar_gtk_menu_lunarx_menu_item_new:
 * @lunarx_menu_item   : a #LunarxMenuItem
 * @menu_to_append_item : #GtkMenuShell on which the item should be appended, or NULL
 *
 * method to create a #GtkMenuItem from a #LunarxMenuItem and append it to the passed #GtkMenuShell
 * This method will as well add all sub-items in case the passed #LunarxMenuItem is a submenu
 *
 * Return value: (transfer full): The new #GtkImageMenuItem.
 **/
GtkWidget*
lunar_gtk_menu_lunarx_menu_item_new (GObject      *lunarx_menu_item,
                                       GtkMenuShell *menu_to_append_item)
{
  gchar        *name, *label_text, *tooltip_text, *icon_name, *accel_path;
  gboolean      sensitive;
  GtkWidget    *gtk_menu_item;
  LunarxMenu  *lunarx_menu;
  GList        *children;
  GList        *lp;
  GtkWidget    *submenu;

  g_return_val_if_fail (LUNARX_IS_MENU_ITEM (lunarx_menu_item), NULL);

  g_object_get (G_OBJECT (lunarx_menu_item),
                "name", &name,
                "label", &label_text,
  ...
  gtk_menu_item = expidus_gtk_image_menu_item_new_from_icon_name (label_text, tooltip_text, accel_path,
                                                               G_CALLBACK (lunarx_menu_item_activate),
                                                               G_OBJECT (lunarx_menu_item), icon_name, menu_to_append_item);

  /* recursively add submenu items if any */
  if (gtk_menu_item != NULL && lunarx_menu != NULL)
  {
    children = lunarx_menu_get_items (lunarx_menu);
    submenu = gtk_menu_new ();
    for (lp = children; lp != NULL; lp = lp->next)
      lunar_gtk_menu_lunarx_menu_item_new (lp->data, GTK_MENU_SHELL (submenu));
    gtk_menu_item_set_submenu (GTK_MENU_ITEM (gtk_menu_item), submenu);
    lunarx_menu_item_list_free (children);
  }
  g_free (name);
  g_free (accel_path);
  g_free (label_text);
 ...
 **/
void
lunar_gtk_menu_clean (GtkMenu *menu)
{
  GList     *children, *lp;
  GtkWidget *submenu;
 ...
 * @menu : a #GtkMenu.
 *
 * Conveniance wrapper for lunar_gtk_menu_run_at_event_pointer, to run a menu for the current event
 **/
void
lunar_gtk_menu_run (GtkMenu *menu)
{
  GdkEvent *event = gtk_get_current_event ();
  lunar_gtk_menu_run_at_event (menu, event);
 ...
 **/
void
lunar_gtk_menu_run_at_event (GtkMenu *menu, GdkEvent *event)
{
  GMainLoop *loop;
  gulong     signal_id;
 ...
 **/
void
lunar_gtk_widget_set_tooltip (GtkWidget   *widget,
                               const gchar *format,
                               ...)
{
  va_list  var_args;
  ...
  _lunar_return_if_fail (GTK_IS_WIDGET (widget));
  _lunar_return_if_fail (g_utf8_validate (format, -1, NULL));
...
/**
 * lunar_gtk_editable_can_cut:
 *
 * Return value: TRUE if it's possible to cut text off of a GtkEditable.
 *               FALSE, otherwise.
 **/
gboolean
lunar_gtk_editable_can_cut (GtkEditable *editable)
{
  return gtk_editable_get_editable (editable) &&
         lunar_gtk_editable_can_copy (editable);
}



/**
 * lunar_gtk_editable_can_copy:
 *
 * Return value: TRUE if it's possible to copy text from a GtkEditable.
 *               FALSE, otherwise.
 **/
gboolean
lunar_gtk_editable_can_copy (GtkEditable *editable)
{
...
/**
 * lunar_gtk_editable_can_paste:
 *
 * Return value: TRUE if it's possible to paste text to a GtkEditable.
 *               FALSE, otherwise.
 **/
gboolean
lunar_gtk_editable_can_paste (GtkEditable *editable)
{
  return gtk_editable_get_editable (editable);
}
