...
static void lunar_apr_image_page_file_changed  (LunarAprAbstractPage    *abstract_page,
                                                 LunarxFileInfo          *file);
...
struct _LunarAprImagePageClass
{
  LunarAprAbstractPageClass __parent__;
};

struct _LunarAprImagePage
{
...
LUNARX_DEFINE_TYPE (LunarAprImagePage,
                     lunar_apr_image_page,
                     LUNAR_APR_TYPE_ABSTRACT_PAGE);



static void
lunar_apr_image_page_class_init (LunarAprImagePageClass *klass)
{
  LunarAprAbstractPageClass *lunarapr_abstract_page_class;

  lunarapr_abstract_page_class = LUNAR_APR_ABSTRACT_PAGE_CLASS (klass);
  lunarapr_abstract_page_class->file_changed = lunar_apr_image_page_file_changed;
}



static void
lunar_apr_image_page_init (LunarAprImagePage *image_page)
{
  ...
  lunarx_property_page_set_label (LUNARX_PROPERTY_PAGE (image_page), _("Image"));
  ...
  for (n = 0; n < G_N_ELEMENTS (TAIP_EXIF); ++n)
    {
      ...
      endo_binding_new (G_OBJECT (image_page->exif_labels[n]), "visible", G_OBJECT (label), "visible");
...
static void
lunar_apr_image_page_file_changed (LunarAprAbstractPage *abstract_page,
                                    LunarxFileInfo       *file)
{
  LunarAprImagePage *image_page = LUNAR_APR_IMAGE_PAGE (abstract_page);
