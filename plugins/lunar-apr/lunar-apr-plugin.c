...
lunar_extension_initialize (LunarxProviderPlugin *plugin)
{
  ...
  g_message ("Initializing LunarApr extension");
#endif

  /* register the types provided by this plugin */
  lunar_apr_abstract_page_register_type (plugin);
  lunar_apr_desktop_page_register_type (plugin);
  lunar_apr_image_page_register_type (plugin);
  lunar_apr_provider_register_type (plugin);

  /* setup the plugin provider type list */
  type_list[0] = LUNAR_APR_TYPE_PROVIDER;
}
...
{
#ifdef G_ENABLE_DEBUG
  g_message ("Shutting down LunarApr extension");
#endif
}
