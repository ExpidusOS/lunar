...
static void    lunar_sbr_date_renamer_finalize     (GObject                   *object);
static void    lunar_sbr_date_renamer_get_property (GObject                   *object,
                                                     guint                      prop_id,
                                                     GValue                    *value,
                                                     GParamSpec                *pspec);
static void    lunar_sbr_date_renamer_set_property (GObject                   *object,
                                                     guint                      prop_id,
                                                     const GValue              *value,
                                                     GParamSpec                *pspec);
static gchar  *lunar_sbr_get_time_string           (guint64                    file_time,
                                                     const gchar               *custom_format);
#ifdef HAVE_EXIF
static guint64 lunar_sbr_get_time_from_string      (const gchar               *string);
#endif
static guint64 lunar_sbr_get_time                  (LunarxFileInfo           *file,
                                                     LunarSbrDateMode          mode);
static gchar  *lunar_sbr_date_renamer_process      (LunarxRenamer            *renamer,
                                                     LunarxFileInfo           *file,
                                                     const gchar               *text,
                                                     guint                      idx);
...
struct _LunarSbrDateRenamerClass
{
  LunarxRenamerClass __parent__;
};

struct _LunarSbrDateRenamer
{
...
LUNARX_DEFINE_TYPE (LunarSbrDateRenamer, lunar_sbr_date_renamer, LUNARX_TYPE_RENAMER);



static void
lunar_sbr_date_renamer_class_init (LunarSbrDateRenamerClass *klass)
{
  LunarxRenamerClass *lunarxrenamer_class;
  GObjectClass        *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_sbr_date_renamer_finalize;
  gobject_class->get_property = lunar_sbr_date_renamer_get_property;
  gobject_class->set_property = lunar_sbr_date_renamer_set_property;

  lunarxrenamer_class = LUNARX_RENAMER_CLASS (klass);
  lunarxrenamer_class->process = lunar_sbr_date_renamer_process;

  /**
   * LunarSbrDateRenamer:mode:
   *
   * The #LunarSbrDateMode to use.
   **/
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_enum ("mode",
                                                      "mode",
                                                      "mode",
                                                      LUNAR_SBR_TYPE_DATE_MODE,
                                                      LUNAR_SBR_DATE_MODE_NOW,
                                                      G_PARAM_READWRITE));

  /**
   * LunarSbrDateRenamer:format:
   *
  ...
  /**
   * LunarSbrDateRenamer:offset-mode:
   *
...
static void
lunar_sbr_date_renamer_init (LunarSbrDateRenamer *date_renamer)
{
  GEnumClass     *klass;
  ...
  klass = g_type_class_ref (LUNAR_SBR_TYPE_DATE_MODE);
  for (n = 0; n < klass->n_values; ++n)
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _(klass->values[n].value_nick));
  endo_mutual_binding_new (G_OBJECT (date_renamer), "mode", G_OBJECT (combo), "active");
  ...
  entry = gtk_entry_new ();
  endo_mutual_binding_new (G_OBJECT (entry), "text", G_OBJECT (date_renamer), "format");
  ...
  endo_mutual_binding_new (G_OBJECT (date_renamer), "offset", G_OBJECT (adjustment), "value");

  combo = gtk_combo_box_text_new ();
  klass = g_type_class_ref (LUNAR_SBR_TYPE_OFFSET_MODE);
  for (n = 0; n < klass->n_values; ++n)
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _(klass->values[n].value_nick));
  endo_mutual_binding_new (G_OBJECT (date_renamer), "offset-mode", G_OBJECT (combo), "active");
...
static void
lunar_sbr_date_renamer_finalize (GObject *object)
{
  LunarSbrDateRenamer *date_renamer = LUNAR_SBR_DATE_RENAMER (object);

  /* release the format */
  g_free (date_renamer->format);

  (*G_OBJECT_CLASS (lunar_sbr_date_renamer_parent_class)->finalize) (object);
}



static void
lunar_sbr_date_renamer_get_property (GObject    *object,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODE:
      g_value_set_enum (value, lunar_sbr_date_renamer_get_mode (date_renamer));
      break;

    case PROP_FORMAT:
      g_value_set_string (value, lunar_sbr_date_renamer_get_format (date_renamer));
      break;

    case PROP_OFFSET:
      g_value_set_uint (value, lunar_sbr_date_renamer_get_offset (date_renamer));
      break;

    case PROP_OFFSET_MODE:
      g_value_set_enum (value, lunar_sbr_date_renamer_get_offset_mode (date_renamer));
      break;
...
static void
lunar_sbr_date_renamer_set_property (GObject      *object,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODE:
      lunar_sbr_date_renamer_set_mode (date_renamer, g_value_get_enum (value));
      break;

    case PROP_FORMAT:
      lunar_sbr_date_renamer_set_format (date_renamer, g_value_get_string (value));
      break;

    case PROP_OFFSET:
      lunar_sbr_date_renamer_set_offset (date_renamer, g_value_get_uint (value));
      break;

    case PROP_OFFSET_MODE:
      lunar_sbr_date_renamer_set_offset_mode (date_renamer, g_value_get_enum (value));
      break;
...
static gchar *
lunar_sbr_get_time_string (guint64      file_time,
                            const gchar *format)
{
  struct tm *tm;
  time_t     _time;
...
#ifdef HAVE_EXIF
static guint64
lunar_sbr_get_time_from_string (const gchar *string)
{
  struct tm tm;
...
static guint64
lunar_sbr_get_time (LunarxFileInfo   *file,
                     LunarSbrDateMode  mode)
{
  ...
  switch (mode)
    {
    case LUNAR_SBR_DATE_MODE_NOW:
      /* set the time to the current time */
      file_time = time (NULL);
      break;

    case LUNAR_SBR_DATE_MODE_ATIME:
    case LUNAR_SBR_DATE_MODE_MTIME:
      /* get the file info */
      file_info = lunarx_file_info_get_file_info (file);

      /* get the time from the info */
      if (mode == LUNAR_SBR_DATE_MODE_ATIME)
        {
...
#ifdef HAVE_EXIF
    case LUNAR_SBR_DATE_MODE_TAKEN:
      /* get the uri */
                  ...
                  if (G_LIKELY (exif_entry != NULL))
                    {
                      ...
                      if (exif_entry_get_value (exif_entry, exif_buffer, sizeof (exif_buffer)) != NULL)
                        file_time = lunar_sbr_get_time_from_string (exif_buffer);
                    }
...
static gchar*
lunar_sbr_date_renamer_process (LunarxRenamer  *renamer,
                                 LunarxFileInfo *file,
                                 const gchar     *text,
                                 guint            idx)
{
  LunarSbrDateRenamer *date_renamer = LUNAR_SBR_DATE_RENAMER (renamer);
  gchar                *string;
  guint64               file_time;
  const gchar          *s;
  GString              *result;
  ...
  offset = (date_renamer->offset_mode == LUNAR_SBR_OFFSET_MODE_LEFT) ? date_renamer->offset : (text_length - date_renamer->offset);
  if (G_UNLIKELY (offset > text_length))
    return g_strdup (text);

  /* get the file time */
  file_time = lunar_sbr_get_time (file, date_renamer->mode);
  if (file_time == 0)
    return g_strdup (text);
  ...
  string = lunar_sbr_get_time_string (file_time, date_renamer->format);
  if (string)
    {
...
/**
 * lunar_sbr_date_renamer_new:
 *
 * Allocates a new #LunarSbrDateRenamer instance.
 *
 * Return value: the newly allocated #LunarSbrDateRenamer.
 **/
LunarSbrDateRenamer*
lunar_sbr_date_renamer_new (void)
{
  return g_object_new (LUNAR_SBR_TYPE_DATE_RENAMER,
...
/**
 * lunar_sbr_date_renamer_get_mode:
 * @date_renamer : a #LunarSbrDateRenamer.
 *
 ...
 **/
LunarSbrDateMode
lunar_sbr_date_renamer_get_mode (LunarSbrDateRenamer *date_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer), LUNAR_SBR_DATE_MODE_NOW);
...
/**
 * lunar_sbr_date_renamer_set_mode:
 * @date_renamer : a #LunarSbrDateRenamer.
 * @mode         : the new #LunarSbrDateMode for @date_renamer.
 *
 * Sets the mode of @date_renamer to @mode.
 **/
void
lunar_sbr_date_renamer_set_mode (LunarSbrDateRenamer *date_renamer,
                                  LunarSbrDateMode     mode)
{
  g_return_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer));
  ...
  if (G_LIKELY (date_renamer->mode != mode))
    {
      ...
      /* update the renamer */
      lunarx_renamer_changed (LUNARX_RENAMER (date_renamer));
...
/**
 * lunar_sbr_date_renamer_get_format:
 * @date_renamer : a #LunarSbrDateRenamer.
 *
 ...
 **/
const gchar*
lunar_sbr_date_renamer_get_format (LunarSbrDateRenamer *date_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer), NULL);
...
/**
 * lunar_sbr_date_renamer_set_format:
 * @date_renamer : a #LunarSbrDateRenamer.
 ...
 **/
void
lunar_sbr_date_renamer_set_format (LunarSbrDateRenamer *date_renamer,
                                    const gchar          *format)
{
  g_return_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer));

  /* check if we have a new format */
  if (G_LIKELY (!endo_str_is_equal (date_renamer->format, format)))
    {
...
/**
 * lunar_sbr_date_renamer_get_offset:
 * @date_renamer : a #LunarSbrInsertRenamer.
 *
 ...
 **/
guint
lunar_sbr_date_renamer_get_offset (LunarSbrDateRenamer *date_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer), 0);
...
/**
 * lunar_sbr_date_renamer_set_offset:
 * @date_renamer : a #LunarSbrInsertRenamer.
 ...
 **/
void
lunar_sbr_date_renamer_set_offset (LunarSbrDateRenamer *date_renamer,
                                    guint                 offset)
{
  g_return_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer));
...
/**
 * lunar_sbr_date_renamer_get_offset_mode:
 * @date_renamer : a #LunarSbrInsertRenamer.
 *
 ...
 **/
LunarSbrOffsetMode
lunar_sbr_date_renamer_get_offset_mode (LunarSbrDateRenamer *date_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer), LUNAR_SBR_OFFSET_MODE_LEFT);
...
/**
 * lunar_sbr_date_renamer_set_offset_mode:
 * @date_renamer : a #LunarSbrInsertRenamer.
 ...
 **/
void
lunar_sbr_date_renamer_set_offset_mode (LunarSbrDateRenamer *date_renamer,
                                         LunarSbrOffsetMode   offset_mode)
{
  g_return_if_fail (LUNAR_SBR_IS_DATE_RENAMER (date_renamer));
