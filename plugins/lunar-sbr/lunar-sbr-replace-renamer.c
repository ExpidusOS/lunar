...
static void   lunar_sbr_replace_renamer_finalize     (GObject                      *object);
static void   lunar_sbr_replace_renamer_get_property (GObject                      *object,
                                                       guint                         prop_id,
                                                       GValue                       *value,
                                                       GParamSpec                   *pspec);
static void   lunar_sbr_replace_renamer_set_property (GObject                      *object,
                                                       guint                         prop_id,
                                                       const GValue                 *value,
                                                       GParamSpec                   *pspec);
static void   lunar_sbr_replace_renamer_realize      (GtkWidget                    *widget);
static gchar *lunar_sbr_replace_renamer_process      (LunarxRenamer               *renamer,
                                                       LunarxFileInfo              *file,
                                                       const gchar                  *text,
                                                       guint                         idx);
#ifdef HAVE_PCRE
static gchar *lunar_sbr_replace_renamer_pcre_exec    (LunarSbrReplaceRenamer      *replace_renamer,
                                                       const gchar                  *text);
static void   lunar_sbr_replace_renamer_pcre_update  (LunarSbrReplaceRenamer      *replace_renamer);
#endif



struct _LunarSbrReplaceRenamerClass
{
  LunarxRenamerClass __parent__;
};

struct _LunarSbrReplaceRenamer
{
...
LUNARX_DEFINE_TYPE (LunarSbrReplaceRenamer, lunar_sbr_replace_renamer, LUNARX_TYPE_RENAMER);



static void
lunar_sbr_replace_renamer_class_init (LunarSbrReplaceRenamerClass *klass)
{
  ...
  gobject_class->finalize = lunar_sbr_replace_renamer_finalize;
  gobject_class->get_property = lunar_sbr_replace_renamer_get_property;
  gobject_class->set_property = lunar_sbr_replace_renamer_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->realize = lunar_sbr_replace_renamer_realize;

  lunarxrenamer_class = LUNARX_RENAMER_CLASS (klass);
  lunarxrenamer_class->process = lunar_sbr_replace_renamer_process;

  /**
   * LunarSbrReplaceRenamer:case-sensitive:
   *
  ...
  /**
   * LunarSbrReplaceRenamer:pattern:
   *
   * The search pattern.
   **/
  g_object_class_install_property (gobject_class,
  ...
  /**
   * LunarSbrReplaceRenamer:replacement:
   *
...
static void
lunar_sbr_replace_renamer_init (LunarSbrReplaceRenamer *replace_renamer)
{
  ...
  endo_mutual_binding_new (G_OBJECT (replace_renamer->pattern_entry), "text", G_OBJECT (replace_renamer), "pattern");
  ...
  endo_mutual_binding_new (G_OBJECT (button), "active", G_OBJECT (replace_renamer), "regexp");
  ...
  endo_mutual_binding_new (G_OBJECT (entry), "text", G_OBJECT (replace_renamer), "replacement");
  ...
  endo_mutual_binding_new (G_OBJECT (button), "active", G_OBJECT (replace_renamer), "case-sensitive");
...
static void
lunar_sbr_replace_renamer_finalize (GObject *object)
{
  LunarSbrReplaceRenamer *replace_renamer = LUNAR_SBR_REPLACE_RENAMER (object);
  ...
  (*G_OBJECT_CLASS (lunar_sbr_replace_renamer_parent_class)->finalize) (object);
}



static void
lunar_sbr_replace_renamer_get_property (GObject    *object,
                                         guint       prop_id,
                                         GValue     *value,
                                         GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_CASE_SENSITIVE:
      g_value_set_boolean (value, lunar_sbr_replace_renamer_get_case_sensitive (replace_renamer));
      break;

    case PROP_PATTERN:
      g_value_set_string (value, lunar_sbr_replace_renamer_get_pattern (replace_renamer));
      break;

    case PROP_REPLACEMENT:
      g_value_set_string (value, lunar_sbr_replace_renamer_get_replacement (replace_renamer));
      break;

    case PROP_REGEXP:
      g_value_set_boolean (value, lunar_sbr_replace_renamer_get_regexp (replace_renamer));
      break;
...
static void
lunar_sbr_replace_renamer_set_property (GObject      *object,
                                         guint         prop_id,
                                         const GValue *value,
                                         GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CASE_SENSITIVE:
      lunar_sbr_replace_renamer_set_case_sensitive (replace_renamer, g_value_get_boolean (value));
      break;

    case PROP_PATTERN:
      lunar_sbr_replace_renamer_set_pattern (replace_renamer, g_value_get_string (value));
      break;

    case PROP_REPLACEMENT:
      lunar_sbr_replace_renamer_set_replacement (replace_renamer, g_value_get_string (value));
      break;

    case PROP_REGEXP:
      lunar_sbr_replace_renamer_set_regexp (replace_renamer, g_value_get_boolean (value));
      break;
...
static void
lunar_sbr_replace_renamer_realize (GtkWidget *widget)
{
  /* realize the widget */
  (*GTK_WIDGET_CLASS (lunar_sbr_replace_renamer_parent_class)->realize) (widget);

#ifdef HAVE_PCRE
  /* update the PCRE pattern */
  lunar_sbr_replace_renamer_pcre_update (LUNAR_SBR_REPLACE_RENAMER (widget));
#endif
}
...
static gchar*
lunar_sbr_replace_renamer_process (LunarxRenamer  *renamer,
                                    LunarxFileInfo *file,
                                    const gchar     *text,
                                    guint            idx)
{
  LunarSbrReplaceRenamer *replace_renamer = LUNAR_SBR_REPLACE_RENAMER (renamer);
  ...
  if (G_UNLIKELY (replace_renamer->regexp))
    {
      ...
      return lunar_sbr_replace_renamer_pcre_exec (replace_renamer, text);
#endif
    }
...
#ifdef HAVE_PCRE
static gchar*
lunar_sbr_replace_renamer_pcre_exec (LunarSbrReplaceRenamer *replace_renamer,
                                      const gchar             *subject)
{
  const gchar *r;
  GString     *result;
...
static void
lunar_sbr_replace_renamer_pcre_update (LunarSbrReplaceRenamer *replace_renamer)
{
...
/**
 * lunar_sbr_replace_renamer_new:
 *
 * Allocates a new #LunarSbrReplaceRenamer instance.
 *
 * Return value: the newly allocated #LunarSbrReplaceRenamer.
 **/
LunarSbrReplaceRenamer*
lunar_sbr_replace_renamer_new (void)
{
  return g_object_new (LUNAR_SBR_TYPE_REPLACE_RENAMER,
...
/**
 * lunar_sbr_replace_renamer_get_case_sensitive:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 *
 ...
 **/
gboolean
lunar_sbr_replace_renamer_get_case_sensitive (LunarSbrReplaceRenamer *replace_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer), FALSE);
...
/**
 * lunar_sbr_replace_renamer_set_case_sensitive:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 ...
 **/
void
lunar_sbr_replace_renamer_set_case_sensitive (LunarSbrReplaceRenamer *replace_renamer,
                                               gboolean                 case_sensitive)
{
  g_return_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer));
  ...
  if (G_LIKELY (replace_renamer->case_sensitive != case_sensitive))
    {
      ...
      lunar_sbr_replace_renamer_pcre_update (replace_renamer);
#endif

      /* update the renamer */
      lunarx_renamer_changed (LUNARX_RENAMER (replace_renamer));
...
/**
 * lunar_sbr_replace_renamer_get_pattern:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 *
 ...
 **/
const gchar*
lunar_sbr_replace_renamer_get_pattern (LunarSbrReplaceRenamer *replace_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer), NULL);
...
/**
 * lunar_sbr_replace_renamer_set_pattern:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 ...
 **/
void
lunar_sbr_replace_renamer_set_pattern (LunarSbrReplaceRenamer *replace_renamer,
                                        const gchar             *pattern)
{
  g_return_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer));
  ...
  if (!endo_str_is_equal (replace_renamer->pattern, pattern))
    {
      /* apply the new value */
      g_free (replace_renamer->pattern);
...
/**
 * lunar_sbr_replace_renamer_get_regexp:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 *
 ...
 **/
gboolean
lunar_sbr_replace_renamer_get_regexp (LunarSbrReplaceRenamer *replace_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer), FALSE);
...
/**
 * lunar_sbr_replace_renamer_set_regexp:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 ...
 **/
void
lunar_sbr_replace_renamer_set_regexp (LunarSbrReplaceRenamer *replace_renamer,
                                       gboolean                 regexp)
{
  g_return_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer));
...
/**
 * lunar_sbr_replace_renamer_get_replacement:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 *
 ...
 **/
const gchar*
lunar_sbr_replace_renamer_get_replacement (LunarSbrReplaceRenamer *replace_renamer)
{
  g_return_val_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer), NULL);
...
/**
 * lunar_sbr_replace_renamer_set_replacement:
 * @replace_renamer : a #LunarSbrReplaceRenamer.
 ...
 **/
void
lunar_sbr_replace_renamer_set_replacement (LunarSbrReplaceRenamer *replace_renamer,
                                            const gchar             *replacement)
{
  g_return_if_fail (LUNAR_SBR_IS_REPLACE_RENAMER (replace_renamer));
  ...
  if (!endo_str_is_equal (replace_renamer->replacement, replacement))
    {
      /* apply the setting */
      g_free (replace_renamer->replacement);
