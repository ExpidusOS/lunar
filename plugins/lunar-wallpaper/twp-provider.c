 ...
 * Copyright (c) 2008-2009 Mike Massonnet <mmassonnet@expidus.org>
...
static void   twp_menu_provider_init            (LunarxMenuProviderIface *iface);
static void   twp_provider_finalize             (GObject                  *object);
static GList *twp_provider_get_file_menu_items  (LunarxMenuProvider      *menu_provider,
                                                 GtkWidget                *window,
                                                 GList                    *files);
static void   twp_action_set_wallpaper          (LunarxMenuItem          *item,
                                                 gpointer                  user_data);
...
LUNARX_DEFINE_TYPE_WITH_CODE (TwpProvider, twp_provider, G_TYPE_OBJECT,
                               LUNARX_IMPLEMENT_INTERFACE (LUNARX_TYPE_MENU_PROVIDER,
                                                            twp_menu_provider_init));

static void
twp_menu_provider_init (LunarxMenuProviderIface *iface)
{
...
static GList*
twp_provider_get_file_menu_items (LunarxMenuProvider *menu_provider,
                                  GtkWidget           *window,
                                  GList               *files)
{
  ...
  if (files->next == NULL)
    {
      ...
      if (!lunarx_file_info_is_directory (files->data))
        {
          /* get the mime type of the file */
          mime_type = lunarx_file_info_get_mime_type (files->data);

          if (g_str_has_prefix (mime_type, "image/")
              && (lunarx_file_info_has_mime_type (files->data, "image/jpeg")
                  || lunarx_file_info_has_mime_type (files->data, "image/png")
                  || lunarx_file_info_has_mime_type (files->data, "image/svg+xml")
                  || lunarx_file_info_has_mime_type (files->data, "image/svg+xml-compressed")))
            {
              item = lunarx_menu_item_new ("Twp::setwallpaper", _("Set as wallpaper"), NULL, "preferences-desktop-wallpaper");
...
static void
twp_action_set_wallpaper (LunarxMenuItem *item,
                          gpointer         user_data)
{
  LunarxFileInfo *file_info = user_data;
  ...
  file_uri = lunarx_file_info_get_uri (file_info);
  ...
  if (g_strcmp0 (desktop_type, "EXPIDUS") == 0)
    {
      g_debug ("set on expidus");

      channel = esconf_channel_get ("expidus1-desktop");
      ...
      esconf_channel_set_string (channel, image_path_prop, file_name);
      esconf_channel_set_bool (channel, image_show_prop, TRUE);

      /* If there isn't a wallpaper style set, then set one */
      current_image_style = esconf_channel_get_int (channel, image_style_prop, -1);
      if (current_image_style == -1)
        {
          esconf_channel_set_int (channel, image_style_prop, 0);
        }
       ...
       * it is) and use that. */
      is_single_workspace = esconf_channel_get_bool (channel, "/backdrop/single-workspace-mode", TRUE);
      if (is_single_workspace)
        {
          workspace = esconf_channel_get_int (channel, "/backdrop/single-workspace-number", 0);
        }
      ...
      if (current_image_style == -1)
        {
          esconf_channel_set_int (channel, image_style_prop, 5);
        }
    ...
    {
      g_warning (("Failed to set wallpaper: $XDG_CURRENT_DESKTOP Desktop type '%s' not supported by lunar wallpaper plugin."), desktop_type);
    }
...
/* Taken from expidus_spawn_get_active_workspace_number in expidus-spawn.c apart of
 * the libexpidus1ui library.
 * https://git.expidus.com/expidus/libexpidus1ui/tree/libexpidus1ui/expidus-spawn.c#n193
 */
static gint
