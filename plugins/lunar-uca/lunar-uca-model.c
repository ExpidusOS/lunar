 ...
 * Copyright (c) 2009-2012 Jannis Pohlmann <jannis@expidus.org>
 *
...
typedef struct _LunarUcaModelItem LunarUcaModelItem;
...
static void               lunar_uca_model_tree_model_init  (GtkTreeModelIface    *iface);
static void               lunar_uca_model_finalize         (GObject              *object);
static GtkTreeModelFlags  lunar_uca_model_get_flags        (GtkTreeModel         *tree_model);
static gint               lunar_uca_model_get_n_columns    (GtkTreeModel         *tree_model);
static GType              lunar_uca_model_get_column_type  (GtkTreeModel         *tree_model,
                                                             gint                  column);
static gboolean           lunar_uca_model_get_iter         (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter,
                                                             GtkTreePath          *path);
static GtkTreePath       *lunar_uca_model_get_path         (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter);
static void               lunar_uca_model_get_value        (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter,
                                                             gint                  column,
                                                             GValue               *value);
static gboolean           lunar_uca_model_iter_next        (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter);
static gboolean           lunar_uca_model_iter_children    (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter,
                                                             GtkTreeIter          *parent);
static gboolean           lunar_uca_model_iter_has_child   (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter);
static gint               lunar_uca_model_iter_n_children  (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter);
static gboolean           lunar_uca_model_iter_nth_child   (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter,
                                                             GtkTreeIter          *parent,
                                                             gint                  n);
static gboolean           lunar_uca_model_iter_parent      (GtkTreeModel         *tree_model,
                                                             GtkTreeIter          *iter,
                                                             GtkTreeIter          *child);
static gboolean           lunar_uca_model_load_from_file   (LunarUcaModel       *uca_model,
                                                             const gchar          *filename,
                                                             GError              **error);
static void               lunar_uca_model_item_reset       (LunarUcaModelItem   *item);
static void               lunar_uca_model_item_free        (gpointer              data);
...
struct _LunarUcaModelClass
{
  GObjectClass __parent__;
};

struct _LunarUcaModel
{
  GObject __parent__;
...
typedef EXPIDUS_GENERIC_STACK(ParserState) ParserStack;
...
LUNARX_DEFINE_TYPE_WITH_CODE (LunarUcaModel,
                               lunar_uca_model,
                               G_TYPE_OBJECT,
                               LUNARX_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_MODEL,
                                                            lunar_uca_model_tree_model_init));



static void
lunar_uca_model_class_init (LunarUcaModelClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunar_uca_model_finalize;
}



static void
lunar_uca_model_tree_model_init (GtkTreeModelIface *iface)
{
  iface->get_flags = lunar_uca_model_get_flags;
  iface->get_n_columns = lunar_uca_model_get_n_columns;
  iface->get_column_type = lunar_uca_model_get_column_type;
  iface->get_iter = lunar_uca_model_get_iter;
  iface->get_path = lunar_uca_model_get_path;
  iface->get_value = lunar_uca_model_get_value;
  iface->iter_next = lunar_uca_model_iter_next;
  iface->iter_children = lunar_uca_model_iter_children;
  iface->iter_has_child = lunar_uca_model_iter_has_child;
  iface->iter_n_children = lunar_uca_model_iter_n_children;
  iface->iter_nth_child = lunar_uca_model_iter_nth_child;
  iface->iter_parent = lunar_uca_model_iter_parent;
}



static void
lunar_uca_model_init (LunarUcaModel *uca_model)
{
  GError *error = NULL;
  gchar  *filename;
  ...
  filename = expidus_resource_lookup (EXPIDUS_RESOURCE_CONFIG, "Lunar/uca.xml");
  if (G_LIKELY (filename != NULL))
    {
      /* try to load the file */
      if (!lunar_uca_model_load_from_file (uca_model, filename, &error))
        {
...
static void
lunar_uca_model_finalize (GObject *object)
{
  LunarUcaModel *uca_model = LUNAR_UCA_MODEL (object);

  /* release all items */
  g_list_free_full (uca_model->items, lunar_uca_model_item_free);

  (*G_OBJECT_CLASS (lunar_uca_model_parent_class)->finalize) (object);
}



static GtkTreeModelFlags
lunar_uca_model_get_flags (GtkTreeModel *tree_model)
{
...
static gint
lunar_uca_model_get_n_columns (GtkTreeModel *tree_model)
{
  return LUNAR_UCA_MODEL_N_COLUMNS;
}



static GType
lunar_uca_model_get_column_type (GtkTreeModel *tree_model,
                                  gint          column)
{
  switch (column)
    {
    case LUNAR_UCA_MODEL_COLUMN_NAME:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_SUB_MENU:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_UNIQUE_ID:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_DESCRIPTION:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_GICON:
      return G_TYPE_ICON;

    case LUNAR_UCA_MODEL_COLUMN_ICON_NAME:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_COMMAND:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_STARTUP_NOTIFY:
      return G_TYPE_BOOLEAN;

    case LUNAR_UCA_MODEL_COLUMN_PATTERNS:
      return G_TYPE_STRING;

    case LUNAR_UCA_MODEL_COLUMN_TYPES:
      return G_TYPE_UINT;

    case LUNAR_UCA_MODEL_COLUMN_STOCK_LABEL:
      return G_TYPE_STRING;
...
static gboolean
lunar_uca_model_get_iter (GtkTreeModel *tree_model,
                           GtkTreeIter  *iter,
                           GtkTreePath  *path)
{
  LunarUcaModel *uca_model = LUNAR_UCA_MODEL (tree_model);

  g_return_val_if_fail (LUNAR_UCA_IS_MODEL (uca_model), FALSE);
...
static GtkTreePath*
lunar_uca_model_get_path (GtkTreeModel *tree_model,
                           GtkTreeIter  *iter)
{
  LunarUcaModel *uca_model = LUNAR_UCA_MODEL (tree_model);
  gint            idx;

  g_return_val_if_fail (LUNAR_UCA_IS_MODEL (uca_model), NULL);
...
static void
lunar_uca_model_get_value (GtkTreeModel *tree_model,
                            GtkTreeIter  *iter,
                            gint          column,
                            GValue       *value)
{
  LunarUcaModelItem *item = ((GList *) iter->user_data)->data;
  LunarUcaModel     *uca_model = LUNAR_UCA_MODEL (tree_model);
  gchar              *str;
...
static gboolean
lunar_uca_model_iter_next (GtkTreeModel *tree_model,
                            GtkTreeIter  *iter)
{
  g_return_val_if_fail (LUNAR_UCA_IS_MODEL (tree_model), FALSE);
  g_return_val_if_fail (iter->stamp == LUNAR_UCA_MODEL (tree_model)->stamp, FALSE);
...
static gboolean
lunar_uca_model_iter_children (GtkTreeModel *tree_model,
                                GtkTreeIter  *iter,
                                GtkTreeIter  *parent)
{
  LunarUcaModel *uca_model = LUNAR_UCA_MODEL (tree_model);
...
static gboolean
lunar_uca_model_iter_has_child (GtkTreeModel *tree_model,
                                 GtkTreeIter  *iter)
{
  return FALSE;
}
...
static gint
lunar_uca_model_iter_n_children (GtkTreeModel *tree_model,
                                  GtkTreeIter  *iter)
{
  LunarUcaModel *uca_model = LUNAR_UCA_MODEL (tree_model);

  g_return_val_if_fail (LUNAR_UCA_IS_MODEL (uca_model), 0);
...
static gboolean
lunar_uca_model_iter_nth_child (GtkTreeModel *tree_model,
                                 GtkTreeIter  *iter,
                                 GtkTreeIter  *parent,
                                 gint          n)
{
...
static gboolean
lunar_uca_model_iter_parent (GtkTreeModel *tree_model,
                              GtkTreeIter  *iter,
                              GtkTreeIter  *child)
{
  return FALSE;
...
static gboolean
lunar_uca_model_load_from_file (LunarUcaModel *uca_model,
                                 const gchar    *filename,
                                 GError        **error)
{
  ...
  parser.stack = expidus_stack_new (ParserStack);
  ...
  expidus_stack_push (parser.stack, PARSER_START);
  ...
  expidus_stack_free (parser.stack);
  g_free (content);

  /* save model to store new ids */
  if (succeed
      && parser.unique_id_generated)
    succeed = lunar_uca_model_save (uca_model, error);
...
static void
lunar_uca_model_item_reset (LunarUcaModelItem *item)
{
...
static void
lunar_uca_model_item_free (gpointer data)
{
  LunarUcaModelItem *item = data;
  lunar_uca_model_item_reset (item);
  g_free (item);
}
                       ...
                       GError             **error)
{
  ...
  switch (expidus_stack_top (parser->stack))
    {
    case PARSER_START:
      if (strcmp (element_name, "actions") == 0)
        expidus_stack_push (parser->stack, PARSER_ACTIONS);
      else
        expidus_stack_push (parser->stack, PARSER_UNKNOWN_ELEMENT);
      break;

    case PARSER_ACTIONS:
      if (strcmp (element_name, "action") == 0)
        {
          parser->name_match = EXPIDUS_LOCALE_NO_MATCH;
          parser->description_match = EXPIDUS_LOCALE_NO_MATCH;
          parser->types = 0;
          parser->startup_notify = FALSE;
          ...
          expidus_stack_push (parser->stack, PARSER_ACTION);
        }
      else
        expidus_stack_push (parser->stack, PARSER_UNKNOWN_ELEMENT);
      break;
      ...
      if (strcmp (element_name, "name") == 0)
        {
          ...
          if (G_LIKELY (attribute_names[n] == NULL))
            {
              parser->name_use = (parser->name_match <= EXPIDUS_LOCALE_NO_MATCH);
            }
          else
            {
              match = expidus_locale_match (parser->locale, attribute_values[n]);
          ...
          expidus_stack_push (parser->stack, PARSER_NAME);
        }
      else if (strcmp (element_name, "submenu") == 0)
        {
          g_string_truncate (parser->submenu, 0);
          expidus_stack_push (parser->stack, PARSER_SUB_FOLDER);
        }
      else if (strcmp (element_name, "unique-id") == 0)
        {
          g_string_truncate (parser->unique_id, 0);
          expidus_stack_push (parser->stack, PARSER_UNIQUE_ID);
        }
      else if (strcmp (element_name, "icon") == 0)
        {
          g_string_truncate (parser->icon_name, 0);
          expidus_stack_push (parser->stack, PARSER_ICON);
        }
      else if (strcmp (element_name, "command") == 0)
        {
          g_string_truncate (parser->command, 0);
          expidus_stack_push (parser->stack, PARSER_COMMAND);
        }
      else if (strcmp (element_name, "patterns") == 0)
        {
          g_string_truncate (parser->patterns, 0);
          expidus_stack_push (parser->stack, PARSER_PATTERNS);
        }
      else if (strcmp (element_name, "description") == 0)
        {
          ...
          if (G_LIKELY (attribute_names[n] == NULL))
            {
              parser->description_use = (parser->description_match <= EXPIDUS_LOCALE_NO_MATCH);
            }
          else
            {
              match = expidus_locale_match (parser->locale, attribute_values[n]);
          ...
          expidus_stack_push (parser->stack, PARSER_DESCRIPTION);
        }
      else if (strcmp (element_name, "startup-notify") == 0)
        {
          parser->startup_notify = TRUE;
          expidus_stack_push (parser->stack, PARSER_STARTUP_NOTIFY);
        }
      else if (strcmp (element_name, "directories") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_DIRECTORIES;
          expidus_stack_push (parser->stack, PARSER_DIRECTORIES);
        }
      else if (strcmp (element_name, "audio-files") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_AUDIO_FILES;
          expidus_stack_push (parser->stack, PARSER_AUDIO_FILES);
        }
      else if (strcmp (element_name, "image-files") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_IMAGE_FILES;
          expidus_stack_push (parser->stack, PARSER_IMAGE_FILES);
        }
      else if (strcmp (element_name, "other-files") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_OTHER_FILES;
          expidus_stack_push (parser->stack, PARSER_OTHER_FILES);
        }
      else if (strcmp (element_name, "text-files") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_TEXT_FILES;
          expidus_stack_push (parser->stack, PARSER_TEXT_FILES);
        }
      else if (strcmp (element_name, "video-files") == 0)
        {
          parser->types |= LUNAR_UCA_TYPE_VIDEO_FILES;
          expidus_stack_push (parser->stack, PARSER_VIDEO_FILES);
        }
      else
        expidus_stack_push (parser->stack, PARSER_UNKNOWN_ELEMENT);
      break;
                     ...
                     GError             **error)
{
  ...
  switch (expidus_stack_top (parser->stack))
    ...
    case PARSER_ACTION:
      if (strcmp (element_name, "action") == 0)
        {
          lunar_uca_model_append (parser->model, &iter);
          lunar_uca_model_update (parser->model, &iter,
                                   parser->name->str,
                                   parser->submenu->str,
          ...
          if (endo_str_is_empty (parser->unique_id->str))
  ...
  expidus_stack_pop (parser->stack);
  return;
...
static gchar *
lunar_uca_model_get_unique_id (void)
{
...
/**
 * lunar_uca_model_get_default:
 *
 * Returns a reference to the default shared
 * #LunarUcaModel instance. The caller is
 * responsible to free the returned instance
 * using g_object_unref() when no longer
 * needed.
 *
 * Return value: a reference to the default
 *               #LunarUcaModel instance.
 **/
LunarUcaModel*
lunar_uca_model_get_default (void)
{
  static LunarUcaModel *model = NULL;

  if (model == NULL)
    {
      model = g_object_new (LUNAR_UCA_TYPE_MODEL, NULL);
...
/**
 * lunar_uca_model_match:
 * @uca_model  : a #LunarUcaModel.
 * @file_infos : a #GList of #LunarxFileInfo<!---->s.
 *
 * Returns the #GtkTreePath<!---->s for the items in @uca_model,
 * that are defined for the #LunarxFileInfo<!---->s in the
 * @file_infos.
 *
 ...
 **/
GList*
lunar_uca_model_match (LunarUcaModel *uca_model,
                        GList          *file_infos)
{
  typedef struct
  {
  ...
  /* determine the LunarUcaFile's for the given file_infos */
  n_files = g_list_length (file_infos);
  files = g_new (LunarUcaFile, n_files);
  for (lp = file_infos, n = 0; lp != NULL; lp = lp->next, ++n)
    {
      location = lunarx_file_info_get_location (lp->data);
      ...
      mime_type = lunarx_file_info_get_mime_type (lp->data);

      files[n].name = lunarx_file_info_get_name (lp->data);
      files[n].types = types_from_mime_type (mime_type);

      if (G_UNLIKELY (files[n].types == 0))
        files[n].types = LUNAR_UCA_TYPE_OTHER_FILES;
  ...
  for (i = 0, lp = uca_model->items; lp != NULL; ++i, lp = lp->next)
    {
      ...
      item = (LunarUcaModelItem *) lp->data;
 ...
 **/
void
lunar_uca_model_append (LunarUcaModel *uca_model,
                         GtkTreeIter    *iter)
{
  LunarUcaModelItem *item;
  GtkTreePath        *path;
  ...
  /* append the new item */
  item = g_new0 (LunarUcaModelItem, 1);
 ...
 **/
void
lunar_uca_model_exchange (LunarUcaModel *uca_model,
                           GtkTreeIter    *iter_a,
                           GtkTreeIter    *iter_b)
{
  LunarUcaModelItem *item;
 ...
 **/
void
lunar_uca_model_remove (LunarUcaModel *uca_model,
                         GtkTreeIter    *iter)
{
  LunarUcaModelItem *item;
  GtkTreePath        *path;
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (uca_model), iter,
                      LUNAR_UCA_MODEL_COLUMN_UNIQUE_ID, &unique_id, 
                      -1);
  accel_path = g_strdup_printf ("<Actions>/LunarActions/uca-action-%s", unique_id);
  ...
  lunar_uca_model_item_free (item);
 ...
 **/
void
lunar_uca_model_update (LunarUcaModel *uca_model,
                         GtkTreeIter    *iter,
                         const gchar    *name,
                         const gchar    *submenu,
                         ...
                         GdkModifierType accel_mods)
{
  ...
  if (item->unique_id == NULL)
    {
      if (G_LIKELY (unique_id != NULL && *unique_id != '\0'))
        ...
        item->unique_id = lunar_uca_model_get_unique_id ();
    }
  ...
  if (accel_key > 0)
    {
      accel_path = g_strdup_printf ("<Actions>/LunarActions/uca-action-%s", item->unique_id);
 ...
 **/
gboolean
lunar_uca_model_save (LunarUcaModel *uca_model,
                       GError        **error)
{
  LunarUcaModelItem *item;
  ...
  path = expidus_resource_save_location (EXPIDUS_RESOURCE_CONFIG, "Lunar/uca.xml", TRUE);
  ...
  for (lp = uca_model->items; lp != NULL; lp = lp->next)
    {
      ...
      if ((item->types & LUNAR_UCA_TYPE_DIRECTORIES) != 0)
        fprintf (fp, "\t<directories/>\n");
      if ((item->types & LUNAR_UCA_TYPE_AUDIO_FILES) != 0)
        fprintf (fp, "\t<audio-files/>\n");
      if ((item->types & LUNAR_UCA_TYPE_IMAGE_FILES) != 0)
        fprintf (fp, "\t<image-files/>\n");
      if ((item->types & LUNAR_UCA_TYPE_OTHER_FILES) != 0)
        fprintf (fp, "\t<other-files/>\n");
      if ((item->types & LUNAR_UCA_TYPE_TEXT_FILES) != 0)
        fprintf (fp, "\t<text-files/>\n");
      if ((item->types & LUNAR_UCA_TYPE_VIDEO_FILES) != 0)
...
/**
 * lunar_uca_model_parse_argv:
 * @uca_model   : a #LunarUcaModel.
 * @iter        : the #GtkTreeIter of the item.
 * @file_infos  : the #GList of #LunarxFileInfo<!---->s to pass to the item.
 ...
 **/
gboolean
lunar_uca_model_parse_argv (LunarUcaModel *uca_model,
                             GtkTreeIter    *iter,
                             GList          *file_infos,
                             gint           *argcp,
                             gchar        ***argvp,
                             GError        **error)
{
  ...
  item = (LunarUcaModelItem *) ((GList *) iter->user_data)->data;
          ...
          switch (*++p)
            ...
            case 'd':
              if (G_LIKELY (file_infos != NULL))
                {
                  location = lunarx_file_info_get_location (file_infos->data);
                  ...
                  expidus_append_quoted (command_line, dirname);
                  g_free (dirname);
                  g_free (path);
                }
              break;
            ...
            case 'n':
              if (G_LIKELY (file_infos != NULL))
                {
                  path = lunarx_file_info_get_name (file_infos->data);
                  expidus_append_quoted (command_line, path);
                  g_free (path);
                }
              break;
              ...
              for (lp = file_infos; lp != NULL; lp = lp->next)
                {
                  ...
                  path = lunarx_file_info_get_name (lp->data);
                  expidus_append_quoted (command_line, path);
                  g_free (path);
                }
              break;
  ...
  for (lp = file_infos; lp != NULL; lp = lp->next)
    uri_list = g_slist_prepend (uri_list, lunarx_file_info_get_uri (lp->data));
  uri_list = g_slist_reverse (uri_list);

  expanded = expidus_expand_desktop_entry_field_codes (command_line->str, uri_list,
