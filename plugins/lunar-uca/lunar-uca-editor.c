...
static const gchar   *lunar_uca_editor_get_icon_name          (const LunarUcaEditor  *uca_editor);
static void           lunar_uca_editor_set_icon_name          (LunarUcaEditor        *uca_editor,
                                                                const gchar            *icon_name);
static LunarUcaTypes lunar_uca_editor_get_types              (const LunarUcaEditor  *uca_editor);
static void           lunar_uca_editor_set_types              (LunarUcaEditor        *uca_editor,
                                                                LunarUcaTypes          types);
static void           lunar_uca_editor_command_clicked        (LunarUcaEditor        *uca_editor);
static void           lunar_uca_editor_shortcut_clicked       (LunarUcaEditor        *uca_editor);
static void           lunar_uca_editor_shortcut_clear_clicked (LunarUcaEditor        *uca_editor);
static void           lunar_uca_editor_icon_clicked           (LunarUcaEditor        *uca_editor);
static void           lunar_uca_editor_constructed            (GObject                *object);



struct _LunarUcaEditorClass
{
  GtkDialogClass __parent__;
};

struct _LunarUcaEditor
{
...
LUNARX_DEFINE_TYPE (LunarUcaEditor, lunar_uca_editor, GTK_TYPE_DIALOG);



static void
lunar_uca_editor_class_init (LunarUcaEditorClass *klass)
{
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);

  /* vfuncs */
  object_class->constructed = lunar_uca_editor_constructed;

  /* Setup the template xml */
  gtk_widget_class_set_template_from_resource (widget_class, "/com/expidus/lunar/uca/editor.ui");

  /* bind stuff */
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, notebook);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, name_entry);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, sub_menu_entry);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, description_entry);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, icon_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, command_entry);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, shortcut_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, sn_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, patterns_entry);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, directories_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, audio_files_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, image_files_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, text_files_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, video_files_button);
  gtk_widget_class_bind_template_child (widget_class, LunarUcaEditor, other_files_button);

  gtk_widget_class_bind_template_callback(widget_class, lunar_uca_editor_icon_clicked);
  gtk_widget_class_bind_template_callback(widget_class, lunar_uca_editor_command_clicked);
  gtk_widget_class_bind_template_callback(widget_class, lunar_uca_editor_shortcut_clicked);
  gtk_widget_class_bind_template_callback(widget_class, lunar_uca_editor_shortcut_clear_clicked);
}



static void
lunar_uca_editor_init (LunarUcaEditor *uca_editor)
{
...
static void
lunar_uca_editor_constructed (GObject *object)
{
  LunarUcaEditor *editor = LUNAR_UCA_EDITOR (object);

  G_OBJECT_CLASS (lunar_uca_editor_parent_class)->constructed (object);
...
static void
lunar_uca_editor_command_clicked (LunarUcaEditor *uca_editor)
{
  ...
  g_return_if_fail (LUNAR_UCA_IS_EDITOR (uca_editor));
...
static void
lunar_uca_editor_shortcut_check (gpointer        data,
                                  const gchar    *path,
                                  guint           key,
                                  GdkModifierType mods,
                                  gboolean        changed)
...
static gboolean
lunar_uca_editor_validate_shortcut (ExpidusShortcutDialog  *dialog,
                                     const gchar         *shortcut,
                                     LunarUcaEditor     *uca_editor)
{
  GdkModifierType accel_mods;
  guint           accel_key;
  ShortcutInfo    info;
  ...
  gtk_accel_map_foreach_unfiltered (&info, lunar_uca_editor_shortcut_check);
  ...
  if (info.in_use)
    {
      ...
      message = g_strdup_printf (_("This keyboard shortcut is currently used by: '%s'"),
                                 command);
      expidus_dialog_show_warning (GTK_WINDOW (dialog), message,
...
static void
lunar_uca_editor_shortcut_clicked (LunarUcaEditor *uca_editor)
{
  GtkWidget       *dialog;
  gint             response;
  const gchar     *shortcut;
  ...
  dialog = expidus_shortcut_dialog_new ("lunar",
                                     gtk_entry_get_text (GTK_ENTRY (uca_editor->name_entry)), "");

  g_signal_connect (dialog, "validate-shortcut",
                    G_CALLBACK (lunar_uca_editor_validate_shortcut),
                    uca_editor);

  response = expidus_shortcut_dialog_run (EXPIDUS_SHORTCUT_DIALOG (dialog),
                                       gtk_widget_get_toplevel (uca_editor->shortcut_button));

  if (G_LIKELY (response == GTK_RESPONSE_OK))
    {
      shortcut = expidus_shortcut_dialog_get_shortcut (EXPIDUS_SHORTCUT_DIALOG (dialog));
      gtk_accelerator_parse (shortcut, &accel_key, &accel_mods);
...
static void
lunar_uca_editor_shortcut_clear_clicked (LunarUcaEditor *uca_editor)
{
...
static void
lunar_uca_editor_icon_clicked (LunarUcaEditor *uca_editor)
{
  const gchar *name;
  GtkWidget   *chooser;
  gchar       *title;
  ...
  chooser = endo_icon_chooser_dialog_new (title, GTK_WINDOW (uca_editor),
                                         _("_Cancel"), GTK_RESPONSE_CANCEL,
  ...
  icon = g_object_get_data (G_OBJECT (uca_editor->icon_button), "lunar-uca-icon-name");
  if (G_LIKELY (icon != NULL && *icon != '\0'))
    endo_icon_chooser_dialog_set_icon (ENDO_ICON_CHOOSER_DIALOG (chooser), icon);
  ...
  if (gtk_dialog_run (GTK_DIALOG (chooser)) == GTK_RESPONSE_ACCEPT)
    {
      ...
      lunar_uca_editor_set_icon_name (uca_editor, icon);
      g_free (icon);
    }
...
static const gchar*
lunar_uca_editor_get_icon_name (const LunarUcaEditor *uca_editor)
{
  g_return_val_if_fail (LUNAR_UCA_IS_EDITOR (uca_editor), NULL);
  return g_object_get_data (G_OBJECT (uca_editor->icon_button), "lunar-uca-icon-name");
}



static void
lunar_uca_editor_set_icon_name (LunarUcaEditor *uca_editor,
                                 const gchar     *icon_name)
{
  GIcon     *icon = NULL;
  GtkWidget *image;
  ...
  if (G_LIKELY (icon != NULL))
    {
      ...
      g_object_set_data_full (G_OBJECT (uca_editor->icon_button), "lunar-uca-icon-name", g_strdup (icon_name), g_free);
  ...
  else
    {
      ...
      g_object_set_data (G_OBJECT (uca_editor->icon_button), "lunar-uca-icon-name", NULL);
...
static void
lunar_uca_editor_set_types (LunarUcaEditor *uca_editor,
                             LunarUcaTypes   types)
{
  g_return_if_fail (LUNAR_UCA_IS_EDITOR (uca_editor));

  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->directories_button), (types & LUNAR_UCA_TYPE_DIRECTORIES));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->audio_files_button), (types & LUNAR_UCA_TYPE_AUDIO_FILES));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->image_files_button), (types & LUNAR_UCA_TYPE_IMAGE_FILES));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->text_files_button),  (types & LUNAR_UCA_TYPE_TEXT_FILES));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->video_files_button), (types & LUNAR_UCA_TYPE_VIDEO_FILES));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (uca_editor->other_files_button), (types & LUNAR_UCA_TYPE_OTHER_FILES));
}



/**
 * lunar_uca_editor_load:
 * @uca_editor  : a #LunarUcaEditor.
 ...
 **/
void
lunar_uca_editor_load (LunarUcaEditor *uca_editor,
                        ...
                        GtkTreeIter     *iter)
{
  ...
  gtk_tree_model_get (GTK_TREE_MODEL (uca_model), iter,
                      LUNAR_UCA_MODEL_COLUMN_DESCRIPTION, &description,
                      LUNAR_UCA_MODEL_COLUMN_PATTERNS, &patterns,
                      LUNAR_UCA_MODEL_COLUMN_COMMAND, &command,
                      LUNAR_UCA_MODEL_COLUMN_TYPES, &types,
                      LUNAR_UCA_MODEL_COLUMN_ICON_NAME, &icon_name,
                      LUNAR_UCA_MODEL_COLUMN_NAME, &name,
                      LUNAR_UCA_MODEL_COLUMN_SUB_MENU, &submenu,
                      LUNAR_UCA_MODEL_COLUMN_STARTUP_NOTIFY, &startup_notify,
                      LUNAR_UCA_MODEL_COLUMN_UNIQUE_ID, &unique_id,
                      -1);

  /* setup the new selection */
  lunar_uca_editor_set_types (uca_editor, types);

  /* setup the new icon */
  lunar_uca_editor_set_icon_name (uca_editor, icon_name);

  /* Resolve shortcut from accelerator */
  uca_editor->accel_path = g_strdup_printf ("<Actions>/LunarActions/uca-action-%s", unique_id);
 ...
 **/
void
lunar_uca_editor_save (LunarUcaEditor *uca_editor,
                        LunarUcaModel  *uca_model,
                        GtkTreeIter     *iter)
{
  gchar         *unique_id;
  ...
  /* always clear the accelerator, it'll be updated in lunar_uca_model_update */
  ...
  lunar_uca_model_update (uca_model, iter,
                           ...
                           lunar_uca_editor_get_types (uca_editor),
