...
/**
 * SECTION: lunarx-menu-item
 * @short_description: The base class for menu items added to the context menus
 * @title: LunarxMenuItem
 * @include: lunarx/lunarx.h
 *
 * The class for menu items that can be added to Lunar's context menus
 * by extensions implementing the #LunarxMenuProvider, #LunarxPreferencesProvider
 * or #LunarxRenamerProvider interfaces. The items returned by extensions from
...
static void lunarx_menu_item_get_property (GObject      *object,
                                            guint         param_id,
                                            GValue       *value,
                                            GParamSpec   *pspec);
static void lunarx_menu_item_set_property (GObject      *object,
                                            guint         param_id,
                                            const GValue *value,
                                            GParamSpec   *pspec);
static void lunarx_menu_item_finalize     (GObject      *object);



struct _LunarxMenuItemPrivate
{
  gchar       *name;
  gchar       *label;
  gchar       *tooltip;
...
G_DEFINE_TYPE_WITH_PRIVATE (LunarxMenuItem, lunarx_menu_item, G_TYPE_OBJECT)



static void
lunarx_menu_item_class_init (LunarxMenuItemClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lunarx_menu_item_finalize;
  gobject_class->get_property = lunarx_menu_item_get_property;
  gobject_class->set_property = lunarx_menu_item_set_property;

  signals[ACTIVATE] =
      g_signal_new ("activate",
                    G_TYPE_FROM_CLASS (klass),
                    G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET (LunarxMenuItemClass,
                                     activate),
                    NULL, NULL,
...
static void
lunarx_menu_item_init (LunarxMenuItem *item)
{
  item->priv = lunarx_menu_item_get_instance_private (item);
...
static void
lunarx_menu_item_get_property (GObject    *object,
                                guint       param_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  LunarxMenuItem *item = LUNARX_MENU_ITEM (object);
...
static void
lunarx_menu_item_set_property (GObject      *object,
                                guint         param_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
...
static void
lunarx_menu_item_finalize (GObject *object)
{
  LunarxMenuItem *item = LUNARX_MENU_ITEM (object);
  ...
  (*G_OBJECT_CLASS (lunarx_menu_item_parent_class)->finalize) (object);
}



/**
 * lunarx_menu_item_new:
 ...
 *
 * Returns: a newly created #LunarxMenuItem
 */
LunarxMenuItem *
lunarx_menu_item_new (const gchar *name,
                       const gchar *label,
                       const gchar *tooltip,
                       const gchar *icon)
{
  ...
  return g_object_new (LUNARX_TYPE_MENU_ITEM,
                       "name", name,
                       "label", label,
                       "tooltip", tooltip,
                       "icon", icon,
...
/**
 * lunarx_menu_item_activate:
 * @item: pointer to a #LunarxMenuItem instance
 *
 * Emits the activate signal.
 */
void
lunarx_menu_item_activate (LunarxMenuItem *item)
{
...
/**
 * lunarx_menu_item_get_sensitive:
 * @item: pointer to a #LunarxMenuItem instance
 *
 * Returns whether the menu item is sensitive.
 */
gboolean
lunarx_menu_item_get_sensitive (LunarxMenuItem *item)
{
  g_return_val_if_fail (LUNARX_IS_MENU_ITEM (item), FALSE);
...
/**
 * lunarx_menu_item_set_sensitive:
 * @item: pointer to a #LunarxMenuItem instance
 ...
 */
void
lunarx_menu_item_set_sensitive (LunarxMenuItem *item,
                                 gboolean         sensitive)
{
...
/**
 * lunarx_menu_item_set_menu:
 * @item: pointer to a #LunarxMenuItem instance
 * @menu: pointer to a #LunarxMenu instance
 *
 * Attaches @menu to menu item.
 */
void
lunarx_menu_item_set_menu (LunarxMenuItem *item,
                            LunarxMenu     *menu)
{
  g_return_if_fail (LUNARX_IS_MENU_ITEM (item));
  g_return_if_fail (LUNARX_IS_MENU (menu));

  g_object_set (item, "menu", LUNARX_MENU (menu), NULL);
}



/**
 * lunarx_menu_item_list_free:
 * @items: (element-type LunarxMenuItem): a list of #LunarxMenuItem
 */
void
lunarx_menu_item_list_free (GList *items)
{
