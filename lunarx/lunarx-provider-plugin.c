...
static void lunarx_provider_plugin_class_init (gpointer klass);

/**
 * SECTION: lunarx-provider-plugin
 * @short_description: The interface to the plugin type registration
 * @title: LunarxProviderPlugin
 * @include: lunarx/lunarx.h
 *
 * #LunarxProviderPlugin is the interface to the file manager's type plugin loader implementation. It
 * provides methods to extensions - also called <emphasis>provider plugins</emphasis> - to register dynamic
 * types and control certain aspects of the plugin (see lunarx_provider_plugin_set_resident()).
 */

GType
lunarx_provider_plugin_get_type (void)
{
  ...
  if (g_once_init_enter (&type__volatile))
    {
      type = g_type_register_static_simple (G_TYPE_INTERFACE,
                                            I_("LunarxProviderPlugin"),
                                            sizeof (LunarxProviderPluginIface),
                                            (GClassInitFunc) (void (*)(void)) lunarx_provider_plugin_class_init,
                                            0,
                                            NULL,
                                            0);
...
static void
lunarx_provider_plugin_class_init (gpointer klass)
{
  /**
   * LunarxProviderPlugin:resident:
   *
   * Tells whether a plugin must reside in memory once loaded for
   * the first time. See lunarx_provider_plugin_get_resident() and
   * lunarx_provider_plugin_set_resident() for more details.
   **/
...
/**
 * lunarx_provider_plugin_get_resident:
 * @plugin : a #LunarxProviderPlugin.
 *
 ...
 **/
gboolean
lunarx_provider_plugin_get_resident (const LunarxProviderPlugin *plugin)
{
  g_return_val_if_fail (LUNARX_IS_PROVIDER_PLUGIN (plugin), FALSE);

  return (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->get_resident) (plugin);
}



/**
 * lunarx_provider_plugin_set_resident:
 * @plugin   : a #LunarxProviderPlugin.
 ...
 * %lunar_extension_initialize method of the @plugin.
 **/
void
lunarx_provider_plugin_set_resident (LunarxProviderPlugin *plugin,
                                      gboolean               resident)
{
  g_return_if_fail (LUNARX_IS_PROVIDER_PLUGIN (plugin));

  (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->set_resident) (plugin, resident);
}



/**
 * lunarx_provider_plugin_register_type:
 * @plugin      : a #LunarxProviderPlugin.
 ...
 **/
GType
lunarx_provider_plugin_register_type (LunarxProviderPlugin *plugin,
                                       GType                  type_parent,
                                       const gchar           *type_name,
                                       const GTypeInfo       *type_info,
                                       GTypeFlags             type_flags)
{
  g_return_val_if_fail (LUNARX_IS_PROVIDER_PLUGIN (plugin), G_TYPE_INVALID);
  ...
  return (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->register_type) (plugin, type_parent, type_name, type_info, type_flags);
}



/**
 * lunarx_provider_plugin_add_interface:
 * @plugin         : a #LunarxProviderPlugin.
 ...
 **/
void
lunarx_provider_plugin_add_interface (LunarxProviderPlugin *plugin,
                                       GType                  instance_type,
                                       ...
                                       const GInterfaceInfo  *interface_info)
{
  ...
  (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->add_interface) (plugin, instance_type, interface_type, interface_info);
}



/**
 * lunarx_provider_plugin_register_enum:
 * @plugin              : a #LunarxProviderPlugin.
 ...
 **/
GType
lunarx_provider_plugin_register_enum (LunarxProviderPlugin *plugin,
                                       const gchar           *name,
                                       const GEnumValue      *const_static_values)
{
  ...
  return (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->register_enum) (plugin, name, const_static_values);
}



/**
 * lunarx_provider_plugin_register_flags:
 * @plugin              : a #LunarxProviderPlugin.
 ...
 **/
GType
lunarx_provider_plugin_register_flags (LunarxProviderPlugin *plugin,
                                        const gchar           *name,
                                        const GFlagsValue     *const_static_values)
{
  ...
  return (*LUNARX_PROVIDER_PLUGIN_GET_IFACE (plugin)->register_flags) (plugin, name, const_static_values);
}
