...
/**
 * SECTION: lunarx-renamer-provider
 * @short_description: The interface to extensions that provide additional bulk renamers
 * @title: LunarxRenamerProvider
 * @include: lunarx/lunarx.h
 *
 * The <interface>LunarxRenamerProvider</interface> interface is implemented by
 * extensions which provide additional bulk renamers that should be used by the
 * bulk rename dialog in Lunar.
 */

GType
lunarx_renamer_provider_get_type (void)
{
  ...
  if (g_once_init_enter (&type__volatile))
    {
      type = g_type_register_static_simple (G_TYPE_INTERFACE,
                                            I_("LunarxRenamerProvider"),
                                            sizeof (LunarxRenamerProviderIface),
                                            NULL,
                                            0,
                                            NULL,
                                            0);
...
/**
 * lunarx_renamer_provider_get_renamers: (skip)
 * @provider : a #LunarxRenamerProvider.
 *
 * Returns the list of #LunarxRenamer<!---->s provided by the
 * specified @provider.
 *
 * The real implementation of this method MUST return the #LunarxRenamer<!---->s
 * with floating references (the default for g_object_new() on #GtkWidget
 * derived types).
 *
 * The returned #LunarxRenamer<!---->s will be reffed and sinked automatically
 * by this function.
 *
 * As a special note, this method automatically takes a reference on the
 * @provider for every #LunarxRenamer returned from the real implementation
 ...
 *
 * Returns: (transfer full) (element-type LunarxRenamer): the list of #LunarxRenamer<!---->s provided by the
 *               specified @provider.
 **/
GList*
lunarx_renamer_provider_get_renamers (LunarxRenamerProvider *provider)
{
  GList *renamers;
  GList *lp;

  g_return_val_if_fail (LUNARX_IS_RENAMER_PROVIDER (provider), NULL);

  /* determine the renamers from the real implementation */
  renamers = (*LUNARX_RENAMER_PROVIDER_GET_IFACE (provider)->get_renamers) (provider);
  ...
  lunarx_object_list_take_reference (renamers, provider);
