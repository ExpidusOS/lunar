...
/**
 * SECTION: lunarx-preferences-provider
 * @short_description: The interface to extensions that provide preferences
 * @title: LunarxPreferencesProvider
 * @include: lunarx/lunarx.h
 *
 * The LunarxPreferencesProvider interface is implemented by extensions that
 ...
 * tied to the file manager (for example, the <literal>lunar-uca</literal> is
 ...
 *
 * The name of <link linkend="LunarxMenuItem"><type>LunarxMenuItem</type></link>s
 * returned from the lunarx_preferences_provider_get_menu items() method must be
 ...
 * provided by the <literal>lunar-uca</literal> extension is called
 * <literal>LunarUca::manage-menu-items</literal>.
 */

GType
lunarx_preferences_provider_get_type (void)
{
  ...
  if (g_once_init_enter (&type__volatile))
    {
      type = g_type_register_static_simple (G_TYPE_INTERFACE,
                                            I_("LunarxPreferencesProvider"),
                                            sizeof (LunarxPreferencesProviderIface),
                                            NULL,
                                            0,
                                            NULL,
                                            0);
...
/**
 * lunarx_preferences_provider_get_menu_items: (skip)
 * @provider : a #LunarxPreferencesProvider.
 * @window   : the #GtkWindow within which the menu items will be used.
 *
 * Returns the list of #LunarxMenuItem<!---->s that @provider has to offer
 ...
 *          #LunarxMenuItem<!---->s that @provider has to offer as preferences
 *          within @window.
 **/
GList*
lunarx_preferences_provider_get_menu_items (LunarxPreferencesProvider *provider,
                                             GtkWidget                  *window)
{
  GList *items;

  g_return_val_if_fail (LUNARX_IS_PREFERENCES_PROVIDER (provider), NULL);
  g_return_val_if_fail (GTK_IS_WINDOW (window), NULL);

  if (LUNARX_PREFERENCES_PROVIDER_GET_IFACE (provider)->get_menu_items != NULL)
    {
      /* query the menu items from the implementation */
      items = (*LUNARX_PREFERENCES_PROVIDER_GET_IFACE (provider)->get_menu_items) (provider, window);
